...
$pc_issuance_list = NULL; // Initialize page object first

class cpc_issuance_list extends cpc_issuance {
	...
	// Page object name
	var $PageObjName = 'pc_issuance_list';

	// Grid form hidden field names
	var $FormName = 'fpc_issuancelist';
	...
	function __construct() {
		...
		$this->AddUrl = "pc_issuanceadd.php";
		$this->InlineAddUrl = $this->PageUrl() . "a=add";
		$this->GridAddUrl = $this->PageUrl() . "a=gridadd";
		$this->GridEditUrl = $this->PageUrl() . "a=gridedit";
		$this->MultiDeleteUrl = "pc_issuancedelete.php";
		$this->MultiUpdateUrl = "pc_issuanceupdate.php";
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fpc_issuancelistsrch";
	...
	function GetFilterList() {
		...
		if (EW_SEARCH_FILTER_OPTION == "Server" && isset($UserProfile))
			$sSavedFilterList = $UserProfile->GetSearchFilters(CurrentUserName(), "fpc_issuancelistsrch");
	...
	function ProcessFilterList() {
		...
		if (@$_POST["ajax"] == "savefilters") { // Save filter request (Ajax)
			...
			$UserProfile->SetSearchFilters(CurrentUserName(), "fpc_issuancelistsrch", $filters);
	...
	function SetupOtherOptions() {
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"fpc_issuancelistsrch\" href=\"#\">" . $Language->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"fpc_issuancelistsrch\" href=\"#\">" . $Language->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->ListActions->Items as $listaction) {
				if ($listaction->Select == EW_ACTION_MULTIPLE) {
					...
					$item->Body = "<a class=\"ewAction ewListAction\" title=\"" . ew_HtmlEncode($caption) . "\" data-caption=\"" . ew_HtmlEncode($caption) . "\" href=\"\" onclick=\"ew_SubmitAction(event,jQuery.extend({f:document.fpc_issuancelist}," . $listaction->ToJson(TRUE) . "));return false;\">" . $icon . "</a>";
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $Language->Phrase("SearchPanel") . "\" data-caption=\"" . $Language->Phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fpc_issuancelistsrch\">" . $Language->Phrase("SearchLink") . "</button>";
		$item->Visible = TRUE;
...
// Create page object
if (!isset($pc_issuance_list)) $pc_issuance_list = new cpc_issuance_list();

// Page init
$pc_issuance_list->Page_Init();

// Page main
$pc_issuance_list->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$pc_issuance_list->Page_Render();
?>
...
var CurrentForm = fpc_issuancelist = new ew_Form("fpc_issuancelist", "list");
fpc_issuancelist.FormKeyCountName = '<?php echo $pc_issuance_list->FormKeyCountName ?>';

// Form_CustomValidate event
fpc_issuancelist.Form_CustomValidate = 
...
<?php if ($pc_issuance->Export == "") { ?>
<div class="ewToolbar">
<?php if ($pc_issuance_list->TotalRecs > 0 && $pc_issuance_list->ExportOptions->Visible()) { ?>
<?php $pc_issuance_list->ExportOptions->Render("body") ?>
<?php } ?>
<?php if ($pc_issuance_list->SearchOptions->Visible()) { ?>
<?php $pc_issuance_list->SearchOptions->Render("body") ?>
<?php } ?>
<?php if ($pc_issuance_list->FilterOptions->Visible()) { ?>
<?php $pc_issuance_list->FilterOptions->Render("body") ?>
<?php } ?>
<div class="clearfix"></div>
</div>
<?php } ?>
<?php
	$bSelectLimit = $pc_issuance_list->UseSelectLimit;
	if ($bSelectLimit) {
		if ($pc_issuance_list->TotalRecs <= 0)
			$pc_issuance_list->TotalRecs = $pc_issuance->ListRecordCount();
	} else {
		if (!$pc_issuance_list->Recordset && ($pc_issuance_list->Recordset = $pc_issuance_list->LoadRecordset()))
			$pc_issuance_list->TotalRecs = $pc_issuance_list->Recordset->RecordCount();
	}
	$pc_issuance_list->StartRec = 1;
	if ($pc_issuance_list->DisplayRecs <= 0 || ($pc_issuance->Export <> "" && $pc_issuance->ExportAll)) // Display all records
		$pc_issuance_list->DisplayRecs = $pc_issuance_list->TotalRecs;
	if (!($pc_issuance->Export <> "" && $pc_issuance->ExportAll))
		$pc_issuance_list->SetupStartRec(); // Set up start record position
	if ($bSelectLimit)
		$pc_issuance_list->Recordset = $pc_issuance_list->LoadRecordset($pc_issuance_list->StartRec-1, $pc_issuance_list->DisplayRecs);

	// Set no record found message
	if ($pc_issuance->CurrentAction == "" && $pc_issuance_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$pc_issuance_list->setWarningMessage(ew_DeniedMsg());
		if ($pc_issuance_list->SearchWhere == "0=101")
...
<?php if ($pc_issuance->Export == "" && $pc_issuance->CurrentAction == "") { ?>
<form name="fpc_issuancelistsrch" id="fpc_issuancelistsrch" class="form-inline ewForm ewExtSearchForm" action="<?php echo ew_CurrentPage() ?>">
<?php $SearchPanelClass = ($pc_issuance_list->SearchWhere <> "") ? " in" : " in"; ?>
<div id="fpc_issuancelistsrch_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
<input type="hidden" name="cmd" value="search">
<input type="hidden" name="t" value="pc_issuance">
	<div class="ewBasicSearch">
<div id="xsr_1" class="ewRow">
	<div class="ewQuickSearch input-group">
	<input type="text" name="<?php echo EW_TABLE_BASIC_SEARCH ?>" id="<?php echo EW_TABLE_BASIC_SEARCH ?>" class="form-control" value="<?php echo ew_HtmlEncode($pc_issuance_list->BasicSearch->getKeyword()) ?>" placeholder="<?php echo ew_HtmlEncode($Language->Phrase("Search")) ?>">
	<input type="hidden" name="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" id="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" value="<?php echo ew_HtmlEncode($pc_issuance_list->BasicSearch->getType()) ?>">
	<div class="input-group-btn">
		<button type="button" data-toggle="dropdown" class="btn btn-default"><span id="searchtype"><?php echo $pc_issuance_list->BasicSearch->getTypeNameShort() ?></span><span class="caret"></span></button>
		<ul class="dropdown-menu pull-right" role="menu">
			<li<?php if ($pc_issuance_list->BasicSearch->getType() == "") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this)"><?php echo $Language->Phrase("QuickSearchAuto") ?></a></li>
			<li<?php if ($pc_issuance_list->BasicSearch->getType() == "=") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'=')"><?php echo $Language->Phrase("QuickSearchExact") ?></a></li>
			<li<?php if ($pc_issuance_list->BasicSearch->getType() == "AND") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'AND')"><?php echo $Language->Phrase("QuickSearchAll") ?></a></li>
			<li<?php if ($pc_issuance_list->BasicSearch->getType() == "OR") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'OR')"><?php echo $Language->Phrase("QuickSearchAny") ?></a></li>
		</ul>
...
<?php } ?>
<?php } ?>
<?php $pc_issuance_list->ShowPageHeader(); ?>
<?php
$pc_issuance_list->ShowMessage();
?>
<?php if ($pc_issuance_list->TotalRecs > 0 || $pc_issuance->CurrentAction <> "") { ?>
<div class="box ewBox ewGrid<?php if ($pc_issuance_list->IsAddOrEdit()) { ?> ewGridAddEdit<?php } ?> pc_issuance">
<?php if ($pc_issuance->Export == "") { ?>
<div class="box-header ewGridUpperPanel">
<?php if ($pc_issuance->CurrentAction <> "gridadd" && $pc_issuance->CurrentAction <> "gridedit") { ?>
<form name="ewPagerForm" class="form-inline ewForm ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($pc_issuance_list->Pager)) $pc_issuance_list->Pager = new cPrevNextPager($pc_issuance_list->StartRec, $pc_issuance_list->DisplayRecs, $pc_issuance_list->TotalRecs, $pc_issuance_list->AutoHidePager) ?>
<?php if ($pc_issuance_list->Pager->RecordCount > 0 && $pc_issuance_list->Pager->Visible) { ?>
<div class="ewPager">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($pc_issuance_list->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $pc_issuance_list->PageUrl() ?>start=<?php echo $pc_issuance_list->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($pc_issuance_list->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $pc_issuance_list->PageUrl() ?>start=<?php echo $pc_issuance_list->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $pc_issuance_list->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($pc_issuance_list->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $pc_issuance_list->PageUrl() ?>start=<?php echo $pc_issuance_list->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($pc_issuance_list->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $pc_issuance_list->PageUrl() ?>start=<?php echo $pc_issuance_list->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $pc_issuance_list->Pager->PageCount ?></span>
</div>
<?php } ?>
<?php if ($pc_issuance_list->Pager->RecordCount > 0) { ?>
<div class="ewPager ewRec">
	<span><?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $pc_issuance_list->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $pc_issuance_list->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $pc_issuance_list->Pager->RecordCount ?></span>
</div>
<?php } ?>
<?php if ($pc_issuance_list->TotalRecs > 0 && (!$pc_issuance_list->AutoHidePageSizeSelector || $pc_issuance_list->Pager->Visible)) { ?>
<div class="ewPager">
<input type="hidden" name="t" value="pc_issuance">
<select name="<?php echo EW_TABLE_REC_PER_PAGE ?>" class="form-control input-sm ewTooltip" title="<?php echo $Language->Phrase("RecordsPerPage") ?>" onchange="this.form.submit();">
<option value="5"<?php if ($pc_issuance_list->DisplayRecs == 5) { ?> selected<?php } ?>>5</option>
<option value="10"<?php if ($pc_issuance_list->DisplayRecs == 10) { ?> selected<?php } ?>>10</option>
<option value="15"<?php if ($pc_issuance_list->DisplayRecs == 15) { ?> selected<?php } ?>>15</option>
<option value="20"<?php if ($pc_issuance_list->DisplayRecs == 20) { ?> selected<?php } ?>>20</option>
<option value="50"<?php if ($pc_issuance_list->DisplayRecs == 50) { ?> selected<?php } ?>>50</option>
<option value="ALL"<?php if ($pc_issuance->getRecordsPerPage() == -1) { ?> selected<?php } ?>><?php echo $Language->Phrase("AllRecords") ?></option>
</select>
</div>
<?php } ?>
</form>
...
<?php
	foreach ($pc_issuance_list->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<div class="clearfix"></div>
...
<?php } ?>
<form name="fpc_issuancelist" id="fpc_issuancelist" class="form-inline ewForm ewListForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($pc_issuance_list->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $pc_issuance_list->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="pc_issuance">
<div id="gmp_pc_issuance" class="<?php if (ew_IsResponsiveLayout()) { ?>table-responsive <?php } ?>ewGridMiddlePanel">
<?php if ($pc_issuance_list->TotalRecs > 0 || $pc_issuance->CurrentAction == "gridedit") { ?>
<table id="tbl_pc_issuancelist" class="table ewTable">
<thead>
	<tr class="ewTableHeader">
<?php

// Header row
$pc_issuance_list->RowType = EW_ROWTYPE_HEADER;

// Render list options
$pc_issuance_list->RenderListOptions();
...
<?php if ($pc_issuance->issued_date->Visible) { // issued_date ?>
	...
	<?php } else { ?>
		<th data-name="issued_date" class="<?php echo $pc_issuance->issued_date->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $pc_issuance->SortUrl($pc_issuance->issued_date) ?>',1);"><div id="elh_pc_issuance_issued_date" class="pc_issuance_issued_date">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $pc_issuance->issued_date->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($pc_issuance->issued_date->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($pc_issuance->issued_date->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($pc_issuance->reference_id->Visible) { // reference_id ?>
	<?php if ($pc_issuance->SortUrl($pc_issuance->reference_id) == "") { ?>
		<th data-name="reference_id" class="<?php echo $pc_issuance->reference_id->HeaderCellClass() ?>"><div id="elh_pc_issuance_reference_id" class="pc_issuance_reference_id"><div class="ewTableHeaderCaption"><?php echo $pc_issuance->reference_id->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="reference_id" class="<?php echo $pc_issuance->reference_id->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $pc_issuance->SortUrl($pc_issuance->reference_id) ?>',1);"><div id="elh_pc_issuance_reference_id" class="pc_issuance_reference_id">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $pc_issuance->reference_id->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($pc_issuance->reference_id->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($pc_issuance->reference_id->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
...
<?php if ($pc_issuance->asset_tag->Visible) { // asset_tag ?>
	...
	<?php } else { ?>
		<th data-name="asset_tag" class="<?php echo $pc_issuance->asset_tag->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $pc_issuance->SortUrl($pc_issuance->asset_tag) ?>',1);"><div id="elh_pc_issuance_asset_tag" class="pc_issuance_asset_tag">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $pc_issuance->asset_tag->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($pc_issuance->asset_tag->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($pc_issuance->asset_tag->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($pc_issuance->make->Visible) { // make ?>
	<?php if ($pc_issuance->SortUrl($pc_issuance->make) == "") { ?>
		<th data-name="make" class="<?php echo $pc_issuance->make->HeaderCellClass() ?>"><div id="elh_pc_issuance_make" class="pc_issuance_make"><div class="ewTableHeaderCaption"><?php echo $pc_issuance->make->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="make" class="<?php echo $pc_issuance->make->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $pc_issuance->SortUrl($pc_issuance->make) ?>',1);"><div id="elh_pc_issuance_make" class="pc_issuance_make">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $pc_issuance->make->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($pc_issuance->make->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($pc_issuance->make->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($pc_issuance->ram->Visible) { // ram ?>
	<?php if ($pc_issuance->SortUrl($pc_issuance->ram) == "") { ?>
		<th data-name="ram" class="<?php echo $pc_issuance->ram->HeaderCellClass() ?>"><div id="elh_pc_issuance_ram" class="pc_issuance_ram"><div class="ewTableHeaderCaption"><?php echo $pc_issuance->ram->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="ram" class="<?php echo $pc_issuance->ram->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $pc_issuance->SortUrl($pc_issuance->ram) ?>',1);"><div id="elh_pc_issuance_ram" class="pc_issuance_ram">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $pc_issuance->ram->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($pc_issuance->ram->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($pc_issuance->ram->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($pc_issuance->hard_disk->Visible) { // hard_disk ?>
	<?php if ($pc_issuance->SortUrl($pc_issuance->hard_disk) == "") { ?>
		<th data-name="hard_disk" class="<?php echo $pc_issuance->hard_disk->HeaderCellClass() ?>"><div id="elh_pc_issuance_hard_disk" class="pc_issuance_hard_disk"><div class="ewTableHeaderCaption"><?php echo $pc_issuance->hard_disk->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="hard_disk" class="<?php echo $pc_issuance->hard_disk->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $pc_issuance->SortUrl($pc_issuance->hard_disk) ?>',1);"><div id="elh_pc_issuance_hard_disk" class="pc_issuance_hard_disk">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $pc_issuance->hard_disk->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($pc_issuance->hard_disk->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($pc_issuance->hard_disk->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($pc_issuance->color->Visible) { // color ?>
	<?php if ($pc_issuance->SortUrl($pc_issuance->color) == "") { ?>
		<th data-name="color" class="<?php echo $pc_issuance->color->HeaderCellClass() ?>"><div id="elh_pc_issuance_color" class="pc_issuance_color"><div class="ewTableHeaderCaption"><?php echo $pc_issuance->color->FldCaption() ?></div></div></th>
	<?php } else { ?>
...
<?php if ($pc_issuance->department->Visible) { // department ?>
	<?php if ($pc_issuance->SortUrl($pc_issuance->department) == "") { ?>
		<th data-name="department" class="<?php echo $pc_issuance->department->HeaderCellClass() ?>"><div id="elh_pc_issuance_department" class="pc_issuance_department"><div class="ewTableHeaderCaption"><?php echo $pc_issuance->department->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="department" class="<?php echo $pc_issuance->department->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $pc_issuance->SortUrl($pc_issuance->department) ?>',1);"><div id="elh_pc_issuance_department" class="pc_issuance_department">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $pc_issuance->department->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($pc_issuance->department->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($pc_issuance->department->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($pc_issuance->designation->Visible) { // designation ?>
	<?php if ($pc_issuance->SortUrl($pc_issuance->designation) == "") { ?>
		<th data-name="designation" class="<?php echo $pc_issuance->designation->HeaderCellClass() ?>"><div id="elh_pc_issuance_designation" class="pc_issuance_designation"><div class="ewTableHeaderCaption"><?php echo $pc_issuance->designation->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="designation" class="<?php echo $pc_issuance->designation->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $pc_issuance->SortUrl($pc_issuance->designation) ?>',1);"><div id="elh_pc_issuance_designation" class="pc_issuance_designation">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $pc_issuance->designation->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($pc_issuance->designation->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($pc_issuance->designation->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($pc_issuance->assign_to->Visible) { // assign_to ?>
	<?php if ($pc_issuance->SortUrl($pc_issuance->assign_to) == "") { ?>
		<th data-name="assign_to" class="<?php echo $pc_issuance->assign_to->HeaderCellClass() ?>"><div id="elh_pc_issuance_assign_to" class="pc_issuance_assign_to"><div class="ewTableHeaderCaption"><?php echo $pc_issuance->assign_to->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="assign_to" class="<?php echo $pc_issuance->assign_to->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $pc_issuance->SortUrl($pc_issuance->assign_to) ?>',1);"><div id="elh_pc_issuance_assign_to" class="pc_issuance_assign_to">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $pc_issuance->assign_to->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($pc_issuance->assign_to->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($pc_issuance->assign_to->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($pc_issuance->date_assign->Visible) { // date_assign ?>
	<?php if ($pc_issuance->SortUrl($pc_issuance->date_assign) == "") { ?>
		<th data-name="date_assign" class="<?php echo $pc_issuance->date_assign->HeaderCellClass() ?>"><div id="elh_pc_issuance_date_assign" class="pc_issuance_date_assign"><div class="ewTableHeaderCaption"><?php echo $pc_issuance->date_assign->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="date_assign" class="<?php echo $pc_issuance->date_assign->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $pc_issuance->SortUrl($pc_issuance->date_assign) ?>',1);"><div id="elh_pc_issuance_date_assign" class="pc_issuance_date_assign">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $pc_issuance->date_assign->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($pc_issuance->date_assign->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($pc_issuance->date_assign->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($pc_issuance->assign_by->Visible) { // assign_by ?>
	...
	<?php } else { ?>
		<th data-name="assign_by" class="<?php echo $pc_issuance->assign_by->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $pc_issuance->SortUrl($pc_issuance->assign_by) ?>',1);"><div id="elh_pc_issuance_assign_by" class="pc_issuance_assign_by">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $pc_issuance->assign_by->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($pc_issuance->assign_by->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($pc_issuance->assign_by->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($pc_issuance->statuse->Visible) { // statuse ?>
	<?php if ($pc_issuance->SortUrl($pc_issuance->statuse) == "") { ?>
...
<?php if ($pc_issuance->date_retrieved->Visible) { // date_retrieved ?>
	<?php if ($pc_issuance->SortUrl($pc_issuance->date_retrieved) == "") { ?>
		<th data-name="date_retrieved" class="<?php echo $pc_issuance->date_retrieved->HeaderCellClass() ?>"><div id="elh_pc_issuance_date_retrieved" class="pc_issuance_date_retrieved"><div class="ewTableHeaderCaption"><?php echo $pc_issuance->date_retrieved->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="date_retrieved" class="<?php echo $pc_issuance->date_retrieved->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $pc_issuance->SortUrl($pc_issuance->date_retrieved) ?>',1);"><div id="elh_pc_issuance_date_retrieved" class="pc_issuance_date_retrieved">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $pc_issuance->date_retrieved->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($pc_issuance->date_retrieved->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($pc_issuance->date_retrieved->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
...
$pc_issuance_list->ListOptions->Render("header", "right");
?>
	</tr>
</thead>
<tbody>
<?php
if ($pc_issuance->ExportAll && $pc_issuance->Export <> "") {
	$pc_issuance_list->StopRec = $pc_issuance_list->TotalRecs;
} else {

	// Set the last record to display
	if ($pc_issuance_list->TotalRecs > $pc_issuance_list->StartRec + $pc_issuance_list->DisplayRecs - 1)
		$pc_issuance_list->StopRec = $pc_issuance_list->StartRec + $pc_issuance_list->DisplayRecs - 1;
	else
		$pc_issuance_list->StopRec = $pc_issuance_list->TotalRecs;
}
$pc_issuance_list->RecCnt = $pc_issuance_list->StartRec - 1;
if ($pc_issuance_list->Recordset && !$pc_issuance_list->Recordset->EOF) {
	$pc_issuance_list->Recordset->MoveFirst();
	$bSelectLimit = $pc_issuance_list->UseSelectLimit;
	if (!$bSelectLimit && $pc_issuance_list->StartRec > 1)
		$pc_issuance_list->Recordset->Move($pc_issuance_list->StartRec - 1);
} elseif (!$pc_issuance->AllowAddDeleteRow && $pc_issuance_list->StopRec == 0) {
	$pc_issuance_list->StopRec = $pc_issuance->GridAddRowCount;
}

// Initialize aggregate
$pc_issuance->RowType = EW_ROWTYPE_AGGREGATEINIT;
$pc_issuance->ResetAttrs();
$pc_issuance_list->RenderRow();
while ($pc_issuance_list->RecCnt < $pc_issuance_list->StopRec) {
	$pc_issuance_list->RecCnt++;
	if (intval($pc_issuance_list->RecCnt) >= intval($pc_issuance_list->StartRec)) {
		$pc_issuance_list->RowCnt++;

		// Set up key count
		$pc_issuance_list->KeyCount = $pc_issuance_list->RowIndex;

		// Init row class and style
		$pc_issuance->ResetAttrs();
		$pc_issuance->CssClass = "";
		if ($pc_issuance->CurrentAction == "gridadd") {
		} else {
			$pc_issuance_list->LoadRowValues($pc_issuance_list->Recordset); // Load row values
		}
		$pc_issuance->RowType = EW_ROWTYPE_VIEW; // Render view

		// Set up row id / data-rowindex
		$pc_issuance->RowAttrs = array_merge($pc_issuance->RowAttrs, array('data-rowindex'=>$pc_issuance_list->RowCnt, 'id'=>'r' . $pc_issuance_list->RowCnt . '_pc_issuance', 'data-rowtype'=>$pc_issuance->RowType));
...
<span id="el<?php echo $pc_issuance_list->RowCnt ?>_pc_issuance_reference_id" class="pc_issuance_reference_id">
...
<span id="el<?php echo $pc_issuance_list->RowCnt ?>_pc_issuance_make" class="pc_issuance_make">
...
<span id="el<?php echo $pc_issuance_list->RowCnt ?>_pc_issuance_ram" class="pc_issuance_ram">
...
<span id="el<?php echo $pc_issuance_list->RowCnt ?>_pc_issuance_hard_disk" class="pc_issuance_hard_disk">
...
<span id="el<?php echo $pc_issuance_list->RowCnt ?>_pc_issuance_department" class="pc_issuance_department">
...
<span id="el<?php echo $pc_issuance_list->RowCnt ?>_pc_issuance_designation" class="pc_issuance_designation">
...
<span id="el<?php echo $pc_issuance_list->RowCnt ?>_pc_issuance_assign_to" class="pc_issuance_assign_to">
...
<span id="el<?php echo $pc_issuance_list->RowCnt ?>_pc_issuance_date_assign" class="pc_issuance_date_assign">
...
<span id="el<?php echo $pc_issuance_list->RowCnt ?>_pc_issuance_statuse" class="pc_issuance_statuse">
...
<span id="el<?php echo $pc_issuance_list->RowCnt ?>_pc_issuance_date_retrieved" class="pc_issuance_date_retrieved">
...
$pc_issuance_list->ListOptions->Render("body", "right", $pc_issuance_list->RowCnt);
?>
	</tr>
<?php
	}
	if ($pc_issuance->CurrentAction <> "gridadd")
		$pc_issuance_list->Recordset->MoveNext();
}
?>
</tbody>
</table>
<?php } ?>
<?php if ($pc_issuance->CurrentAction == "") { ?>
...
// Close recordset
if ($pc_issuance_list->Recordset)
	$pc_issuance_list->Recordset->Close();
?>
</div>
<?php } ?>
<?php if ($pc_issuance_list->TotalRecs == 0 && $pc_issuance->CurrentAction == "") { // Show other options ?>
<div class="ewListOtherOptions">
<?php
	foreach ($pc_issuance_list->OtherOptions as &$option) {
		$option->ButtonClass = "";
...
<?php if ($pc_issuance->Export == "") { ?>
<script type="text/javascript">
fpc_issuancelistsrch.FilterList = <?php echo $pc_issuance_list->GetFilterList() ?>;
fpc_issuancelistsrch.Init();
fpc_issuancelist.Init();
</script>
<?php } ?>
<?php
$pc_issuance_list->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($pc_issuance->Export == "") { ?>
...
<?php
$pc_issuance_list->Page_Terminate();
?>
