...
$system_restock_list = NULL; // Initialize page object first

class csystem_restock_list extends csystem_restock {
	...
	// Page object name
	var $PageObjName = 'system_restock_list';

	// Grid form hidden field names
	var $FormName = 'fsystem_restocklist';
	...
	function __construct() {
		...
		$this->AddUrl = "system_restockadd.php";
		$this->InlineAddUrl = $this->PageUrl() . "a=add";
		$this->GridAddUrl = $this->PageUrl() . "a=gridadd";
		$this->GridEditUrl = $this->PageUrl() . "a=gridedit";
		$this->MultiDeleteUrl = "system_restockdelete.php";
		$this->MultiUpdateUrl = "system_restockupdate.php";
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fsystem_restocklistsrch";
	...
	function GetFilterList() {
		...
		if (EW_SEARCH_FILTER_OPTION == "Server" && isset($UserProfile))
			$sSavedFilterList = $UserProfile->GetSearchFilters(CurrentUserName(), "fsystem_restocklistsrch");
	...
	function ProcessFilterList() {
		...
		if (@$_POST["ajax"] == "savefilters") { // Save filter request (Ajax)
			...
			$UserProfile->SetSearchFilters(CurrentUserName(), "fsystem_restocklistsrch", $filters);
	...
	function SetupOtherOptions() {
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"fsystem_restocklistsrch\" href=\"#\">" . $Language->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"fsystem_restocklistsrch\" href=\"#\">" . $Language->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->ListActions->Items as $listaction) {
				if ($listaction->Select == EW_ACTION_MULTIPLE) {
					...
					$item->Body = "<a class=\"ewAction ewListAction\" title=\"" . ew_HtmlEncode($caption) . "\" data-caption=\"" . ew_HtmlEncode($caption) . "\" href=\"\" onclick=\"ew_SubmitAction(event,jQuery.extend({f:document.fsystem_restocklist}," . $listaction->ToJson(TRUE) . "));return false;\">" . $icon . "</a>";
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $Language->Phrase("SearchPanel") . "\" data-caption=\"" . $Language->Phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fsystem_restocklistsrch\">" . $Language->Phrase("SearchLink") . "</button>";
		$item->Visible = TRUE;
	...
	function Page_Load() {
		...
		//echo "Page Load";
			if (CurrentPageID() == "list"){
			 $_SESSION['SYSTO_ID'] = generateSYSTOKey();
		 }
	}
...
// Create page object
if (!isset($system_restock_list)) $system_restock_list = new csystem_restock_list();

// Page init
$system_restock_list->Page_Init();

// Page main
$system_restock_list->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$system_restock_list->Page_Render();
?>
...
var CurrentForm = fsystem_restocklist = new ew_Form("fsystem_restocklist", "list");
fsystem_restocklist.FormKeyCountName = '<?php echo $system_restock_list->FormKeyCountName ?>';

// Form_CustomValidate event
fsystem_restocklist.Form_CustomValidate = 
...
fsystem_restocklist.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;
...
var CurrentSearchForm = fsystem_restocklistsrch = new ew_Form("fsystem_restocklistsrch");
</script>
<script type="text/javascript">
...
<?php if ($system_restock->Export == "") { ?>
<div class="ewToolbar">
<?php if ($system_restock_list->TotalRecs > 0 && $system_restock_list->ExportOptions->Visible()) { ?>
<?php $system_restock_list->ExportOptions->Render("body") ?>
<?php } ?>
<?php if ($system_restock_list->SearchOptions->Visible()) { ?>
<?php $system_restock_list->SearchOptions->Render("body") ?>
<?php } ?>
<?php if ($system_restock_list->FilterOptions->Visible()) { ?>
<?php $system_restock_list->FilterOptions->Render("body") ?>
<?php } ?>
<div class="clearfix"></div>
</div>
<?php } ?>
<?php
	$bSelectLimit = $system_restock_list->UseSelectLimit;
	if ($bSelectLimit) {
		if ($system_restock_list->TotalRecs <= 0)
			$system_restock_list->TotalRecs = $system_restock->ListRecordCount();
	} else {
		if (!$system_restock_list->Recordset && ($system_restock_list->Recordset = $system_restock_list->LoadRecordset()))
			$system_restock_list->TotalRecs = $system_restock_list->Recordset->RecordCount();
	}
	$system_restock_list->StartRec = 1;
	if ($system_restock_list->DisplayRecs <= 0 || ($system_restock->Export <> "" && $system_restock->ExportAll)) // Display all records
		$system_restock_list->DisplayRecs = $system_restock_list->TotalRecs;
	if (!($system_restock->Export <> "" && $system_restock->ExportAll))
		$system_restock_list->SetupStartRec(); // Set up start record position
	if ($bSelectLimit)
		$system_restock_list->Recordset = $system_restock_list->LoadRecordset($system_restock_list->StartRec-1, $system_restock_list->DisplayRecs);

	// Set no record found message
	if ($system_restock->CurrentAction == "" && $system_restock_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$system_restock_list->setWarningMessage(ew_DeniedMsg());
		if ($system_restock_list->SearchWhere == "0=101")
			$system_restock_list->setWarningMessage($Language->Phrase("EnterSearchCriteria"));
		else
			$system_restock_list->setWarningMessage($Language->Phrase("NoRecord"));
	}
$system_restock_list->RenderOtherOptions();
?>
<?php if ($Security->CanSearch()) { ?>
<?php if ($system_restock->Export == "" && $system_restock->CurrentAction == "") { ?>
<form name="fsystem_restocklistsrch" id="fsystem_restocklistsrch" class="form-inline ewForm ewExtSearchForm" action="<?php echo ew_CurrentPage() ?>">
<?php $SearchPanelClass = ($system_restock_list->SearchWhere <> "") ? " in" : " in"; ?>
<div id="fsystem_restocklistsrch_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
<input type="hidden" name="cmd" value="search">
<input type="hidden" name="t" value="system_restock">
	<div class="ewBasicSearch">
<div id="xsr_1" class="ewRow">
	<div class="ewQuickSearch input-group">
	<input type="text" name="<?php echo EW_TABLE_BASIC_SEARCH ?>" id="<?php echo EW_TABLE_BASIC_SEARCH ?>" class="form-control" value="<?php echo ew_HtmlEncode($system_restock_list->BasicSearch->getKeyword()) ?>" placeholder="<?php echo ew_HtmlEncode($Language->Phrase("Search")) ?>">
	<input type="hidden" name="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" id="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" value="<?php echo ew_HtmlEncode($system_restock_list->BasicSearch->getType()) ?>">
	<div class="input-group-btn">
		<button type="button" data-toggle="dropdown" class="btn btn-default"><span id="searchtype"><?php echo $system_restock_list->BasicSearch->getTypeNameShort() ?></span><span class="caret"></span></button>
		<ul class="dropdown-menu pull-right" role="menu">
			<li<?php if ($system_restock_list->BasicSearch->getType() == "") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this)"><?php echo $Language->Phrase("QuickSearchAuto") ?></a></li>
			<li<?php if ($system_restock_list->BasicSearch->getType() == "=") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'=')"><?php echo $Language->Phrase("QuickSearchExact") ?></a></li>
			<li<?php if ($system_restock_list->BasicSearch->getType() == "AND") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'AND')"><?php echo $Language->Phrase("QuickSearchAll") ?></a></li>
			<li<?php if ($system_restock_list->BasicSearch->getType() == "OR") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'OR')"><?php echo $Language->Phrase("QuickSearchAny") ?></a></li>
		</ul>
...
<?php } ?>
<?php } ?>
<?php $system_restock_list->ShowPageHeader(); ?>
<?php
$system_restock_list->ShowMessage();
?>
<?php if ($system_restock_list->TotalRecs > 0 || $system_restock->CurrentAction <> "") { ?>
<div class="box ewBox ewGrid<?php if ($system_restock_list->IsAddOrEdit()) { ?> ewGridAddEdit<?php } ?> system_restock">
<?php if ($system_restock->Export == "") { ?>
<div class="box-header ewGridUpperPanel">
<?php if ($system_restock->CurrentAction <> "gridadd" && $system_restock->CurrentAction <> "gridedit") { ?>
<form name="ewPagerForm" class="form-inline ewForm ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($system_restock_list->Pager)) $system_restock_list->Pager = new cPrevNextPager($system_restock_list->StartRec, $system_restock_list->DisplayRecs, $system_restock_list->TotalRecs, $system_restock_list->AutoHidePager) ?>
<?php if ($system_restock_list->Pager->RecordCount > 0 && $system_restock_list->Pager->Visible) { ?>
<div class="ewPager">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($system_restock_list->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $system_restock_list->PageUrl() ?>start=<?php echo $system_restock_list->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($system_restock_list->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $system_restock_list->PageUrl() ?>start=<?php echo $system_restock_list->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $system_restock_list->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($system_restock_list->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $system_restock_list->PageUrl() ?>start=<?php echo $system_restock_list->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($system_restock_list->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $system_restock_list->PageUrl() ?>start=<?php echo $system_restock_list->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $system_restock_list->Pager->PageCount ?></span>
</div>
<?php } ?>
<?php if ($system_restock_list->Pager->RecordCount > 0) { ?>
<div class="ewPager ewRec">
	<span><?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $system_restock_list->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $system_restock_list->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $system_restock_list->Pager->RecordCount ?></span>
</div>
<?php } ?>
<?php if ($system_restock_list->TotalRecs > 0 && (!$system_restock_list->AutoHidePageSizeSelector || $system_restock_list->Pager->Visible)) { ?>
<div class="ewPager">
<input type="hidden" name="t" value="system_restock">
<select name="<?php echo EW_TABLE_REC_PER_PAGE ?>" class="form-control input-sm ewTooltip" title="<?php echo $Language->Phrase("RecordsPerPage") ?>" onchange="this.form.submit();">
<option value="5"<?php if ($system_restock_list->DisplayRecs == 5) { ?> selected<?php } ?>>5</option>
<option value="10"<?php if ($system_restock_list->DisplayRecs == 10) { ?> selected<?php } ?>>10</option>
<option value="15"<?php if ($system_restock_list->DisplayRecs == 15) { ?> selected<?php } ?>>15</option>
<option value="20"<?php if ($system_restock_list->DisplayRecs == 20) { ?> selected<?php } ?>>20</option>
<option value="50"<?php if ($system_restock_list->DisplayRecs == 50) { ?> selected<?php } ?>>50</option>
<option value="ALL"<?php if ($system_restock->getRecordsPerPage() == -1) { ?> selected<?php } ?>><?php echo $Language->Phrase("AllRecords") ?></option>
</select>
</div>
<?php } ?>
</form>
...
<?php
	foreach ($system_restock_list->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<div class="clearfix"></div>
...
<?php } ?>
<form name="fsystem_restocklist" id="fsystem_restocklist" class="form-inline ewForm ewListForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($system_restock_list->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $system_restock_list->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="system_restock">
<div id="gmp_system_restock" class="<?php if (ew_IsResponsiveLayout()) { ?>table-responsive <?php } ?>ewGridMiddlePanel">
<?php if ($system_restock_list->TotalRecs > 0 || $system_restock->CurrentAction == "gridedit") { ?>
<table id="tbl_system_restocklist" class="table ewTable">
<thead>
	<tr class="ewTableHeader">
<?php

// Header row
$system_restock_list->RowType = EW_ROWTYPE_HEADER;

// Render list options
$system_restock_list->RenderListOptions();
...
<?php if ($system_restock->date_restocked->Visible) { // date_restocked ?>
	...
	<?php } else { ?>
		<th data-name="date_restocked" class="<?php echo $system_restock->date_restocked->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $system_restock->SortUrl($system_restock->date_restocked) ?>',1);"><div id="elh_system_restock_date_restocked" class="system_restock_date_restocked">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $system_restock->date_restocked->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($system_restock->date_restocked->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($system_restock->date_restocked->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($system_restock->reference_id->Visible) { // reference_id ?>
	<?php if ($system_restock->SortUrl($system_restock->reference_id) == "") { ?>
		<th data-name="reference_id" class="<?php echo $system_restock->reference_id->HeaderCellClass() ?>"><div id="elh_system_restock_reference_id" class="system_restock_reference_id"><div class="ewTableHeaderCaption"><?php echo $system_restock->reference_id->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="reference_id" class="<?php echo $system_restock->reference_id->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $system_restock->SortUrl($system_restock->reference_id) ?>',1);"><div id="elh_system_restock_reference_id" class="system_restock_reference_id">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $system_restock->reference_id->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($system_restock->reference_id->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($system_restock->reference_id->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($system_restock->material_name->Visible) { // material_name ?>
	<?php if ($system_restock->SortUrl($system_restock->material_name) == "") { ?>
		<th data-name="material_name" class="<?php echo $system_restock->material_name->HeaderCellClass() ?>"><div id="elh_system_restock_material_name" class="system_restock_material_name"><div class="ewTableHeaderCaption"><?php echo $system_restock->material_name->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="material_name" class="<?php echo $system_restock->material_name->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $system_restock->SortUrl($system_restock->material_name) ?>',1);"><div id="elh_system_restock_material_name" class="system_restock_material_name">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $system_restock->material_name->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($system_restock->material_name->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($system_restock->material_name->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($system_restock->quantity->Visible) { // quantity ?>
	<?php if ($system_restock->SortUrl($system_restock->quantity) == "") { ?>
		<th data-name="quantity" class="<?php echo $system_restock->quantity->HeaderCellClass() ?>"><div id="elh_system_restock_quantity" class="system_restock_quantity"><div class="ewTableHeaderCaption"><?php echo $system_restock->quantity->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="quantity" class="<?php echo $system_restock->quantity->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $system_restock->SortUrl($system_restock->quantity) ?>',1);"><div id="elh_system_restock_quantity" class="system_restock_quantity">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $system_restock->quantity->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($system_restock->quantity->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($system_restock->quantity->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($system_restock->restocked_by->Visible) { // restocked_by ?>
	<?php if ($system_restock->SortUrl($system_restock->restocked_by) == "") { ?>
		<th data-name="restocked_by" class="<?php echo $system_restock->restocked_by->HeaderCellClass() ?>"><div id="elh_system_restock_restocked_by" class="system_restock_restocked_by"><div class="ewTableHeaderCaption"><?php echo $system_restock->restocked_by->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="restocked_by" class="<?php echo $system_restock->restocked_by->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $system_restock->SortUrl($system_restock->restocked_by) ?>',1);"><div id="elh_system_restock_restocked_by" class="system_restock_restocked_by">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $system_restock->restocked_by->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($system_restock->restocked_by->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($system_restock->restocked_by->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
...
$system_restock_list->ListOptions->Render("header", "right");
?>
	</tr>
</thead>
<tbody>
<?php
if ($system_restock->ExportAll && $system_restock->Export <> "") {
	$system_restock_list->StopRec = $system_restock_list->TotalRecs;
} else {

	// Set the last record to display
	if ($system_restock_list->TotalRecs > $system_restock_list->StartRec + $system_restock_list->DisplayRecs - 1)
		$system_restock_list->StopRec = $system_restock_list->StartRec + $system_restock_list->DisplayRecs - 1;
	else
		$system_restock_list->StopRec = $system_restock_list->TotalRecs;
}
$system_restock_list->RecCnt = $system_restock_list->StartRec - 1;
if ($system_restock_list->Recordset && !$system_restock_list->Recordset->EOF) {
	$system_restock_list->Recordset->MoveFirst();
	$bSelectLimit = $system_restock_list->UseSelectLimit;
	if (!$bSelectLimit && $system_restock_list->StartRec > 1)
		$system_restock_list->Recordset->Move($system_restock_list->StartRec - 1);
} elseif (!$system_restock->AllowAddDeleteRow && $system_restock_list->StopRec == 0) {
	$system_restock_list->StopRec = $system_restock->GridAddRowCount;
}

// Initialize aggregate
$system_restock->RowType = EW_ROWTYPE_AGGREGATEINIT;
$system_restock->ResetAttrs();
$system_restock_list->RenderRow();
while ($system_restock_list->RecCnt < $system_restock_list->StopRec) {
	$system_restock_list->RecCnt++;
	if (intval($system_restock_list->RecCnt) >= intval($system_restock_list->StartRec)) {
		$system_restock_list->RowCnt++;

		// Set up key count
		$system_restock_list->KeyCount = $system_restock_list->RowIndex;

		// Init row class and style
		$system_restock->ResetAttrs();
		$system_restock->CssClass = "";
		if ($system_restock->CurrentAction == "gridadd") {
		} else {
			$system_restock_list->LoadRowValues($system_restock_list->Recordset); // Load row values
		}
		$system_restock->RowType = EW_ROWTYPE_VIEW; // Render view

		// Set up row id / data-rowindex
		$system_restock->RowAttrs = array_merge($system_restock->RowAttrs, array('data-rowindex'=>$system_restock_list->RowCnt, 'id'=>'r' . $system_restock_list->RowCnt . '_system_restock', 'data-rowtype'=>$system_restock->RowType));
...
<span id="el<?php echo $system_restock_list->RowCnt ?>_system_restock_reference_id" class="system_restock_reference_id">
...
<span id="el<?php echo $system_restock_list->RowCnt ?>_system_restock_material_name" class="system_restock_material_name">
...
<span id="el<?php echo $system_restock_list->RowCnt ?>_system_restock_quantity" class="system_restock_quantity">
...
<span id="el<?php echo $system_restock_list->RowCnt ?>_system_restock_restocked_by" class="system_restock_restocked_by">
...
$system_restock_list->ListOptions->Render("body", "right", $system_restock_list->RowCnt);
?>
	</tr>
<?php
	}
	if ($system_restock->CurrentAction <> "gridadd")
		$system_restock_list->Recordset->MoveNext();
}
?>
</tbody>
</table>
<?php } ?>
<?php if ($system_restock->CurrentAction == "") { ?>
...
// Close recordset
if ($system_restock_list->Recordset)
	$system_restock_list->Recordset->Close();
?>
</div>
<?php } ?>
<?php if ($system_restock_list->TotalRecs == 0 && $system_restock->CurrentAction == "") { // Show other options ?>
<div class="ewListOtherOptions">
<?php
	foreach ($system_restock_list->OtherOptions as &$option) {
		$option->ButtonClass = "";
...
<?php if ($system_restock->Export == "") { ?>
<script type="text/javascript">
fsystem_restocklistsrch.FilterList = <?php echo $system_restock_list->GetFilterList() ?>;
fsystem_restocklistsrch.Init();
fsystem_restocklist.Init();
</script>
<?php } ?>
<?php
$system_restock_list->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($system_restock->Export == "") { ?>
...
<?php
$system_restock_list->Page_Terminate();
?>
