...
$issuance_store_edit = NULL; // Initialize page object first

class cissuance_store_edit extends cissuance_store {
	...
	// Page object name
	var $PageObjName = 'issuance_store_edit';
	...
	function Page_Init() {
		...
		if (!$Security->CanEdit()) {
			...
			if ($Security->CanList())
				$this->Page_Terminate(ew_GetUrl("issuance_storelist.php"));
			else
		...
		if ($url <> "") {
			...
			if ($this->IsModal) { // Show as modal
				...
				if ($pageName != $this->GetListUrl()) { // Not List page
					...
					if ($pageName == "issuance_storeview.php")
						$row["view"] = "1";
	...
	function Page_Main() {
		...
		if ($this->TotalRecs <= 0) { // No record found
			...
			$this->Page_Terminate("issuance_storelist.php"); // Return to list page
		...
		switch ($this->CurrentAction) {
			...
			Case "U": // Update
				...
				if (ew_GetPageName($sReturnUrl) == "issuance_storelist.php")
	...
	function SetupBreadcrumb() {
		...
		$Breadcrumb->Add("list", $this->TableVar, $this->AddMasterUrl("issuance_storelist.php"), "", $this->TableVar, TRUE);
		$PageId = "edit";
...
// Create page object
if (!isset($issuance_store_edit)) $issuance_store_edit = new cissuance_store_edit();

// Page init
$issuance_store_edit->Page_Init();

// Page main
$issuance_store_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$issuance_store_edit->Page_Render();
?>
...
var CurrentForm = fissuance_storeedit = new ew_Form("fissuance_storeedit", "edit");

// Validate form
fissuance_storeedit.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $issuance_store->date->FldCaption(), $issuance_store->date->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_date");
			if (elm && !ew_CheckDateDef(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($issuance_store->date->FldErrMsg()) ?>");
			elm = this.GetElements("x" + infix + "_reference_id");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $issuance_store->reference_id->FldCaption(), $issuance_store->reference_id->ReqErrMsg)) ?>");
			...
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $issuance_store->quantity_in->FldCaption(), $issuance_store->quantity_in->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_quantity_type");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $issuance_store->quantity_type->FldCaption(), $issuance_store->quantity_type->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_quantity_out");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $issuance_store->quantity_out->FldCaption(), $issuance_store->quantity_out->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_total_quantity");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $issuance_store->total_quantity->FldCaption(), $issuance_store->total_quantity->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_treated_by");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $issuance_store->treated_by->FldCaption(), $issuance_store->treated_by->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_issued_action");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $issuance_store->issued_action->FldCaption(), $issuance_store->issued_action->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_issued_comment");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $issuance_store->issued_comment->FldCaption(), $issuance_store->issued_comment->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_approver_date");
			if (elm && !ew_CheckDateDef(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($issuance_store->approver_date->FldErrMsg()) ?>");
			elm = this.GetElements("x" + infix + "_approver_action");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $issuance_store->approver_action->FldCaption(), $issuance_store->approver_action->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_approved_comment");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $issuance_store->approved_comment->FldCaption(), $issuance_store->approved_comment->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_verified_date");
			if (elm && !ew_CheckDateDef(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($issuance_store->verified_date->FldErrMsg()) ?>");
			elm = this.GetElements("x" + infix + "_verified_action");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $issuance_store->verified_action->FldCaption(), $issuance_store->verified_action->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_verified_comment");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $issuance_store->verified_comment->FldCaption(), $issuance_store->verified_comment->ReqErrMsg)) ?>");
...
fissuance_storeedit.Form_CustomValidate = 
...
fissuance_storeedit.Lists["x_material_name"].Data = "<?php echo $issuance_store_edit->material_name->LookupFilterQuery(FALSE, "edit") ?>";
fissuance_storeedit.Lists["x_treated_by"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_firstname","x_lastname","x_staffno",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"users"};
fissuance_storeedit.Lists["x_treated_by"].Data = "<?php echo $issuance_store_edit->treated_by->LookupFilterQuery(FALSE, "edit") ?>";
fissuance_storeedit.AutoSuggests["x_treated_by"] = <?php echo json_encode(array("data" => "ajax=autosuggest&" . $issuance_store_edit->treated_by->LookupFilterQuery(TRUE, "edit"))) ?>;
fissuance_storeedit.Lists["x_issued_action"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
...
fissuance_storeedit.Lists["x_approved_by"].Data = "<?php echo $issuance_store_edit->approved_by->LookupFilterQuery(FALSE, "edit") ?>";
fissuance_storeedit.AutoSuggests["x_approved_by"] = <?php echo json_encode(array("data" => "ajax=autosuggest&" . $issuance_store_edit->approved_by->LookupFilterQuery(TRUE, "edit"))) ?>;
fissuance_storeedit.Lists["x_verified_action"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fissuance_storeedit.Lists["x_verified_action"].Options = <?php echo json_encode($issuance_store_edit->verified_action->Options()) ?>;
fissuance_storeedit.Lists["x_verified_by"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_firstname","x_lastname","x_staffno",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"users"};
fissuance_storeedit.Lists["x_verified_by"].Data = "<?php echo $issuance_store_edit->verified_by->LookupFilterQuery(FALSE, "edit") ?>";
fissuance_storeedit.AutoSuggests["x_verified_by"] = <?php echo json_encode(array("data" => "ajax=autosuggest&" . $issuance_store_edit->verified_by->LookupFilterQuery(TRUE, "edit"))) ?>;
fissuance_storeedit.Lists["x_statuss"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_description","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"statuss"};
fissuance_storeedit.Lists["x_statuss"].Data = "<?php echo $issuance_store_edit->statuss->LookupFilterQuery(FALSE, "edit") ?>";
...
<?php $issuance_store_edit->ShowPageHeader(); ?>
<?php
$issuance_store_edit->ShowMessage();
?>
<?php if (!$issuance_store_edit->IsModal) { ?>
<?php if ($issuance_store->CurrentAction <> "F") { // Confirm page ?>
<form name="ewPagerForm" class="form-horizontal ewForm ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($issuance_store_edit->Pager)) $issuance_store_edit->Pager = new cPrevNextPager($issuance_store_edit->StartRec, $issuance_store_edit->DisplayRecs, $issuance_store_edit->TotalRecs, $issuance_store_edit->AutoHidePager) ?>
<?php if ($issuance_store_edit->Pager->RecordCount > 0 && $issuance_store_edit->Pager->Visible) { ?>
<div class="ewPager">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($issuance_store_edit->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $issuance_store_edit->PageUrl() ?>start=<?php echo $issuance_store_edit->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($issuance_store_edit->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $issuance_store_edit->PageUrl() ?>start=<?php echo $issuance_store_edit->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $issuance_store_edit->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($issuance_store_edit->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $issuance_store_edit->PageUrl() ?>start=<?php echo $issuance_store_edit->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($issuance_store_edit->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $issuance_store_edit->PageUrl() ?>start=<?php echo $issuance_store_edit->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $issuance_store_edit->Pager->PageCount ?></span>
</div>
<?php } ?>
...
<?php } ?>
<?php } ?>
<form name="fissuance_storeedit" id="fissuance_storeedit" class="<?php echo $issuance_store_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($issuance_store_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $issuance_store_edit->Token ?>">
<?php } ?>
...
<?php } ?>
<input type="hidden" name="modal" value="<?php echo intval($issuance_store_edit->IsModal) ?>">
<div class="ewEditDiv"><!-- page* -->
<?php if ($issuance_store->id->Visible) { // id ?>
	<div id="r_id" class="form-group">
		<label id="elh_issuance_store_id" class="<?php echo $issuance_store_edit->LeftColumnClass ?>"><?php echo $issuance_store->id->FldCaption() ?></label>
		<div class="<?php echo $issuance_store_edit->RightColumnClass ?>"><div<?php echo $issuance_store->id->CellAttributes() ?>>
<?php if ($issuance_store->CurrentAction <> "F") { ?>
<span id="el_issuance_store_id">
<span<?php echo $issuance_store->id->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $issuance_store->id->EditValue ?></p></span>
</span>
<input type="hidden" data-table="issuance_store" data-field="x_id" data-page="1" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($issuance_store->id->CurrentValue) ?>">
<?php } else { ?>
<span id="el_issuance_store_id">
<span<?php echo $issuance_store->id->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $issuance_store->id->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="issuance_store" data-field="x_id" data-page="1" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($issuance_store->id->FormValue) ?>">
<?php } ?>
<?php echo $issuance_store->id->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($issuance_store->date->Visible) { // date ?>
	<div id="r_date" class="form-group">
		<label id="elh_issuance_store_date" for="x_date" class="<?php echo $issuance_store_edit->LeftColumnClass ?>"><?php echo $issuance_store->date->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $issuance_store_edit->RightColumnClass ?>"><div<?php echo $issuance_store->date->CellAttributes() ?>>
<?php if ($issuance_store->CurrentAction <> "F") { ?>
<span id="el_issuance_store_date">
<input type="text" data-table="issuance_store" data-field="x_date" data-page="1" name="x_date" id="x_date" size="30" maxlength="50" placeholder="<?php echo ew_HtmlEncode($issuance_store->date->getPlaceHolder()) ?>" value="<?php echo $issuance_store->date->EditValue ?>"<?php echo $issuance_store->date->EditAttributes() ?>>
</span>
<?php } else { ?>
<span id="el_issuance_store_date">
<span<?php echo $issuance_store->date->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $issuance_store->date->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="issuance_store" data-field="x_date" data-page="1" name="x_date" id="x_date" value="<?php echo ew_HtmlEncode($issuance_store->date->FormValue) ?>">
<?php } ?>
<?php echo $issuance_store->date->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($issuance_store->reference_id->Visible) { // reference_id ?>
	<div id="r_reference_id" class="form-group">
		<label id="elh_issuance_store_reference_id" for="x_reference_id" class="<?php echo $issuance_store_edit->LeftColumnClass ?>"><?php echo $issuance_store->reference_id->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $issuance_store_edit->RightColumnClass ?>"><div<?php echo $issuance_store->reference_id->CellAttributes() ?>>
<?php if ($issuance_store->CurrentAction <> "F") { ?>
<span id="el_issuance_store_reference_id">
<input type="text" data-table="issuance_store" data-field="x_reference_id" data-page="1" name="x_reference_id" id="x_reference_id" size="30" maxlength="50" placeholder="<?php echo ew_HtmlEncode($issuance_store->reference_id->getPlaceHolder()) ?>" value="<?php echo $issuance_store->reference_id->EditValue ?>"<?php echo $issuance_store->reference_id->EditAttributes() ?>>
</span>
<?php } else { ?>
<span id="el_issuance_store_reference_id">
<span<?php echo $issuance_store->reference_id->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $issuance_store->reference_id->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="issuance_store" data-field="x_reference_id" data-page="1" name="x_reference_id" id="x_reference_id" value="<?php echo ew_HtmlEncode($issuance_store->reference_id->FormValue) ?>">
<?php } ?>
<?php echo $issuance_store->reference_id->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($issuance_store->staff_id->Visible) { // staff_id ?>
	<div id="r_staff_id" class="form-group">
		<label id="elh_issuance_store_staff_id" class="<?php echo $issuance_store_edit->LeftColumnClass ?>"><?php echo $issuance_store->staff_id->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $issuance_store_edit->RightColumnClass ?>"><div<?php echo $issuance_store->staff_id->CellAttributes() ?>>
<?php if ($issuance_store->CurrentAction <> "F") { ?>
<span id="el_issuance_store_staff_id">
<?php
$wrkonchange = trim(" " . @$issuance_store->staff_id->EditAttrs["onchange"]);
if ($wrkonchange <> "") $wrkonchange = " onchange=\"" . ew_JsEncode2($wrkonchange) . "\"";
$issuance_store->staff_id->EditAttrs["onchange"] = "";
?>
<span id="as_x_staff_id" style="white-space: nowrap; z-index: 8960">
	<input type="text" name="sv_x_staff_id" id="sv_x_staff_id" value="<?php echo $issuance_store->staff_id->EditValue ?>" size="30" placeholder="<?php echo ew_HtmlEncode($issuance_store->staff_id->getPlaceHolder()) ?>" data-placeholder="<?php echo ew_HtmlEncode($issuance_store->staff_id->getPlaceHolder()) ?>"<?php echo $issuance_store->staff_id->EditAttributes() ?>>
</span>
<input type="hidden" data-table="issuance_store" data-field="x_staff_id" data-page="1" data-value-separator="<?php echo $issuance_store->staff_id->DisplayValueSeparatorAttribute() ?>" name="x_staff_id" id="x_staff_id" value="<?php echo ew_HtmlEncode($issuance_store->staff_id->CurrentValue) ?>"<?php echo $wrkonchange ?>>
<script type="text/javascript">
fissuance_storeedit.CreateAutoSuggest({"id":"x_staff_id","forceSelect":false});
</script>
</span>
<?php } else { ?>
<span id="el_issuance_store_staff_id">
<span<?php echo $issuance_store->staff_id->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $issuance_store->staff_id->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="issuance_store" data-field="x_staff_id" data-page="1" name="x_staff_id" id="x_staff_id" value="<?php echo ew_HtmlEncode($issuance_store->staff_id->FormValue) ?>">
<?php } ?>
<?php echo $issuance_store->staff_id->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($issuance_store->material_name->Visible) { // material_name ?>
	<div id="r_material_name" class="form-group">
		<label id="elh_issuance_store_material_name" for="x_material_name" class="<?php echo $issuance_store_edit->LeftColumnClass ?>"><?php echo $issuance_store->material_name->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $issuance_store_edit->RightColumnClass ?>"><div<?php echo $issuance_store->material_name->CellAttributes() ?>>
<?php if ($issuance_store->CurrentAction <> "F") { ?>
<span id="el_issuance_store_material_name">
<span class="ewLookupList">
	<span onclick="jQuery(this).parent().next(":not([disabled])").click();" tabindex="-1" class="form-control ewLookupText" id="lu_x_material_name"><?php echo (strval($issuance_store->material_name->ViewValue) == "" ? $Language->Phrase("PleaseSelect") : $issuance_store->material_name->ViewValue); ?></span>
</span>
<button type="button" title="<?php echo ew_HtmlEncode(str_replace("%s", ew_RemoveHtml($issuance_store->material_name->FldCaption()), $Language->Phrase("LookupLink", TRUE))) ?>" onclick="ew_ModalLookupShow({lnk:this,el:'x_material_name',m:0,n:10});" class="ewLookupBtn btn btn-default btn-sm"<?php echo (($issuance_store->material_name->ReadOnly || $issuance_store->material_name->Disabled) ? " disabled" : "")?>><span class="glyphicon glyphicon-search ewIcon"></span></button>
<input type="hidden" data-table="issuance_store" data-field="x_material_name" data-page="1" data-multiple="0" data-lookup="1" data-value-separator="<?php echo $issuance_store->material_name->DisplayValueSeparatorAttribute() ?>" name="x_material_name" id="x_material_name" value="<?php echo $issuance_store->material_name->CurrentValue ?>"<?php echo $issuance_store->material_name->EditAttributes() ?>>
</span>
<?php } else { ?>
<span id="el_issuance_store_material_name">
<span<?php echo $issuance_store->material_name->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $issuance_store->material_name->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="issuance_store" data-field="x_material_name" data-page="1" name="x_material_name" id="x_material_name" value="<?php echo ew_HtmlEncode($issuance_store->material_name->FormValue) ?>">
<?php } ?>
<?php echo $issuance_store->material_name->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($issuance_store->quantity_in->Visible) { // quantity_in ?>
	<div id="r_quantity_in" class="form-group">
		<label id="elh_issuance_store_quantity_in" for="x_quantity_in" class="<?php echo $issuance_store_edit->LeftColumnClass ?>"><?php echo $issuance_store->quantity_in->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $issuance_store_edit->RightColumnClass ?>"><div<?php echo $issuance_store->quantity_in->CellAttributes() ?>>
<?php if ($issuance_store->CurrentAction <> "F") { ?>
<span id="el_issuance_store_quantity_in">
<input type="text" data-table="issuance_store" data-field="x_quantity_in" data-page="1" name="x_quantity_in" id="x_quantity_in" size="30" maxlength="50" placeholder="<?php echo ew_HtmlEncode($issuance_store->quantity_in->getPlaceHolder()) ?>" value="<?php echo $issuance_store->quantity_in->EditValue ?>"<?php echo $issuance_store->quantity_in->EditAttributes() ?>>
</span>
<?php } else { ?>
<span id="el_issuance_store_quantity_in">
<span<?php echo $issuance_store->quantity_in->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $issuance_store->quantity_in->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="issuance_store" data-field="x_quantity_in" data-page="1" name="x_quantity_in" id="x_quantity_in" value="<?php echo ew_HtmlEncode($issuance_store->quantity_in->FormValue) ?>">
<?php } ?>
<?php echo $issuance_store->quantity_in->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($issuance_store->quantity_type->Visible) { // quantity_type ?>
	<div id="r_quantity_type" class="form-group">
		<label id="elh_issuance_store_quantity_type" for="x_quantity_type" class="<?php echo $issuance_store_edit->LeftColumnClass ?>"><?php echo $issuance_store->quantity_type->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $issuance_store_edit->RightColumnClass ?>"><div<?php echo $issuance_store->quantity_type->CellAttributes() ?>>
<?php if ($issuance_store->CurrentAction <> "F") { ?>
<span id="el_issuance_store_quantity_type">
<input type="text" data-table="issuance_store" data-field="x_quantity_type" data-page="1" name="x_quantity_type" id="x_quantity_type" size="30" maxlength="50" placeholder="<?php echo ew_HtmlEncode($issuance_store->quantity_type->getPlaceHolder()) ?>" value="<?php echo $issuance_store->quantity_type->EditValue ?>"<?php echo $issuance_store->quantity_type->EditAttributes() ?>>
</span>
<?php } else { ?>
<span id="el_issuance_store_quantity_type">
<span<?php echo $issuance_store->quantity_type->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $issuance_store->quantity_type->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="issuance_store" data-field="x_quantity_type" data-page="1" name="x_quantity_type" id="x_quantity_type" value="<?php echo ew_HtmlEncode($issuance_store->quantity_type->FormValue) ?>">
<?php } ?>
<?php echo $issuance_store->quantity_type->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($issuance_store->quantity_out->Visible) { // quantity_out ?>
	<div id="r_quantity_out" class="form-group">
		<label id="elh_issuance_store_quantity_out" for="x_quantity_out" class="<?php echo $issuance_store_edit->LeftColumnClass ?>"><?php echo $issuance_store->quantity_out->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $issuance_store_edit->RightColumnClass ?>"><div<?php echo $issuance_store->quantity_out->CellAttributes() ?>>
<?php if ($issuance_store->CurrentAction <> "F") { ?>
<span id="el_issuance_store_quantity_out">
<input type="text" data-table="issuance_store" data-field="x_quantity_out" data-page="1" name="x_quantity_out" id="x_quantity_out" size="30" maxlength="50" placeholder="<?php echo ew_HtmlEncode($issuance_store->quantity_out->getPlaceHolder()) ?>" value="<?php echo $issuance_store->quantity_out->EditValue ?>"<?php echo $issuance_store->quantity_out->EditAttributes() ?>>
</span>
<?php } else { ?>
<span id="el_issuance_store_quantity_out">
<span<?php echo $issuance_store->quantity_out->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $issuance_store->quantity_out->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="issuance_store" data-field="x_quantity_out" data-page="1" name="x_quantity_out" id="x_quantity_out" value="<?php echo ew_HtmlEncode($issuance_store->quantity_out->FormValue) ?>">
<?php } ?>
<?php echo $issuance_store->quantity_out->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($issuance_store->total_quantity->Visible) { // total_quantity ?>
	<div id="r_total_quantity" class="form-group">
		<label id="elh_issuance_store_total_quantity" for="x_total_quantity" class="<?php echo $issuance_store_edit->LeftColumnClass ?>"><?php echo $issuance_store->total_quantity->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $issuance_store_edit->RightColumnClass ?>"><div<?php echo $issuance_store->total_quantity->CellAttributes() ?>>
<?php if ($issuance_store->CurrentAction <> "F") { ?>
<span id="el_issuance_store_total_quantity">
<input type="text" data-table="issuance_store" data-field="x_total_quantity" data-page="1" name="x_total_quantity" id="x_total_quantity" size="30" maxlength="50" placeholder="<?php echo ew_HtmlEncode($issuance_store->total_quantity->getPlaceHolder()) ?>" value="<?php echo $issuance_store->total_quantity->EditValue ?>"<?php echo $issuance_store->total_quantity->EditAttributes() ?>>
</span>
<?php } else { ?>
<span id="el_issuance_store_total_quantity">
<span<?php echo $issuance_store->total_quantity->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $issuance_store->total_quantity->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="issuance_store" data-field="x_total_quantity" data-page="1" name="x_total_quantity" id="x_total_quantity" value="<?php echo ew_HtmlEncode($issuance_store->total_quantity->FormValue) ?>">
<?php } ?>
<?php echo $issuance_store->total_quantity->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($issuance_store->treated_by->Visible) { // treated_by ?>
	<div id="r_treated_by" class="form-group">
		<label id="elh_issuance_store_treated_by" class="<?php echo $issuance_store_edit->LeftColumnClass ?>"><?php echo $issuance_store->treated_by->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $issuance_store_edit->RightColumnClass ?>"><div<?php echo $issuance_store->treated_by->CellAttributes() ?>>
<?php if ($issuance_store->CurrentAction <> "F") { ?>
<span id="el_issuance_store_treated_by">
<?php
$wrkonchange = trim(" " . @$issuance_store->treated_by->EditAttrs["onchange"]);
if ($wrkonchange <> "") $wrkonchange = " onchange=\"" . ew_JsEncode2($wrkonchange) . "\"";
$issuance_store->treated_by->EditAttrs["onchange"] = "";
?>
<span id="as_x_treated_by" style="white-space: nowrap; z-index: 8900">
	<input type="text" name="sv_x_treated_by" id="sv_x_treated_by" value="<?php echo $issuance_store->treated_by->EditValue ?>" size="30" placeholder="<?php echo ew_HtmlEncode($issuance_store->treated_by->getPlaceHolder()) ?>" data-placeholder="<?php echo ew_HtmlEncode($issuance_store->treated_by->getPlaceHolder()) ?>"<?php echo $issuance_store->treated_by->EditAttributes() ?>>
</span>
<input type="hidden" data-table="issuance_store" data-field="x_treated_by" data-page="1" data-value-separator="<?php echo $issuance_store->treated_by->DisplayValueSeparatorAttribute() ?>" name="x_treated_by" id="x_treated_by" value="<?php echo ew_HtmlEncode($issuance_store->treated_by->CurrentValue) ?>"<?php echo $wrkonchange ?>>
<script type="text/javascript">
fissuance_storeedit.CreateAutoSuggest({"id":"x_treated_by","forceSelect":false});
</script>
</span>
<?php } else { ?>
<span id="el_issuance_store_treated_by">
<span<?php echo $issuance_store->treated_by->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $issuance_store->treated_by->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="issuance_store" data-field="x_treated_by" data-page="1" name="x_treated_by" id="x_treated_by" value="<?php echo ew_HtmlEncode($issuance_store->treated_by->FormValue) ?>">
<?php } ?>
<?php echo $issuance_store->treated_by->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($issuance_store->issued_action->Visible) { // issued_action ?>
	<div id="r_issued_action" class="form-group">
		<label id="elh_issuance_store_issued_action" class="<?php echo $issuance_store_edit->LeftColumnClass ?>"><?php echo $issuance_store->issued_action->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $issuance_store_edit->RightColumnClass ?>"><div<?php echo $issuance_store->issued_action->CellAttributes() ?>>
<?php if ($issuance_store->CurrentAction <> "F") { ?>
<span id="el_issuance_store_issued_action">
<div id="tp_x_issued_action" class="ewTemplate"><input type="radio" data-table="issuance_store" data-field="x_issued_action" data-page="1" data-value-separator="<?php echo $issuance_store->issued_action->DisplayValueSeparatorAttribute() ?>" name="x_issued_action" id="x_issued_action" value="{value}"<?php echo $issuance_store->issued_action->EditAttributes() ?>></div>
<div id="dsl_x_issued_action" data-repeatcolumn="5" class="ewItemList" style="display: none;"><div>
<?php echo $issuance_store->issued_action->RadioButtonListHtml(FALSE, "x_issued_action", 1) ?>
</div></div>
</span>
<?php } else { ?>
<span id="el_issuance_store_issued_action">
<span<?php echo $issuance_store->issued_action->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $issuance_store->issued_action->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="issuance_store" data-field="x_issued_action" data-page="1" name="x_issued_action" id="x_issued_action" value="<?php echo ew_HtmlEncode($issuance_store->issued_action->FormValue) ?>">
<?php } ?>
<?php echo $issuance_store->issued_action->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($issuance_store->issued_comment->Visible) { // issued_comment ?>
	<div id="r_issued_comment" class="form-group">
		<label id="elh_issuance_store_issued_comment" for="x_issued_comment" class="<?php echo $issuance_store_edit->LeftColumnClass ?>"><?php echo $issuance_store->issued_comment->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $issuance_store_edit->RightColumnClass ?>"><div<?php echo $issuance_store->issued_comment->CellAttributes() ?>>
<?php if ($issuance_store->CurrentAction <> "F") { ?>
<span id="el_issuance_store_issued_comment">
<textarea data-table="issuance_store" data-field="x_issued_comment" data-page="1" name="x_issued_comment" id="x_issued_comment" cols="35" rows="4" placeholder="<?php echo ew_HtmlEncode($issuance_store->issued_comment->getPlaceHolder()) ?>"<?php echo $issuance_store->issued_comment->EditAttributes() ?>><?php echo $issuance_store->issued_comment->EditValue ?></textarea>
</span>
<?php } else { ?>
<span id="el_issuance_store_issued_comment">
<span<?php echo $issuance_store->issued_comment->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $issuance_store->issued_comment->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="issuance_store" data-field="x_issued_comment" data-page="1" name="x_issued_comment" id="x_issued_comment" value="<?php echo ew_HtmlEncode($issuance_store->issued_comment->FormValue) ?>">
<?php } ?>
<?php echo $issuance_store->issued_comment->CustomMsg ?></div></div>
	</div>
<?php } ?>
...
<p class="form-control-static"><?php echo $issuance_store->issued_by->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="issuance_store" data-field="x_issued_by" data-page="1" name="x_issued_by" id="x_issued_by" value="<?php echo ew_HtmlEncode($issuance_store->issued_by->FormValue) ?>">
<?php } ?>
<?php echo $issuance_store->issued_by->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($issuance_store->approver_date->Visible) { // approver_date ?>
	<div id="r_approver_date" class="form-group">
		<label id="elh_issuance_store_approver_date" for="x_approver_date" class="<?php echo $issuance_store_edit->LeftColumnClass ?>"><?php echo $issuance_store->approver_date->FldCaption() ?></label>
		<div class="<?php echo $issuance_store_edit->RightColumnClass ?>"><div<?php echo $issuance_store->approver_date->CellAttributes() ?>>
<?php if ($issuance_store->CurrentAction <> "F") { ?>
<span id="el_issuance_store_approver_date">
<input type="text" data-table="issuance_store" data-field="x_approver_date" data-page="1" name="x_approver_date" id="x_approver_date" size="30" placeholder="<?php echo ew_HtmlEncode($issuance_store->approver_date->getPlaceHolder()) ?>" value="<?php echo $issuance_store->approver_date->EditValue ?>"<?php echo $issuance_store->approver_date->EditAttributes() ?>>
</span>
<?php } else { ?>
<span id="el_issuance_store_approver_date">
<span<?php echo $issuance_store->approver_date->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $issuance_store->approver_date->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="issuance_store" data-field="x_approver_date" data-page="1" name="x_approver_date" id="x_approver_date" value="<?php echo ew_HtmlEncode($issuance_store->approver_date->FormValue) ?>">
<?php } ?>
<?php echo $issuance_store->approver_date->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($issuance_store->approver_action->Visible) { // approver_action ?>
	<div id="r_approver_action" class="form-group">
		<label id="elh_issuance_store_approver_action" class="<?php echo $issuance_store_edit->LeftColumnClass ?>"><?php echo $issuance_store->approver_action->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $issuance_store_edit->RightColumnClass ?>"><div<?php echo $issuance_store->approver_action->CellAttributes() ?>>
<?php if ($issuance_store->CurrentAction <> "F") { ?>
<span id="el_issuance_store_approver_action">
<div id="tp_x_approver_action" class="ewTemplate"><input type="radio" data-table="issuance_store" data-field="x_approver_action" data-page="1" data-value-separator="<?php echo $issuance_store->approver_action->DisplayValueSeparatorAttribute() ?>" name="x_approver_action" id="x_approver_action" value="{value}"<?php echo $issuance_store->approver_action->EditAttributes() ?>></div>
<div id="dsl_x_approver_action" data-repeatcolumn="5" class="ewItemList" style="display: none;"><div>
<?php echo $issuance_store->approver_action->RadioButtonListHtml(FALSE, "x_approver_action", 1) ?>
</div></div>
</span>
<?php } else { ?>
<span id="el_issuance_store_approver_action">
<span<?php echo $issuance_store->approver_action->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $issuance_store->approver_action->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="issuance_store" data-field="x_approver_action" data-page="1" name="x_approver_action" id="x_approver_action" value="<?php echo ew_HtmlEncode($issuance_store->approver_action->FormValue) ?>">
<?php } ?>
<?php echo $issuance_store->approver_action->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($issuance_store->approved_comment->Visible) { // approved_comment ?>
	<div id="r_approved_comment" class="form-group">
		<label id="elh_issuance_store_approved_comment" for="x_approved_comment" class="<?php echo $issuance_store_edit->LeftColumnClass ?>"><?php echo $issuance_store->approved_comment->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $issuance_store_edit->RightColumnClass ?>"><div<?php echo $issuance_store->approved_comment->CellAttributes() ?>>
<?php if ($issuance_store->CurrentAction <> "F") { ?>
<span id="el_issuance_store_approved_comment">
<textarea data-table="issuance_store" data-field="x_approved_comment" data-page="1" name="x_approved_comment" id="x_approved_comment" cols="35" rows="4" placeholder="<?php echo ew_HtmlEncode($issuance_store->approved_comment->getPlaceHolder()) ?>"<?php echo $issuance_store->approved_comment->EditAttributes() ?>><?php echo $issuance_store->approved_comment->EditValue ?></textarea>
</span>
<?php } else { ?>
<span id="el_issuance_store_approved_comment">
<span<?php echo $issuance_store->approved_comment->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $issuance_store->approved_comment->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="issuance_store" data-field="x_approved_comment" data-page="1" name="x_approved_comment" id="x_approved_comment" value="<?php echo ew_HtmlEncode($issuance_store->approved_comment->FormValue) ?>">
<?php } ?>
<?php echo $issuance_store->approved_comment->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($issuance_store->approved_by->Visible) { // approved_by ?>
	<div id="r_approved_by" class="form-group">
		<label id="elh_issuance_store_approved_by" class="<?php echo $issuance_store_edit->LeftColumnClass ?>"><?php echo $issuance_store->approved_by->FldCaption() ?></label>
		<div class="<?php echo $issuance_store_edit->RightColumnClass ?>"><div<?php echo $issuance_store->approved_by->CellAttributes() ?>>
<?php if ($issuance_store->CurrentAction <> "F") { ?>
<span id="el_issuance_store_approved_by">
<?php
$wrkonchange = trim(" " . @$issuance_store->approved_by->EditAttrs["onchange"]);
if ($wrkonchange <> "") $wrkonchange = " onchange=\"" . ew_JsEncode2($wrkonchange) . "\"";
$issuance_store->approved_by->EditAttrs["onchange"] = "";
?>
<span id="as_x_approved_by" style="white-space: nowrap; z-index: 8830">
	<input type="text" name="sv_x_approved_by" id="sv_x_approved_by" value="<?php echo $issuance_store->approved_by->EditValue ?>" size="30" placeholder="<?php echo ew_HtmlEncode($issuance_store->approved_by->getPlaceHolder()) ?>" data-placeholder="<?php echo ew_HtmlEncode($issuance_store->approved_by->getPlaceHolder()) ?>"<?php echo $issuance_store->approved_by->EditAttributes() ?>>
</span>
<input type="hidden" data-table="issuance_store" data-field="x_approved_by" data-page="1" data-value-separator="<?php echo $issuance_store->approved_by->DisplayValueSeparatorAttribute() ?>" name="x_approved_by" id="x_approved_by" value="<?php echo ew_HtmlEncode($issuance_store->approved_by->CurrentValue) ?>"<?php echo $wrkonchange ?>>
<script type="text/javascript">
fissuance_storeedit.CreateAutoSuggest({"id":"x_approved_by","forceSelect":false});
</script>
</span>
<?php } else { ?>
<span id="el_issuance_store_approved_by">
<span<?php echo $issuance_store->approved_by->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $issuance_store->approved_by->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="issuance_store" data-field="x_approved_by" data-page="1" name="x_approved_by" id="x_approved_by" value="<?php echo ew_HtmlEncode($issuance_store->approved_by->FormValue) ?>">
<?php } ?>
<?php echo $issuance_store->approved_by->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($issuance_store->verified_date->Visible) { // verified_date ?>
	<div id="r_verified_date" class="form-group">
		<label id="elh_issuance_store_verified_date" for="x_verified_date" class="<?php echo $issuance_store_edit->LeftColumnClass ?>"><?php echo $issuance_store->verified_date->FldCaption() ?></label>
		<div class="<?php echo $issuance_store_edit->RightColumnClass ?>"><div<?php echo $issuance_store->verified_date->CellAttributes() ?>>
<?php if ($issuance_store->CurrentAction <> "F") { ?>
<span id="el_issuance_store_verified_date">
<input type="text" data-table="issuance_store" data-field="x_verified_date" data-page="1" name="x_verified_date" id="x_verified_date" placeholder="<?php echo ew_HtmlEncode($issuance_store->verified_date->getPlaceHolder()) ?>" value="<?php echo $issuance_store->verified_date->EditValue ?>"<?php echo $issuance_store->verified_date->EditAttributes() ?>>
</span>
<?php } else { ?>
<span id="el_issuance_store_verified_date">
<span<?php echo $issuance_store->verified_date->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $issuance_store->verified_date->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="issuance_store" data-field="x_verified_date" data-page="1" name="x_verified_date" id="x_verified_date" value="<?php echo ew_HtmlEncode($issuance_store->verified_date->FormValue) ?>">
<?php } ?>
<?php echo $issuance_store->verified_date->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($issuance_store->verified_action->Visible) { // verified_action ?>
	<div id="r_verified_action" class="form-group">
		<label id="elh_issuance_store_verified_action" class="<?php echo $issuance_store_edit->LeftColumnClass ?>"><?php echo $issuance_store->verified_action->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $issuance_store_edit->RightColumnClass ?>"><div<?php echo $issuance_store->verified_action->CellAttributes() ?>>
<?php if ($issuance_store->CurrentAction <> "F") { ?>
<span id="el_issuance_store_verified_action">
<div id="tp_x_verified_action" class="ewTemplate"><input type="radio" data-table="issuance_store" data-field="x_verified_action" data-page="1" data-value-separator="<?php echo $issuance_store->verified_action->DisplayValueSeparatorAttribute() ?>" name="x_verified_action" id="x_verified_action" value="{value}"<?php echo $issuance_store->verified_action->EditAttributes() ?>></div>
<div id="dsl_x_verified_action" data-repeatcolumn="5" class="ewItemList" style="display: none;"><div>
<?php echo $issuance_store->verified_action->RadioButtonListHtml(FALSE, "x_verified_action", 1) ?>
</div></div>
</span>
<?php } else { ?>
<span id="el_issuance_store_verified_action">
<span<?php echo $issuance_store->verified_action->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $issuance_store->verified_action->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="issuance_store" data-field="x_verified_action" data-page="1" name="x_verified_action" id="x_verified_action" value="<?php echo ew_HtmlEncode($issuance_store->verified_action->FormValue) ?>">
<?php } ?>
<?php echo $issuance_store->verified_action->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($issuance_store->verified_comment->Visible) { // verified_comment ?>
	<div id="r_verified_comment" class="form-group">
		<label id="elh_issuance_store_verified_comment" for="x_verified_comment" class="<?php echo $issuance_store_edit->LeftColumnClass ?>"><?php echo $issuance_store->verified_comment->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $issuance_store_edit->RightColumnClass ?>"><div<?php echo $issuance_store->verified_comment->CellAttributes() ?>>
<?php if ($issuance_store->CurrentAction <> "F") { ?>
<span id="el_issuance_store_verified_comment">
<textarea data-table="issuance_store" data-field="x_verified_comment" data-page="1" name="x_verified_comment" id="x_verified_comment" cols="35" rows="4" placeholder="<?php echo ew_HtmlEncode($issuance_store->verified_comment->getPlaceHolder()) ?>"<?php echo $issuance_store->verified_comment->EditAttributes() ?>><?php echo $issuance_store->verified_comment->EditValue ?></textarea>
</span>
<?php } else { ?>
<span id="el_issuance_store_verified_comment">
<span<?php echo $issuance_store->verified_comment->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $issuance_store->verified_comment->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="issuance_store" data-field="x_verified_comment" data-page="1" name="x_verified_comment" id="x_verified_comment" value="<?php echo ew_HtmlEncode($issuance_store->verified_comment->FormValue) ?>">
<?php } ?>
<?php echo $issuance_store->verified_comment->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($issuance_store->verified_by->Visible) { // verified_by ?>
	<div id="r_verified_by" class="form-group">
		<label id="elh_issuance_store_verified_by" class="<?php echo $issuance_store_edit->LeftColumnClass ?>"><?php echo $issuance_store->verified_by->FldCaption() ?></label>
		<div class="<?php echo $issuance_store_edit->RightColumnClass ?>"><div<?php echo $issuance_store->verified_by->CellAttributes() ?>>
<?php if ($issuance_store->CurrentAction <> "F") { ?>
<span id="el_issuance_store_verified_by">
<?php
$wrkonchange = trim(" " . @$issuance_store->verified_by->EditAttrs["onchange"]);
if ($wrkonchange <> "") $wrkonchange = " onchange=\"" . ew_JsEncode2($wrkonchange) . "\"";
$issuance_store->verified_by->EditAttrs["onchange"] = "";
?>
<span id="as_x_verified_by" style="white-space: nowrap; z-index: 8790">
	<input type="text" name="sv_x_verified_by" id="sv_x_verified_by" value="<?php echo $issuance_store->verified_by->EditValue ?>" size="30" placeholder="<?php echo ew_HtmlEncode($issuance_store->verified_by->getPlaceHolder()) ?>" data-placeholder="<?php echo ew_HtmlEncode($issuance_store->verified_by->getPlaceHolder()) ?>"<?php echo $issuance_store->verified_by->EditAttributes() ?>>
</span>
<input type="hidden" data-table="issuance_store" data-field="x_verified_by" data-page="1" data-value-separator="<?php echo $issuance_store->verified_by->DisplayValueSeparatorAttribute() ?>" name="x_verified_by" id="x_verified_by" value="<?php echo ew_HtmlEncode($issuance_store->verified_by->CurrentValue) ?>"<?php echo $wrkonchange ?>>
<script type="text/javascript">
fissuance_storeedit.CreateAutoSuggest({"id":"x_verified_by","forceSelect":false});
</script>
</span>
<?php } else { ?>
<span id="el_issuance_store_verified_by">
<span<?php echo $issuance_store->verified_by->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $issuance_store->verified_by->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="issuance_store" data-field="x_verified_by" data-page="1" name="x_verified_by" id="x_verified_by" value="<?php echo ew_HtmlEncode($issuance_store->verified_by->FormValue) ?>">
<?php } ?>
<?php echo $issuance_store->verified_by->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($issuance_store->statuss->Visible) { // statuss ?>
	<div id="r_statuss" class="form-group">
		<label id="elh_issuance_store_statuss" for="x_statuss" class="<?php echo $issuance_store_edit->LeftColumnClass ?>"><?php echo $issuance_store->statuss->FldCaption() ?></label>
		<div class="<?php echo $issuance_store_edit->RightColumnClass ?>"><div<?php echo $issuance_store->statuss->CellAttributes() ?>>
<?php if ($issuance_store->CurrentAction <> "F") { ?>
<span id="el_issuance_store_statuss">
<select data-table="issuance_store" data-field="x_statuss" data-page="1" data-value-separator="<?php echo $issuance_store->statuss->DisplayValueSeparatorAttribute() ?>" id="x_statuss" name="x_statuss"<?php echo $issuance_store->statuss->EditAttributes() ?>>
<?php echo $issuance_store->statuss->SelectOptionListHtml("x_statuss") ?>
</select>
</span>
<?php } else { ?>
<span id="el_issuance_store_statuss">
<span<?php echo $issuance_store->statuss->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $issuance_store->statuss->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="issuance_store" data-field="x_statuss" data-page="1" name="x_statuss" id="x_statuss" value="<?php echo ew_HtmlEncode($issuance_store->statuss->FormValue) ?>">
<?php } ?>
<?php echo $issuance_store->statuss->CustomMsg ?></div></div>
	</div>
<?php } ?>
</div><!-- /page* -->
<?php if (!$issuance_store_edit->IsModal) { ?>
<div class="form-group"><!-- buttons .form-group -->
	<div class="<?php echo $issuance_store_edit->OffsetColumnClass ?>"><!-- buttons offset -->
<?php if ($issuance_store->CurrentAction <> "F") { // Confirm page ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $issuance_store_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
<?php } else { ?>
...
<script type="text/javascript">
fissuance_storeedit.Init();
</script>
<?php
$issuance_store_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$issuance_store_edit->Page_Terminate();
?>
