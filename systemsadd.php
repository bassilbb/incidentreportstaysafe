...
$systems_add = NULL; // Initialize page object first

class csystems_add extends csystems {
		...
		switch ($this->CurrentAction) {
			...
			case "C": // Copy an existing record
				if (!$loaded) { // Record not loaded
					...
					$this->Page_Terminate("systemslist.php"); // No matching record, return to list
				}
				break;
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "systemslist.php")
						$sReturnUrl = $this->AddMasterUrl($sReturnUrl); // List page, return to List page with correct master key if necessary
					elseif (ew_GetPageName($sReturnUrl) == "systemsview.php")
	...
	function LoadDefaultValues() {
		...
		$this->start_sate->CurrentValue = NULL;
		$this->start_sate->OldValue = $this->start_sate->CurrentValue;
		$this->end_date->CurrentValue = NULL;
		$this->end_date->OldValue = $this->end_date->CurrentValue;
		$this->cost_for_repair->CurrentValue = NULL;
		$this->cost_for_repair->OldValue = $this->cost_for_repair->CurrentValue;
		$this->service_provider->CurrentValue = NULL;
		$this->service_provider->OldValue = $this->service_provider->CurrentValue;
		$this->address->CurrentValue = NULL;
		$this->address->OldValue = $this->address->CurrentValue;
		$this->type_of_repair->CurrentValue = NULL;
		$this->type_of_repair->OldValue = $this->type_of_repair->CurrentValue;
		$this->note->CurrentValue = NULL;
		$this->note->OldValue = $this->note->CurrentValue;
		$this->status->CurrentValue = NULL;
		$this->status->OldValue = $this->status->CurrentValue;
		$this->asset_category->CurrentValue = NULL;
		$this->asset_category->OldValue = $this->asset_category->CurrentValue;
		$this->asset_sub_category->CurrentValue = NULL;
		$this->asset_sub_category->OldValue = $this->asset_sub_category->CurrentValue;
		$this->serial_number->CurrentValue = NULL;
		$this->serial_number->OldValue = $this->serial_number->CurrentValue;
		$this->programe_area->CurrentValue = NULL;
		$this->programe_area->OldValue = $this->programe_area->CurrentValue;
		...
		$this->device_number->CurrentValue = NULL;
		$this->device_number->OldValue = $this->device_number->CurrentValue;
		$this->tablet_imie_number->CurrentValue = NULL;
		$this->tablet_imie_number->OldValue = $this->tablet_imie_number->CurrentValue;
	...
	function NewRow() {
		...
		$row['start_sate'] = $this->start_sate->CurrentValue;
		$row['end_date'] = $this->end_date->CurrentValue;
		$row['cost_for_repair'] = $this->cost_for_repair->CurrentValue;
		$row['service_provider'] = $this->service_provider->CurrentValue;
		$row['address'] = $this->address->CurrentValue;
		$row['type_of_repair'] = $this->type_of_repair->CurrentValue;
		$row['note'] = $this->note->CurrentValue;
		$row['status'] = $this->status->CurrentValue;
		$row['asset_category'] = $this->asset_category->CurrentValue;
		$row['asset_sub_category'] = $this->asset_sub_category->CurrentValue;
		$row['serial_number'] = $this->serial_number->CurrentValue;
		$row['programe_area'] = $this->programe_area->CurrentValue;
		...
		$row['device_number'] = $this->device_number->CurrentValue;
		$row['tablet_imie_number'] = $this->tablet_imie_number->CurrentValue;
	...
	function AddRow($rsold = NULL) {
		...
		// start_sate
		$this->start_sate->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->start_sate->CurrentValue, 0), NULL, FALSE);

		// end_date
		$this->end_date->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->end_date->CurrentValue, 0), NULL, FALSE);

		// cost_for_repair
		$this->cost_for_repair->SetDbValueDef($rsnew, $this->cost_for_repair->CurrentValue, NULL, FALSE);

		// service_provider
		$this->service_provider->SetDbValueDef($rsnew, $this->service_provider->CurrentValue, NULL, FALSE);

		// address
		$this->address->SetDbValueDef($rsnew, $this->address->CurrentValue, NULL, FALSE);

		// type_of_repair
		$this->type_of_repair->SetDbValueDef($rsnew, $this->type_of_repair->CurrentValue, NULL, FALSE);
		...
		// asset_category
		$this->asset_category->SetDbValueDef($rsnew, $this->asset_category->CurrentValue, NULL, FALSE);

		// asset_sub_category
		$this->asset_sub_category->SetDbValueDef($rsnew, $this->asset_sub_category->CurrentValue, NULL, FALSE);

		// serial_number
		$this->serial_number->SetDbValueDef($rsnew, $this->serial_number->CurrentValue, NULL, FALSE);

		// programe_area
		$this->programe_area->SetDbValueDef($rsnew, $this->programe_area->CurrentValue, NULL, FALSE);
		...
		// device_number
		$this->device_number->SetDbValueDef($rsnew, $this->device_number->CurrentValue, NULL, FALSE);

		// tablet_imie_number
		$this->tablet_imie_number->SetDbValueDef($rsnew, $this->tablet_imie_number->CurrentValue, NULL, FALSE);
		...
		// updated_by
		$this->updated_by->SetDbValueDef($rsnew, $this->updated_by->CurrentValue, NULL, FALSE);

		// received_date
		$this->received_date->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->received_date->CurrentValue, 0), NULL, FALSE);
...
// Create page object
if (!isset($systems_add)) $systems_add = new csystems_add();

// Page init
$systems_add->Page_Init();

// Page main
$systems_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$systems_add->Page_Render();
?>
...
var CurrentForm = fsystemsadd = new ew_Form("fsystemsadd", "add");

// Validate form
fsystemsadd.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			if (elm && !ew_CheckDateDef(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($systems->start_sate->FldErrMsg()) ?>");
			elm = this.GetElements("x" + infix + "_end_date");
			if (elm && !ew_CheckDateDef(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($systems->end_date->FldErrMsg()) ?>");
			elm = this.GetElements("x" + infix + "_cost_for_repair");
			if (elm && !ew_CheckInteger(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($systems->cost_for_repair->FldErrMsg()) ?>");
			elm = this.GetElements("x" + infix + "_created_date");
			if (elm && !ew_CheckDateDef(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($systems->created_date->FldErrMsg()) ?>");
			elm = this.GetElements("x" + infix + "_flag");
			if (elm && !ew_CheckInteger(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($systems->flag->FldErrMsg()) ?>");
			elm = this.GetElements("x" + infix + "_updated_date");
			if (elm && !ew_CheckDateDef(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($systems->updated_date->FldErrMsg()) ?>");
			elm = this.GetElements("x" + infix + "_updated_by");
			if (elm && !ew_CheckInteger(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($systems->updated_by->FldErrMsg()) ?>");
			elm = this.GetElements("x" + infix + "_received_date");
			if (elm && !ew_CheckDateDef(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($systems->received_date->FldErrMsg()) ?>");
			elm = this.GetElements("x" + infix + "_received_by");
			if (elm && !ew_CheckInteger(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($systems->received_by->FldErrMsg()) ?>");
...
fsystemsadd.Form_CustomValidate = 
...
fsystemsadd.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;
...
<?php $systems_add->ShowPageHeader(); ?>
<?php
$systems_add->ShowMessage();
?>
<form name="fsystemsadd" id="fsystemsadd" class="<?php echo $systems_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($systems_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $systems_add->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="systems">
<input type="hidden" name="a_add" id="a_add" value="A">
<input type="hidden" name="modal" value="<?php echo intval($systems_add->IsModal) ?>">
<div class="ewAddDiv"><!-- page* -->
<?php if ($systems->asset_tag->Visible) { // asset_tag ?>
	<div id="r_asset_tag" class="form-group">
		<label id="elh_systems_asset_tag" for="x_asset_tag" class="<?php echo $systems_add->LeftColumnClass ?>"><?php echo $systems->asset_tag->FldCaption() ?></label>
		<div class="<?php echo $systems_add->RightColumnClass ?>"><div<?php echo $systems->asset_tag->CellAttributes() ?>>
<span id="el_systems_asset_tag">
<input type="text" data-table="systems" data-field="x_asset_tag" name="x_asset_tag" id="x_asset_tag" size="30" maxlength="12" placeholder="<?php echo ew_HtmlEncode($systems->asset_tag->getPlaceHolder()) ?>" value="<?php echo $systems->asset_tag->EditValue ?>"<?php echo $systems->asset_tag->EditAttributes() ?>>
</span>
<?php echo $systems->asset_tag->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($systems->start_sate->Visible) { // start_sate ?>
	<div id="r_start_sate" class="form-group">
		<label id="elh_systems_start_sate" for="x_start_sate" class="<?php echo $systems_add->LeftColumnClass ?>"><?php echo $systems->start_sate->FldCaption() ?></label>
		<div class="<?php echo $systems_add->RightColumnClass ?>"><div<?php echo $systems->start_sate->CellAttributes() ?>>
<span id="el_systems_start_sate">
<input type="text" data-table="systems" data-field="x_start_sate" name="x_start_sate" id="x_start_sate" placeholder="<?php echo ew_HtmlEncode($systems->start_sate->getPlaceHolder()) ?>" value="<?php echo $systems->start_sate->EditValue ?>"<?php echo $systems->start_sate->EditAttributes() ?>>
</span>
<?php echo $systems->start_sate->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($systems->end_date->Visible) { // end_date ?>
	<div id="r_end_date" class="form-group">
		<label id="elh_systems_end_date" for="x_end_date" class="<?php echo $systems_add->LeftColumnClass ?>"><?php echo $systems->end_date->FldCaption() ?></label>
		<div class="<?php echo $systems_add->RightColumnClass ?>"><div<?php echo $systems->end_date->CellAttributes() ?>>
<span id="el_systems_end_date">
<input type="text" data-table="systems" data-field="x_end_date" name="x_end_date" id="x_end_date" placeholder="<?php echo ew_HtmlEncode($systems->end_date->getPlaceHolder()) ?>" value="<?php echo $systems->end_date->EditValue ?>"<?php echo $systems->end_date->EditAttributes() ?>>
</span>
<?php echo $systems->end_date->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($systems->cost_for_repair->Visible) { // cost_for_repair ?>
	<div id="r_cost_for_repair" class="form-group">
		<label id="elh_systems_cost_for_repair" for="x_cost_for_repair" class="<?php echo $systems_add->LeftColumnClass ?>"><?php echo $systems->cost_for_repair->FldCaption() ?></label>
		<div class="<?php echo $systems_add->RightColumnClass ?>"><div<?php echo $systems->cost_for_repair->CellAttributes() ?>>
<span id="el_systems_cost_for_repair">
<input type="text" data-table="systems" data-field="x_cost_for_repair" name="x_cost_for_repair" id="x_cost_for_repair" size="30" placeholder="<?php echo ew_HtmlEncode($systems->cost_for_repair->getPlaceHolder()) ?>" value="<?php echo $systems->cost_for_repair->EditValue ?>"<?php echo $systems->cost_for_repair->EditAttributes() ?>>
</span>
<?php echo $systems->cost_for_repair->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($systems->service_provider->Visible) { // service_provider ?>
	<div id="r_service_provider" class="form-group">
		<label id="elh_systems_service_provider" for="x_service_provider" class="<?php echo $systems_add->LeftColumnClass ?>"><?php echo $systems->service_provider->FldCaption() ?></label>
		<div class="<?php echo $systems_add->RightColumnClass ?>"><div<?php echo $systems->service_provider->CellAttributes() ?>>
<span id="el_systems_service_provider">
<input type="text" data-table="systems" data-field="x_service_provider" name="x_service_provider" id="x_service_provider" size="30" maxlength="10" placeholder="<?php echo ew_HtmlEncode($systems->service_provider->getPlaceHolder()) ?>" value="<?php echo $systems->service_provider->EditValue ?>"<?php echo $systems->service_provider->EditAttributes() ?>>
</span>
<?php echo $systems->service_provider->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($systems->address->Visible) { // address ?>
	<div id="r_address" class="form-group">
		<label id="elh_systems_address" for="x_address" class="<?php echo $systems_add->LeftColumnClass ?>"><?php echo $systems->address->FldCaption() ?></label>
		<div class="<?php echo $systems_add->RightColumnClass ?>"><div<?php echo $systems->address->CellAttributes() ?>>
<span id="el_systems_address">
<input type="text" data-table="systems" data-field="x_address" name="x_address" id="x_address" size="30" maxlength="31" placeholder="<?php echo ew_HtmlEncode($systems->address->getPlaceHolder()) ?>" value="<?php echo $systems->address->EditValue ?>"<?php echo $systems->address->EditAttributes() ?>>
</span>
<?php echo $systems->address->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($systems->type_of_repair->Visible) { // type_of_repair ?>
	<div id="r_type_of_repair" class="form-group">
		<label id="elh_systems_type_of_repair" for="x_type_of_repair" class="<?php echo $systems_add->LeftColumnClass ?>"><?php echo $systems->type_of_repair->FldCaption() ?></label>
		<div class="<?php echo $systems_add->RightColumnClass ?>"><div<?php echo $systems->type_of_repair->CellAttributes() ?>>
<span id="el_systems_type_of_repair">
<input type="text" data-table="systems" data-field="x_type_of_repair" name="x_type_of_repair" id="x_type_of_repair" size="30" maxlength="35" placeholder="<?php echo ew_HtmlEncode($systems->type_of_repair->getPlaceHolder()) ?>" value="<?php echo $systems->type_of_repair->EditValue ?>"<?php echo $systems->type_of_repair->EditAttributes() ?>>
</span>
<?php echo $systems->type_of_repair->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($systems->note->Visible) { // note ?>
	<div id="r_note" class="form-group">
		<label id="elh_systems_note" for="x_note" class="<?php echo $systems_add->LeftColumnClass ?>"><?php echo $systems->note->FldCaption() ?></label>
		<div class="<?php echo $systems_add->RightColumnClass ?>"><div<?php echo $systems->note->CellAttributes() ?>>
<span id="el_systems_note">
<input type="text" data-table="systems" data-field="x_note" name="x_note" id="x_note" size="30" maxlength="38" placeholder="<?php echo ew_HtmlEncode($systems->note->getPlaceHolder()) ?>" value="<?php echo $systems->note->EditValue ?>"<?php echo $systems->note->EditAttributes() ?>>
</span>
<?php echo $systems->note->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($systems->status->Visible) { // status ?>
	<div id="r_status" class="form-group">
		<label id="elh_systems_status" for="x_status" class="<?php echo $systems_add->LeftColumnClass ?>"><?php echo $systems->status->FldCaption() ?></label>
		<div class="<?php echo $systems_add->RightColumnClass ?>"><div<?php echo $systems->status->CellAttributes() ?>>
<span id="el_systems_status">
<input type="text" data-table="systems" data-field="x_status" name="x_status" id="x_status" size="30" maxlength="6" placeholder="<?php echo ew_HtmlEncode($systems->status->getPlaceHolder()) ?>" value="<?php echo $systems->status->EditValue ?>"<?php echo $systems->status->EditAttributes() ?>>
</span>
<?php echo $systems->status->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($systems->asset_category->Visible) { // asset_category ?>
	<div id="r_asset_category" class="form-group">
		<label id="elh_systems_asset_category" for="x_asset_category" class="<?php echo $systems_add->LeftColumnClass ?>"><?php echo $systems->asset_category->FldCaption() ?></label>
		<div class="<?php echo $systems_add->RightColumnClass ?>"><div<?php echo $systems->asset_category->CellAttributes() ?>>
<span id="el_systems_asset_category">
<input type="text" data-table="systems" data-field="x_asset_category" name="x_asset_category" id="x_asset_category" size="30" maxlength="8" placeholder="<?php echo ew_HtmlEncode($systems->asset_category->getPlaceHolder()) ?>" value="<?php echo $systems->asset_category->EditValue ?>"<?php echo $systems->asset_category->EditAttributes() ?>>
</span>
<?php echo $systems->asset_category->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($systems->asset_sub_category->Visible) { // asset_sub_category ?>
	<div id="r_asset_sub_category" class="form-group">
		<label id="elh_systems_asset_sub_category" for="x_asset_sub_category" class="<?php echo $systems_add->LeftColumnClass ?>"><?php echo $systems->asset_sub_category->FldCaption() ?></label>
		<div class="<?php echo $systems_add->RightColumnClass ?>"><div<?php echo $systems->asset_sub_category->CellAttributes() ?>>
<span id="el_systems_asset_sub_category">
<input type="text" data-table="systems" data-field="x_asset_sub_category" name="x_asset_sub_category" id="x_asset_sub_category" size="30" maxlength="41" placeholder="<?php echo ew_HtmlEncode($systems->asset_sub_category->getPlaceHolder()) ?>" value="<?php echo $systems->asset_sub_category->EditValue ?>"<?php echo $systems->asset_sub_category->EditAttributes() ?>>
</span>
<?php echo $systems->asset_sub_category->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($systems->serial_number->Visible) { // serial_number ?>
	<div id="r_serial_number" class="form-group">
		<label id="elh_systems_serial_number" for="x_serial_number" class="<?php echo $systems_add->LeftColumnClass ?>"><?php echo $systems->serial_number->FldCaption() ?></label>
		<div class="<?php echo $systems_add->RightColumnClass ?>"><div<?php echo $systems->serial_number->CellAttributes() ?>>
<span id="el_systems_serial_number">
<input type="text" data-table="systems" data-field="x_serial_number" name="x_serial_number" id="x_serial_number" size="30" maxlength="16" placeholder="<?php echo ew_HtmlEncode($systems->serial_number->getPlaceHolder()) ?>" value="<?php echo $systems->serial_number->EditValue ?>"<?php echo $systems->serial_number->EditAttributes() ?>>
</span>
<?php echo $systems->serial_number->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($systems->programe_area->Visible) { // programe_area ?>
	<div id="r_programe_area" class="form-group">
		<label id="elh_systems_programe_area" for="x_programe_area" class="<?php echo $systems_add->LeftColumnClass ?>"><?php echo $systems->programe_area->FldCaption() ?></label>
		<div class="<?php echo $systems_add->RightColumnClass ?>"><div<?php echo $systems->programe_area->CellAttributes() ?>>
<span id="el_systems_programe_area">
<input type="text" data-table="systems" data-field="x_programe_area" name="x_programe_area" id="x_programe_area" size="30" maxlength="50" placeholder="<?php echo ew_HtmlEncode($systems->programe_area->getPlaceHolder()) ?>" value="<?php echo $systems->programe_area->EditValue ?>"<?php echo $systems->programe_area->EditAttributes() ?>>
</span>
<?php echo $systems->programe_area->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($systems->division->Visible) { // division ?>
	<div id="r_division" class="form-group">
		<label id="elh_systems_division" for="x_division" class="<?php echo $systems_add->LeftColumnClass ?>"><?php echo $systems->division->FldCaption() ?></label>
		<div class="<?php echo $systems_add->RightColumnClass ?>"><div<?php echo $systems->division->CellAttributes() ?>>
<span id="el_systems_division">
<input type="text" data-table="systems" data-field="x_division" name="x_division" id="x_division" size="30" maxlength="50" placeholder="<?php echo ew_HtmlEncode($systems->division->getPlaceHolder()) ?>" value="<?php echo $systems->division->EditValue ?>"<?php echo $systems->division->EditAttributes() ?>>
</span>
<?php echo $systems->division->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($systems->branch->Visible) { // branch ?>
	<div id="r_branch" class="form-group">
		<label id="elh_systems_branch" for="x_branch" class="<?php echo $systems_add->LeftColumnClass ?>"><?php echo $systems->branch->FldCaption() ?></label>
		<div class="<?php echo $systems_add->RightColumnClass ?>"><div<?php echo $systems->branch->CellAttributes() ?>>
<span id="el_systems_branch">
<input type="text" data-table="systems" data-field="x_branch" name="x_branch" id="x_branch" size="30" maxlength="50" placeholder="<?php echo ew_HtmlEncode($systems->branch->getPlaceHolder()) ?>" value="<?php echo $systems->branch->EditValue ?>"<?php echo $systems->branch->EditAttributes() ?>>
</span>
<?php echo $systems->branch->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($systems->department->Visible) { // department ?>
	<div id="r_department" class="form-group">
		<label id="elh_systems_department" for="x_department" class="<?php echo $systems_add->LeftColumnClass ?>"><?php echo $systems->department->FldCaption() ?></label>
		<div class="<?php echo $systems_add->RightColumnClass ?>"><div<?php echo $systems->department->CellAttributes() ?>>
<span id="el_systems_department">
<input type="text" data-table="systems" data-field="x_department" name="x_department" id="x_department" size="30" maxlength="50" placeholder="<?php echo ew_HtmlEncode($systems->department->getPlaceHolder()) ?>" value="<?php echo $systems->department->EditValue ?>"<?php echo $systems->department->EditAttributes() ?>>
</span>
<?php echo $systems->department->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($systems->staff_id->Visible) { // staff_id ?>
	<div id="r_staff_id" class="form-group">
		<label id="elh_systems_staff_id" for="x_staff_id" class="<?php echo $systems_add->LeftColumnClass ?>"><?php echo $systems->staff_id->FldCaption() ?></label>
		<div class="<?php echo $systems_add->RightColumnClass ?>"><div<?php echo $systems->staff_id->CellAttributes() ?>>
<span id="el_systems_staff_id">
<input type="text" data-table="systems" data-field="x_staff_id" name="x_staff_id" id="x_staff_id" size="30" maxlength="50" placeholder="<?php echo ew_HtmlEncode($systems->staff_id->getPlaceHolder()) ?>" value="<?php echo $systems->staff_id->EditValue ?>"<?php echo $systems->staff_id->EditAttributes() ?>>
</span>
<?php echo $systems->staff_id->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($systems->created_by->Visible) { // created_by ?>
	<div id="r_created_by" class="form-group">
		<label id="elh_systems_created_by" for="x_created_by" class="<?php echo $systems_add->LeftColumnClass ?>"><?php echo $systems->created_by->FldCaption() ?></label>
		<div class="<?php echo $systems_add->RightColumnClass ?>"><div<?php echo $systems->created_by->CellAttributes() ?>>
<span id="el_systems_created_by">
<input type="text" data-table="systems" data-field="x_created_by" name="x_created_by" id="x_created_by" size="30" maxlength="50" placeholder="<?php echo ew_HtmlEncode($systems->created_by->getPlaceHolder()) ?>" value="<?php echo $systems->created_by->EditValue ?>"<?php echo $systems->created_by->EditAttributes() ?>>
</span>
<?php echo $systems->created_by->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($systems->created_date->Visible) { // created_date ?>
	<div id="r_created_date" class="form-group">
		<label id="elh_systems_created_date" for="x_created_date" class="<?php echo $systems_add->LeftColumnClass ?>"><?php echo $systems->created_date->FldCaption() ?></label>
		<div class="<?php echo $systems_add->RightColumnClass ?>"><div<?php echo $systems->created_date->CellAttributes() ?>>
<span id="el_systems_created_date">
<input type="text" data-table="systems" data-field="x_created_date" name="x_created_date" id="x_created_date" placeholder="<?php echo ew_HtmlEncode($systems->created_date->getPlaceHolder()) ?>" value="<?php echo $systems->created_date->EditValue ?>"<?php echo $systems->created_date->EditAttributes() ?>>
</span>
<?php echo $systems->created_date->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($systems->device_number->Visible) { // device_number ?>
	<div id="r_device_number" class="form-group">
		<label id="elh_systems_device_number" for="x_device_number" class="<?php echo $systems_add->LeftColumnClass ?>"><?php echo $systems->device_number->FldCaption() ?></label>
		<div class="<?php echo $systems_add->RightColumnClass ?>"><div<?php echo $systems->device_number->CellAttributes() ?>>
<span id="el_systems_device_number">
<input type="text" data-table="systems" data-field="x_device_number" name="x_device_number" id="x_device_number" size="30" maxlength="9" placeholder="<?php echo ew_HtmlEncode($systems->device_number->getPlaceHolder()) ?>" value="<?php echo $systems->device_number->EditValue ?>"<?php echo $systems->device_number->EditAttributes() ?>>
</span>
<?php echo $systems->device_number->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($systems->tablet_imie_number->Visible) { // tablet_imie_number ?>
	<div id="r_tablet_imie_number" class="form-group">
		<label id="elh_systems_tablet_imie_number" for="x_tablet_imie_number" class="<?php echo $systems_add->LeftColumnClass ?>"><?php echo $systems->tablet_imie_number->FldCaption() ?></label>
		<div class="<?php echo $systems_add->RightColumnClass ?>"><div<?php echo $systems->tablet_imie_number->CellAttributes() ?>>
<span id="el_systems_tablet_imie_number">
<input type="text" data-table="systems" data-field="x_tablet_imie_number" name="x_tablet_imie_number" id="x_tablet_imie_number" size="30" maxlength="16" placeholder="<?php echo ew_HtmlEncode($systems->tablet_imie_number->getPlaceHolder()) ?>" value="<?php echo $systems->tablet_imie_number->EditValue ?>"<?php echo $systems->tablet_imie_number->EditAttributes() ?>>
</span>
<?php echo $systems->tablet_imie_number->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($systems->model->Visible) { // model ?>
	<div id="r_model" class="form-group">
		<label id="elh_systems_model" for="x_model" class="<?php echo $systems_add->LeftColumnClass ?>"><?php echo $systems->model->FldCaption() ?></label>
		<div class="<?php echo $systems_add->RightColumnClass ?>"><div<?php echo $systems->model->CellAttributes() ?>>
<span id="el_systems_model">
<input type="text" data-table="systems" data-field="x_model" name="x_model" id="x_model" size="30" maxlength="7" placeholder="<?php echo ew_HtmlEncode($systems->model->getPlaceHolder()) ?>" value="<?php echo $systems->model->EditValue ?>"<?php echo $systems->model->EditAttributes() ?>>
</span>
<?php echo $systems->model->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($systems->flag->Visible) { // flag ?>
	<div id="r_flag" class="form-group">
		<label id="elh_systems_flag" for="x_flag" class="<?php echo $systems_add->LeftColumnClass ?>"><?php echo $systems->flag->FldCaption() ?></label>
		<div class="<?php echo $systems_add->RightColumnClass ?>"><div<?php echo $systems->flag->CellAttributes() ?>>
<span id="el_systems_flag">
<input type="text" data-table="systems" data-field="x_flag" name="x_flag" id="x_flag" size="30" placeholder="<?php echo ew_HtmlEncode($systems->flag->getPlaceHolder()) ?>" value="<?php echo $systems->flag->EditValue ?>"<?php echo $systems->flag->EditAttributes() ?>>
</span>
<?php echo $systems->flag->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($systems->area->Visible) { // area ?>
	<div id="r_area" class="form-group">
		<label id="elh_systems_area" for="x_area" class="<?php echo $systems_add->LeftColumnClass ?>"><?php echo $systems->area->FldCaption() ?></label>
		<div class="<?php echo $systems_add->RightColumnClass ?>"><div<?php echo $systems->area->CellAttributes() ?>>
<span id="el_systems_area">
<input type="text" data-table="systems" data-field="x_area" name="x_area" id="x_area" size="30" maxlength="50" placeholder="<?php echo ew_HtmlEncode($systems->area->getPlaceHolder()) ?>" value="<?php echo $systems->area->EditValue ?>"<?php echo $systems->area->EditAttributes() ?>>
</span>
<?php echo $systems->area->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($systems->updated_date->Visible) { // updated_date ?>
	<div id="r_updated_date" class="form-group">
		<label id="elh_systems_updated_date" for="x_updated_date" class="<?php echo $systems_add->LeftColumnClass ?>"><?php echo $systems->updated_date->FldCaption() ?></label>
		<div class="<?php echo $systems_add->RightColumnClass ?>"><div<?php echo $systems->updated_date->CellAttributes() ?>>
<span id="el_systems_updated_date">
<input type="text" data-table="systems" data-field="x_updated_date" name="x_updated_date" id="x_updated_date" placeholder="<?php echo ew_HtmlEncode($systems->updated_date->getPlaceHolder()) ?>" value="<?php echo $systems->updated_date->EditValue ?>"<?php echo $systems->updated_date->EditAttributes() ?>>
</span>
<?php echo $systems->updated_date->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($systems->updated_by->Visible) { // updated_by ?>
	<div id="r_updated_by" class="form-group">
		<label id="elh_systems_updated_by" for="x_updated_by" class="<?php echo $systems_add->LeftColumnClass ?>"><?php echo $systems->updated_by->FldCaption() ?></label>
		<div class="<?php echo $systems_add->RightColumnClass ?>"><div<?php echo $systems->updated_by->CellAttributes() ?>>
<span id="el_systems_updated_by">
<input type="text" data-table="systems" data-field="x_updated_by" name="x_updated_by" id="x_updated_by" size="30" placeholder="<?php echo ew_HtmlEncode($systems->updated_by->getPlaceHolder()) ?>" value="<?php echo $systems->updated_by->EditValue ?>"<?php echo $systems->updated_by->EditAttributes() ?>>
</span>
<?php echo $systems->updated_by->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($systems->received_date->Visible) { // received_date ?>
	<div id="r_received_date" class="form-group">
		<label id="elh_systems_received_date" for="x_received_date" class="<?php echo $systems_add->LeftColumnClass ?>"><?php echo $systems->received_date->FldCaption() ?></label>
		<div class="<?php echo $systems_add->RightColumnClass ?>"><div<?php echo $systems->received_date->CellAttributes() ?>>
<span id="el_systems_received_date">
<input type="text" data-table="systems" data-field="x_received_date" name="x_received_date" id="x_received_date" placeholder="<?php echo ew_HtmlEncode($systems->received_date->getPlaceHolder()) ?>" value="<?php echo $systems->received_date->EditValue ?>"<?php echo $systems->received_date->EditAttributes() ?>>
</span>
<?php echo $systems->received_date->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($systems->received_by->Visible) { // received_by ?>
	<div id="r_received_by" class="form-group">
		<label id="elh_systems_received_by" for="x_received_by" class="<?php echo $systems_add->LeftColumnClass ?>"><?php echo $systems->received_by->FldCaption() ?></label>
		<div class="<?php echo $systems_add->RightColumnClass ?>"><div<?php echo $systems->received_by->CellAttributes() ?>>
<span id="el_systems_received_by">
<input type="text" data-table="systems" data-field="x_received_by" name="x_received_by" id="x_received_by" size="30" placeholder="<?php echo ew_HtmlEncode($systems->received_by->getPlaceHolder()) ?>" value="<?php echo $systems->received_by->EditValue ?>"<?php echo $systems->received_by->EditAttributes() ?>>
</span>
<?php echo $systems->received_by->CustomMsg ?></div></div>
	</div>
<?php } ?>
</div><!-- /page* -->
<?php if (!$systems_add->IsModal) { ?>
<div class="form-group"><!-- buttons .form-group -->
	<div class="<?php echo $systems_add->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("AddBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $systems_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div><!-- /buttons offset -->
