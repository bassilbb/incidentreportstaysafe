...
$gen_maintenance_update = NULL; // Initialize page object first

class cgen_maintenance_update extends cgen_maintenance {
	...
	// Page object name
	var $PageObjName = 'gen_maintenance_update';
	...
	function Page_Main() {
		...
		if (count($this->RecKeys) <= 0)
			$this->Page_Terminate("gen_maintenancelist.php"); // No records selected, return to list
		...
		if ($this->Recordset = $this->LoadRecordset()) {
			...
			while (!$this->Recordset->EOF) {
				if ($i == 1) {
					...
					$this->reference_id->setDbValue($this->Recordset->fields('reference_id'));
					$this->gen_name->setDbValue($this->Recordset->fields('gen_name'));
					$this->maintenance_type->setDbValue($this->Recordset->fields('maintenance_type'));
					$this->running_hours->setDbValue($this->Recordset->fields('running_hours'));
					$this->cost->setDbValue($this->Recordset->fields('cost'));
					$this->labour_fee->setDbValue($this->Recordset->fields('labour_fee'));
					$this->total->setDbValue($this->Recordset->fields('total'));
					$this->staff_id->setDbValue($this->Recordset->fields('staff_id'));
					$this->status->setDbValue($this->Recordset->fields('status'));
					$this->initiator_action->setDbValue($this->Recordset->fields('initiator_action'));
					$this->initiator_comment->setDbValue($this->Recordset->fields('initiator_comment'));
					...
					$this->approved_by->setDbValue($this->Recordset->fields('approved_by'));
					$this->flag->setDbValue($this->Recordset->fields('flag'));
				} else {
					if (!ew_CompareValue($this->datetime->DbValue, $this->Recordset->fields('datetime')))
						$this->datetime->CurrentValue = NULL;
					if (!ew_CompareValue($this->reference_id->DbValue, $this->Recordset->fields('reference_id')))
						$this->reference_id->CurrentValue = NULL;
					if (!ew_CompareValue($this->gen_name->DbValue, $this->Recordset->fields('gen_name')))
						$this->gen_name->CurrentValue = NULL;
					if (!ew_CompareValue($this->maintenance_type->DbValue, $this->Recordset->fields('maintenance_type')))
						$this->maintenance_type->CurrentValue = NULL;
					if (!ew_CompareValue($this->running_hours->DbValue, $this->Recordset->fields('running_hours')))
						$this->running_hours->CurrentValue = NULL;
					if (!ew_CompareValue($this->cost->DbValue, $this->Recordset->fields('cost')))
						$this->cost->CurrentValue = NULL;
					if (!ew_CompareValue($this->labour_fee->DbValue, $this->Recordset->fields('labour_fee')))
						$this->labour_fee->CurrentValue = NULL;
					if (!ew_CompareValue($this->total->DbValue, $this->Recordset->fields('total')))
						$this->total->CurrentValue = NULL;
					if (!ew_CompareValue($this->staff_id->DbValue, $this->Recordset->fields('staff_id')))
						$this->staff_id->CurrentValue = NULL;
					if (!ew_CompareValue($this->status->DbValue, $this->Recordset->fields('status')))
						$this->status->CurrentValue = NULL;
					if (!ew_CompareValue($this->initiator_action->DbValue, $this->Recordset->fields('initiator_action')))
						$this->initiator_action->CurrentValue = NULL;
					if (!ew_CompareValue($this->initiator_comment->DbValue, $this->Recordset->fields('initiator_comment')))
					...
					if (!ew_CompareValue($this->approved_by->DbValue, $this->Recordset->fields('approved_by')))
						$this->approved_by->CurrentValue = NULL;
					if (!ew_CompareValue($this->flag->DbValue, $this->Recordset->fields('flag')))
	...
	function LoadFormValues() {
		...
		$this->initiator_action->MultiUpdate = $objForm->GetValue("u_initiator_action");
		if (!$this->initiator_comment->FldIsDetailKey) {
			$this->initiator_comment->setFormValue($objForm->GetValue("x_initiator_comment"));
		}
		$this->initiator_comment->MultiUpdate = $objForm->GetValue("u_initiator_comment");
		...
		$this->approved_by->MultiUpdate = $objForm->GetValue("u_approved_by");
		if (!$this->flag->FldIsDetailKey) {
			$this->flag->setFormValue($objForm->GetValue("x_flag"));
		}
		$this->flag->MultiUpdate = $objForm->GetValue("u_flag");
	...
	function ValidateForm() {
		...
		if ($this->reference_id->MultiUpdate == "1") $lUpdateCnt++;
		if ($this->gen_name->MultiUpdate == "1") $lUpdateCnt++;
		if ($this->maintenance_type->MultiUpdate == "1") $lUpdateCnt++;
		if ($this->running_hours->MultiUpdate == "1") $lUpdateCnt++;
		if ($this->cost->MultiUpdate == "1") $lUpdateCnt++;
		if ($this->labour_fee->MultiUpdate == "1") $lUpdateCnt++;
		if ($this->total->MultiUpdate == "1") $lUpdateCnt++;
		if ($this->staff_id->MultiUpdate == "1") $lUpdateCnt++;
		if ($this->status->MultiUpdate == "1") $lUpdateCnt++;
		if ($this->initiator_action->MultiUpdate == "1") $lUpdateCnt++;
		if ($this->initiator_comment->MultiUpdate == "1") $lUpdateCnt++;
		...
		if ($this->approved_by->MultiUpdate == "1") $lUpdateCnt++;
		if ($this->flag->MultiUpdate == "1") $lUpdateCnt++;
		if ($lUpdateCnt == 0) {
		...
		if ($this->gen_name->MultiUpdate <> "" && !$this->gen_name->FldIsDetailKey && !is_null($this->gen_name->FormValue) && $this->gen_name->FormValue == "") {
			ew_AddMessage($gsFormError, str_replace("%s", $this->gen_name->FldCaption(), $this->gen_name->ReqErrMsg));
		}
		if ($this->maintenance_type->MultiUpdate <> "" && !$this->maintenance_type->FldIsDetailKey && !is_null($this->maintenance_type->FormValue) && $this->maintenance_type->FormValue == "") {
			ew_AddMessage($gsFormError, str_replace("%s", $this->maintenance_type->FldCaption(), $this->maintenance_type->ReqErrMsg));
		}
		if ($this->running_hours->MultiUpdate <> "" && !$this->running_hours->FldIsDetailKey && !is_null($this->running_hours->FormValue) && $this->running_hours->FormValue == "") {
			ew_AddMessage($gsFormError, str_replace("%s", $this->running_hours->FldCaption(), $this->running_hours->ReqErrMsg));
		}
		if ($this->cost->MultiUpdate <> "") {
			if (!ew_CheckNumber($this->cost->FormValue)) {
				ew_AddMessage($gsFormError, $this->cost->FldErrMsg());
			}
		}
		if ($this->labour_fee->MultiUpdate <> "") {
			if (!ew_CheckNumber($this->labour_fee->FormValue)) {
				ew_AddMessage($gsFormError, $this->labour_fee->FldErrMsg());
			}
		}
		if ($this->total->MultiUpdate <> "") {
			if (!ew_CheckNumber($this->total->FormValue)) {
				ew_AddMessage($gsFormError, $this->total->FldErrMsg());
			}
		}
		if ($this->initiator_action->MultiUpdate <> "" && $this->initiator_action->FormValue == "") {
			ew_AddMessage($gsFormError, str_replace("%s", $this->initiator_action->FldCaption(), $this->initiator_action->ReqErrMsg));
		}
		if ($this->initiator_comment->MultiUpdate <> "" && !$this->initiator_comment->FldIsDetailKey && !is_null($this->initiator_comment->FormValue) && $this->initiator_comment->FormValue == "") {
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// datetime
			$this->datetime->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->datetime->CurrentValue, 0), NULL, $this->datetime->ReadOnly || $this->datetime->MultiUpdate <> "1");

			// reference_id
			$this->reference_id->SetDbValueDef($rsnew, $this->reference_id->CurrentValue, NULL, $this->reference_id->ReadOnly || $this->reference_id->MultiUpdate <> "1");

			// gen_name
			$this->gen_name->SetDbValueDef($rsnew, $this->gen_name->CurrentValue, NULL, $this->gen_name->ReadOnly || $this->gen_name->MultiUpdate <> "1");

			// maintenance_type
			$this->maintenance_type->SetDbValueDef($rsnew, $this->maintenance_type->CurrentValue, NULL, $this->maintenance_type->ReadOnly || $this->maintenance_type->MultiUpdate <> "1");

			// running_hours
			$this->running_hours->SetDbValueDef($rsnew, $this->running_hours->CurrentValue, NULL, $this->running_hours->ReadOnly || $this->running_hours->MultiUpdate <> "1");

			// cost
			$this->cost->SetDbValueDef($rsnew, $this->cost->CurrentValue, NULL, $this->cost->ReadOnly || $this->cost->MultiUpdate <> "1");

			// labour_fee
			$this->labour_fee->SetDbValueDef($rsnew, $this->labour_fee->CurrentValue, NULL, $this->labour_fee->ReadOnly || $this->labour_fee->MultiUpdate <> "1");

			// total
			$this->total->SetDbValueDef($rsnew, $this->total->CurrentValue, NULL, $this->total->ReadOnly || $this->total->MultiUpdate <> "1");

			// staff_id
			$this->staff_id->SetDbValueDef($rsnew, $this->staff_id->CurrentValue, NULL, $this->staff_id->ReadOnly || $this->staff_id->MultiUpdate <> "1");

			// status
			$this->status->SetDbValueDef($rsnew, $this->status->CurrentValue, NULL, $this->status->ReadOnly || $this->status->MultiUpdate <> "1");

			// initiator_action
			$this->initiator_action->SetDbValueDef($rsnew, $this->initiator_action->CurrentValue, NULL, $this->initiator_action->ReadOnly || $this->initiator_action->MultiUpdate <> "1");

			// initiator_comment
			$this->initiator_comment->SetDbValueDef($rsnew, $this->initiator_comment->CurrentValue, NULL, $this->initiator_comment->ReadOnly || $this->initiator_comment->MultiUpdate <> "1");
			...
			// approved_by
			$this->approved_by->SetDbValueDef($rsnew, $this->approved_by->CurrentValue, NULL, $this->approved_by->ReadOnly || $this->approved_by->MultiUpdate <> "1");

			// flag
			$this->flag->SetDbValueDef($rsnew, $this->flag->CurrentValue, NULL, $this->flag->ReadOnly || $this->flag->MultiUpdate <> "1");
...
// Create page object
if (!isset($gen_maintenance_update)) $gen_maintenance_update = new cgen_maintenance_update();

// Page init
$gen_maintenance_update->Page_Init();

// Page main
$gen_maintenance_update->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$gen_maintenance_update->Page_Render();
?>
...
var CurrentForm = fgen_maintenanceupdate = new ew_Form("fgen_maintenanceupdate", "update");

// Validate form
fgen_maintenanceupdate.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			uelm = this.GetElements("u" + infix + "_gen_name");
			...
			uelm = this.GetElements("u" + infix + "_maintenance_type");
			if (uelm && uelm.checked) {
				if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
			...
			uelm = this.GetElements("u" + infix + "_running_hours");
			if (uelm && uelm.checked) {
				if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
			...
			uelm = this.GetElements("u" + infix + "_cost");
			...
			uelm = this.GetElements("u" + infix + "_labour_fee");
			if (uelm && uelm.checked && elm && !ew_CheckNumber(elm.value))
			...
			uelm = this.GetElements("u" + infix + "_initiator_action");
			if (uelm && uelm.checked) {
				if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
			...
			uelm = this.GetElements("u" + infix + "_initiator_comment");
			if (uelm && uelm.checked) {
				if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
			...
			uelm = this.GetElements("u" + infix + "_approved_by");
			...
			uelm = this.GetElements("u" + infix + "_flag");
			if (uelm && uelm.checked && elm && !ew_CheckInteger(elm.value))
...
fgen_maintenanceupdate.Form_CustomValidate = 
...
fgen_maintenanceupdate.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fgen_maintenanceupdate.Lists["x_gen_name"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_gen_name","x_location","x_kva",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"generator_registration"};
fgen_maintenanceupdate.Lists["x_gen_name"].Data = "<?php echo $gen_maintenance_update->gen_name->LookupFilterQuery(FALSE, "update") ?>";
fgen_maintenanceupdate.Lists["x_maintenance_type"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_description","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"maintenance_type"};
fgen_maintenanceupdate.Lists["x_maintenance_type"].Data = "<?php echo $gen_maintenance_update->maintenance_type->LookupFilterQuery(FALSE, "update") ?>";
fgen_maintenanceupdate.Lists["x_staff_id"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_firstname","x_lastname","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"users"};
fgen_maintenanceupdate.Lists["x_staff_id"].Data = "<?php echo $gen_maintenance_update->staff_id->LookupFilterQuery(FALSE, "update") ?>";
fgen_maintenanceupdate.Lists["x_status"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_description","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"gen_status"};
fgen_maintenanceupdate.Lists["x_status"].Data = "<?php echo $gen_maintenance_update->status->LookupFilterQuery(FALSE, "update") ?>";
fgen_maintenanceupdate.Lists["x_initiator_action"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fgen_maintenanceupdate.Lists["x_initiator_action"].Options = <?php echo json_encode($gen_maintenance_update->initiator_action->Options()) ?>;
fgen_maintenanceupdate.Lists["x_approver_action"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
fgen_maintenanceupdate.Lists["x_approver_action"].Options = <?php echo json_encode($gen_maintenance_update->approver_action->Options()) ?>;
fgen_maintenanceupdate.Lists["x_approved_by"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_firstname","x_lastname","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"users"};
fgen_maintenanceupdate.Lists["x_approved_by"].Data = "<?php echo $gen_maintenance_update->approved_by->LookupFilterQuery(FALSE, "update") ?>";
fgen_maintenanceupdate.AutoSuggests["x_approved_by"] = <?php echo json_encode(array("data" => "ajax=autosuggest&" . $gen_maintenance_update->approved_by->LookupFilterQuery(TRUE, "update"))) ?>;
...
<?php $gen_maintenance_update->ShowPageHeader(); ?>
<?php
$gen_maintenance_update->ShowMessage();
?>
<form name="fgen_maintenanceupdate" id="fgen_maintenanceupdate" class="<?php echo $gen_maintenance_update->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($gen_maintenance_update->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $gen_maintenance_update->Token ?>">
<?php } ?>
...
<?php } ?>
<input type="hidden" name="modal" value="<?php echo intval($gen_maintenance_update->IsModal) ?>">
<?php foreach ($gen_maintenance_update->RecKeys as $key) { ?>
<?php $keyvalue = is_array($key) ? implode($EW_COMPOSITE_KEY_SEPARATOR, $key) : $key; ?>
<input type="hidden" name="key_m[]" value="<?php echo ew_HtmlEncode($keyvalue) ?>">
<?php } ?>
<div id="tbl_gen_maintenanceupdate" class="ewUpdateDiv"><!-- page -->
	<div class="checkbox">
		<label><input type="checkbox" name="u" id="u" onclick="ew_SelectAll(this);"<?php echo $gen_maintenance_update->Disabled ?>> <?php echo $Language->Phrase("UpdateSelectAll") ?></label>
	</div>
<?php if ($gen_maintenance->datetime->Visible) { // datetime ?>
	<div id="r_datetime" class="form-group">
		<label for="x_datetime" class="<?php echo $gen_maintenance_update->LeftColumnClass ?>"><div class="checkbox"><label>
<?php if ($gen_maintenance->CurrentAction <> "F") { ?>
<input type="checkbox" name="u_datetime" id="u_datetime" class="ewMultiSelect" value="1"<?php echo ($gen_maintenance->datetime->MultiUpdate == "1") ? " checked" : "" ?>>
<?php } else { ?>
<input type="checkbox" disabled<?php echo ($gen_maintenance->datetime->MultiUpdate == "1") ? " checked" : "" ?>>
<input type="hidden" name="u_datetime" id="u_datetime" value="<?php echo $gen_maintenance->datetime->MultiUpdate ?>">
<?php } ?>
 <?php echo $gen_maintenance->datetime->FldCaption() ?></label></div></label>
		<div class="<?php echo $gen_maintenance_update->RightColumnClass ?>"><div<?php echo $gen_maintenance->datetime->CellAttributes() ?>>
<?php if ($gen_maintenance->CurrentAction <> "F") { ?>
...
<input type="hidden" data-table="gen_maintenance" data-field="x_datetime" name="x_datetime" id="x_datetime" value="<?php echo ew_HtmlEncode($gen_maintenance->datetime->FormValue) ?>">
<?php } ?>
...
<?php if ($gen_maintenance->reference_id->Visible) { // reference_id ?>
	<div id="r_reference_id" class="form-group">
		<label for="x_reference_id" class="<?php echo $gen_maintenance_update->LeftColumnClass ?>"><div class="checkbox"><label>
<?php if ($gen_maintenance->CurrentAction <> "F") { ?>
<input type="checkbox" name="u_reference_id" id="u_reference_id" class="ewMultiSelect" value="1"<?php echo ($gen_maintenance->reference_id->MultiUpdate == "1") ? " checked" : "" ?>>
<?php } else { ?>
<input type="checkbox" disabled<?php echo ($gen_maintenance->reference_id->MultiUpdate == "1") ? " checked" : "" ?>>
<input type="hidden" name="u_reference_id" id="u_reference_id" value="<?php echo $gen_maintenance->reference_id->MultiUpdate ?>">
<?php } ?>
 <?php echo $gen_maintenance->reference_id->FldCaption() ?></label></div></label>
		<div class="<?php echo $gen_maintenance_update->RightColumnClass ?>"><div<?php echo $gen_maintenance->reference_id->CellAttributes() ?>>
<?php if ($gen_maintenance->CurrentAction <> "F") { ?>
...
<p class="form-control-static"><?php echo $gen_maintenance->reference_id->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="gen_maintenance" data-field="x_reference_id" name="x_reference_id" id="x_reference_id" value="<?php echo ew_HtmlEncode($gen_maintenance->reference_id->FormValue) ?>">
<?php } ?>
...
<?php if ($gen_maintenance->gen_name->Visible) { // gen_name ?>
	<div id="r_gen_name" class="form-group">
		<label for="x_gen_name" class="<?php echo $gen_maintenance_update->LeftColumnClass ?>"><div class="checkbox"><label>
<?php if ($gen_maintenance->CurrentAction <> "F") { ?>
<input type="checkbox" name="u_gen_name" id="u_gen_name" class="ewMultiSelect" value="1"<?php echo ($gen_maintenance->gen_name->MultiUpdate == "1") ? " checked" : "" ?>>
<?php } else { ?>
<input type="checkbox" disabled<?php echo ($gen_maintenance->gen_name->MultiUpdate == "1") ? " checked" : "" ?>>
<input type="hidden" name="u_gen_name" id="u_gen_name" value="<?php echo $gen_maintenance->gen_name->MultiUpdate ?>">
<?php } ?>
 <?php echo $gen_maintenance->gen_name->FldCaption() ?></label></div></label>
		<div class="<?php echo $gen_maintenance_update->RightColumnClass ?>"><div<?php echo $gen_maintenance->gen_name->CellAttributes() ?>>
<?php if ($gen_maintenance->CurrentAction <> "F") { ?>
...
<input type="hidden" data-table="gen_maintenance" data-field="x_gen_name" name="x_gen_name" id="x_gen_name" value="<?php echo ew_HtmlEncode($gen_maintenance->gen_name->FormValue) ?>">
<?php } ?>
...
<?php if ($gen_maintenance->maintenance_type->Visible) { // maintenance_type ?>
	<div id="r_maintenance_type" class="form-group">
		<label for="x_maintenance_type" class="<?php echo $gen_maintenance_update->LeftColumnClass ?>"><div class="checkbox"><label>
<?php if ($gen_maintenance->CurrentAction <> "F") { ?>
<input type="checkbox" name="u_maintenance_type" id="u_maintenance_type" class="ewMultiSelect" value="1"<?php echo ($gen_maintenance->maintenance_type->MultiUpdate == "1") ? " checked" : "" ?>>
<?php } else { ?>
<input type="checkbox" disabled<?php echo ($gen_maintenance->maintenance_type->MultiUpdate == "1") ? " checked" : "" ?>>
<input type="hidden" name="u_maintenance_type" id="u_maintenance_type" value="<?php echo $gen_maintenance->maintenance_type->MultiUpdate ?>">
<?php } ?>
 <?php echo $gen_maintenance->maintenance_type->FldCaption() ?></label></div></label>
		<div class="<?php echo $gen_maintenance_update->RightColumnClass ?>"><div<?php echo $gen_maintenance->maintenance_type->CellAttributes() ?>>
<?php if ($gen_maintenance->CurrentAction <> "F") { ?>
...
<input type="hidden" data-table="gen_maintenance" data-field="x_maintenance_type" name="x_maintenance_type" id="x_maintenance_type" value="<?php echo ew_HtmlEncode($gen_maintenance->maintenance_type->FormValue) ?>">
<?php } ?>
...
<?php if ($gen_maintenance->running_hours->Visible) { // running_hours ?>
	<div id="r_running_hours" class="form-group">
		<label for="x_running_hours" class="<?php echo $gen_maintenance_update->LeftColumnClass ?>"><div class="checkbox"><label>
<?php if ($gen_maintenance->CurrentAction <> "F") { ?>
<input type="checkbox" name="u_running_hours" id="u_running_hours" class="ewMultiSelect" value="1"<?php echo ($gen_maintenance->running_hours->MultiUpdate == "1") ? " checked" : "" ?>>
<?php } else { ?>
<input type="checkbox" disabled<?php echo ($gen_maintenance->running_hours->MultiUpdate == "1") ? " checked" : "" ?>>
<input type="hidden" name="u_running_hours" id="u_running_hours" value="<?php echo $gen_maintenance->running_hours->MultiUpdate ?>">
<?php } ?>
 <?php echo $gen_maintenance->running_hours->FldCaption() ?></label></div></label>
		<div class="<?php echo $gen_maintenance_update->RightColumnClass ?>"><div<?php echo $gen_maintenance->running_hours->CellAttributes() ?>>
<?php if ($gen_maintenance->CurrentAction <> "F") { ?>
...
<input type="hidden" data-table="gen_maintenance" data-field="x_running_hours" name="x_running_hours" id="x_running_hours" value="<?php echo ew_HtmlEncode($gen_maintenance->running_hours->FormValue) ?>">
<?php } ?>
...
<?php if ($gen_maintenance->cost->Visible) { // cost ?>
	<div id="r_cost" class="form-group">
		<label for="x_cost" class="<?php echo $gen_maintenance_update->LeftColumnClass ?>"><div class="checkbox"><label>
<?php if ($gen_maintenance->CurrentAction <> "F") { ?>
<input type="checkbox" name="u_cost" id="u_cost" class="ewMultiSelect" value="1"<?php echo ($gen_maintenance->cost->MultiUpdate == "1") ? " checked" : "" ?>>
<?php } else { ?>
<input type="checkbox" disabled<?php echo ($gen_maintenance->cost->MultiUpdate == "1") ? " checked" : "" ?>>
<input type="hidden" name="u_cost" id="u_cost" value="<?php echo $gen_maintenance->cost->MultiUpdate ?>">
<?php } ?>
 <?php echo $gen_maintenance->cost->FldCaption() ?></label></div></label>
		<div class="<?php echo $gen_maintenance_update->RightColumnClass ?>"><div<?php echo $gen_maintenance->cost->CellAttributes() ?>>
<?php if ($gen_maintenance->CurrentAction <> "F") { ?>
...
<input type="hidden" data-table="gen_maintenance" data-field="x_cost" name="x_cost" id="x_cost" value="<?php echo ew_HtmlEncode($gen_maintenance->cost->FormValue) ?>">
<?php } ?>
...
<?php if ($gen_maintenance->labour_fee->Visible) { // labour_fee ?>
	<div id="r_labour_fee" class="form-group">
		<label for="x_labour_fee" class="<?php echo $gen_maintenance_update->LeftColumnClass ?>"><div class="checkbox"><label>
<?php if ($gen_maintenance->CurrentAction <> "F") { ?>
<input type="checkbox" name="u_labour_fee" id="u_labour_fee" class="ewMultiSelect" value="1"<?php echo ($gen_maintenance->labour_fee->MultiUpdate == "1") ? " checked" : "" ?>>
<?php } else { ?>
<input type="checkbox" disabled<?php echo ($gen_maintenance->labour_fee->MultiUpdate == "1") ? " checked" : "" ?>>
<input type="hidden" name="u_labour_fee" id="u_labour_fee" value="<?php echo $gen_maintenance->labour_fee->MultiUpdate ?>">
<?php } ?>
 <?php echo $gen_maintenance->labour_fee->FldCaption() ?></label></div></label>
		<div class="<?php echo $gen_maintenance_update->RightColumnClass ?>"><div<?php echo $gen_maintenance->labour_fee->CellAttributes() ?>>
<?php if ($gen_maintenance->CurrentAction <> "F") { ?>
...
<input type="hidden" data-table="gen_maintenance" data-field="x_labour_fee" name="x_labour_fee" id="x_labour_fee" value="<?php echo ew_HtmlEncode($gen_maintenance->labour_fee->FormValue) ?>">
<?php } ?>
...
<?php if ($gen_maintenance->total->Visible) { // total ?>
	<div id="r_total" class="form-group">
		<label for="x_total" class="<?php echo $gen_maintenance_update->LeftColumnClass ?>"><div class="checkbox"><label>
<?php if ($gen_maintenance->CurrentAction <> "F") { ?>
<input type="checkbox" name="u_total" id="u_total" class="ewMultiSelect" value="1"<?php echo ($gen_maintenance->total->MultiUpdate == "1") ? " checked" : "" ?>>
<?php } else { ?>
<input type="checkbox" disabled<?php echo ($gen_maintenance->total->MultiUpdate == "1") ? " checked" : "" ?>>
<input type="hidden" name="u_total" id="u_total" value="<?php echo $gen_maintenance->total->MultiUpdate ?>">
<?php } ?>
 <?php echo $gen_maintenance->total->FldCaption() ?></label></div></label>
		<div class="<?php echo $gen_maintenance_update->RightColumnClass ?>"><div<?php echo $gen_maintenance->total->CellAttributes() ?>>
<?php if ($gen_maintenance->CurrentAction <> "F") { ?>
...
<input type="hidden" data-table="gen_maintenance" data-field="x_total" name="x_total" id="x_total" value="<?php echo ew_HtmlEncode($gen_maintenance->total->FormValue) ?>">
<?php } ?>
...
<?php if ($gen_maintenance->staff_id->Visible) { // staff_id ?>
	<div id="r_staff_id" class="form-group">
		<label for="x_staff_id" class="<?php echo $gen_maintenance_update->LeftColumnClass ?>"><div class="checkbox"><label>
<?php if ($gen_maintenance->CurrentAction <> "F") { ?>
<input type="checkbox" name="u_staff_id" id="u_staff_id" class="ewMultiSelect" value="1"<?php echo ($gen_maintenance->staff_id->MultiUpdate == "1") ? " checked" : "" ?>>
<?php } else { ?>
<input type="checkbox" disabled<?php echo ($gen_maintenance->staff_id->MultiUpdate == "1") ? " checked" : "" ?>>
<input type="hidden" name="u_staff_id" id="u_staff_id" value="<?php echo $gen_maintenance->staff_id->MultiUpdate ?>">
<?php } ?>
 <?php echo $gen_maintenance->staff_id->FldCaption() ?></label></div></label>
		<div class="<?php echo $gen_maintenance_update->RightColumnClass ?>"><div<?php echo $gen_maintenance->staff_id->CellAttributes() ?>>
<?php if ($gen_maintenance->CurrentAction <> "F") { ?>
...
<input type="hidden" data-table="gen_maintenance" data-field="x_staff_id" name="x_staff_id" id="x_staff_id" value="<?php echo ew_HtmlEncode($gen_maintenance->staff_id->FormValue) ?>">
<?php } ?>
...
<?php if ($gen_maintenance->status->Visible) { // status ?>
	<div id="r_status" class="form-group">
		<label for="x_status" class="<?php echo $gen_maintenance_update->LeftColumnClass ?>"><div class="checkbox"><label>
<?php if ($gen_maintenance->CurrentAction <> "F") { ?>
<input type="checkbox" name="u_status" id="u_status" class="ewMultiSelect" value="1"<?php echo ($gen_maintenance->status->MultiUpdate == "1") ? " checked" : "" ?>>
<?php } else { ?>
<input type="checkbox" disabled<?php echo ($gen_maintenance->status->MultiUpdate == "1") ? " checked" : "" ?>>
<input type="hidden" name="u_status" id="u_status" value="<?php echo $gen_maintenance->status->MultiUpdate ?>">
<?php } ?>
 <?php echo $gen_maintenance->status->FldCaption() ?></label></div></label>
		<div class="<?php echo $gen_maintenance_update->RightColumnClass ?>"><div<?php echo $gen_maintenance->status->CellAttributes() ?>>
<?php if ($gen_maintenance->CurrentAction <> "F") { ?>
...
<input type="hidden" data-table="gen_maintenance" data-field="x_status" name="x_status" id="x_status" value="<?php echo ew_HtmlEncode($gen_maintenance->status->FormValue) ?>">
<?php } ?>
...
<?php if ($gen_maintenance->initiator_action->Visible) { // initiator_action ?>
	<div id="r_initiator_action" class="form-group">
		<label class="<?php echo $gen_maintenance_update->LeftColumnClass ?>"><div class="checkbox"><label>
<?php if ($gen_maintenance->CurrentAction <> "F") { ?>
<input type="checkbox" name="u_initiator_action" id="u_initiator_action" class="ewMultiSelect" value="1"<?php echo ($gen_maintenance->initiator_action->MultiUpdate == "1") ? " checked" : "" ?>>
<?php } else { ?>
<input type="checkbox" disabled<?php echo ($gen_maintenance->initiator_action->MultiUpdate == "1") ? " checked" : "" ?>>
<input type="hidden" name="u_initiator_action" id="u_initiator_action" value="<?php echo $gen_maintenance->initiator_action->MultiUpdate ?>">
<?php } ?>
 <?php echo $gen_maintenance->initiator_action->FldCaption() ?></label></div></label>
		<div class="<?php echo $gen_maintenance_update->RightColumnClass ?>"><div<?php echo $gen_maintenance->initiator_action->CellAttributes() ?>>
<?php if ($gen_maintenance->CurrentAction <> "F") { ?>
...
<input type="hidden" data-table="gen_maintenance" data-field="x_initiator_action" name="x_initiator_action" id="x_initiator_action" value="<?php echo ew_HtmlEncode($gen_maintenance->initiator_action->FormValue) ?>">
<?php } ?>
...
<?php if ($gen_maintenance->initiator_comment->Visible) { // initiator_comment ?>
	<div id="r_initiator_comment" class="form-group">
		<label for="x_initiator_comment" class="<?php echo $gen_maintenance_update->LeftColumnClass ?>"><div class="checkbox"><label>
<?php if ($gen_maintenance->CurrentAction <> "F") { ?>
<input type="checkbox" name="u_initiator_comment" id="u_initiator_comment" class="ewMultiSelect" value="1"<?php echo ($gen_maintenance->initiator_comment->MultiUpdate == "1") ? " checked" : "" ?>>
<?php } else { ?>
<input type="checkbox" disabled<?php echo ($gen_maintenance->initiator_comment->MultiUpdate == "1") ? " checked" : "" ?>>
<input type="hidden" name="u_initiator_comment" id="u_initiator_comment" value="<?php echo $gen_maintenance->initiator_comment->MultiUpdate ?>">
<?php } ?>
 <?php echo $gen_maintenance->initiator_comment->FldCaption() ?></label></div></label>
		<div class="<?php echo $gen_maintenance_update->RightColumnClass ?>"><div<?php echo $gen_maintenance->initiator_comment->CellAttributes() ?>>
<?php if ($gen_maintenance->CurrentAction <> "F") { ?>
<span id="el_gen_maintenance_initiator_comment">
<textarea data-table="gen_maintenance" data-field="x_initiator_comment" name="x_initiator_comment" id="x_initiator_comment" cols="30" rows="4" placeholder="<?php echo ew_HtmlEncode($gen_maintenance->initiator_comment->getPlaceHolder()) ?>"<?php echo $gen_maintenance->initiator_comment->EditAttributes() ?>><?php echo $gen_maintenance->initiator_comment->EditValue ?></textarea>
</span>
<?php } else { ?>
<span id="el_gen_maintenance_initiator_comment">
...
<input type="hidden" data-table="gen_maintenance" data-field="x_initiator_comment" name="x_initiator_comment" id="x_initiator_comment" value="<?php echo ew_HtmlEncode($gen_maintenance->initiator_comment->FormValue) ?>">
<?php } ?>
...
<?php if ($gen_maintenance->approver_date->Visible) { // approver_date ?>
	<div id="r_approver_date" class="form-group">
		<label for="x_approver_date" class="<?php echo $gen_maintenance_update->LeftColumnClass ?>"><div class="checkbox"><label>
<?php if ($gen_maintenance->CurrentAction <> "F") { ?>
<input type="checkbox" name="u_approver_date" id="u_approver_date" class="ewMultiSelect" value="1"<?php echo ($gen_maintenance->approver_date->MultiUpdate == "1") ? " checked" : "" ?>>
<?php } else { ?>
<input type="checkbox" disabled<?php echo ($gen_maintenance->approver_date->MultiUpdate == "1") ? " checked" : "" ?>>
<input type="hidden" name="u_approver_date" id="u_approver_date" value="<?php echo $gen_maintenance->approver_date->MultiUpdate ?>">
<?php } ?>
 <?php echo $gen_maintenance->approver_date->FldCaption() ?></label></div></label>
		<div class="<?php echo $gen_maintenance_update->RightColumnClass ?>"><div<?php echo $gen_maintenance->approver_date->CellAttributes() ?>>
<?php if ($gen_maintenance->CurrentAction <> "F") { ?>
...
<input type="hidden" data-table="gen_maintenance" data-field="x_approver_date" name="x_approver_date" id="x_approver_date" value="<?php echo ew_HtmlEncode($gen_maintenance->approver_date->FormValue) ?>">
<?php } ?>
...
<?php if ($gen_maintenance->approver_action->Visible) { // approver_action ?>
	<div id="r_approver_action" class="form-group">
		<label class="<?php echo $gen_maintenance_update->LeftColumnClass ?>"><div class="checkbox"><label>
<?php if ($gen_maintenance->CurrentAction <> "F") { ?>
<input type="checkbox" name="u_approver_action" id="u_approver_action" class="ewMultiSelect" value="1"<?php echo ($gen_maintenance->approver_action->MultiUpdate == "1") ? " checked" : "" ?>>
<?php } else { ?>
<input type="checkbox" disabled<?php echo ($gen_maintenance->approver_action->MultiUpdate == "1") ? " checked" : "" ?>>
<input type="hidden" name="u_approver_action" id="u_approver_action" value="<?php echo $gen_maintenance->approver_action->MultiUpdate ?>">
<?php } ?>
 <?php echo $gen_maintenance->approver_action->FldCaption() ?></label></div></label>
		<div class="<?php echo $gen_maintenance_update->RightColumnClass ?>"><div<?php echo $gen_maintenance->approver_action->CellAttributes() ?>>
<?php if ($gen_maintenance->CurrentAction <> "F") { ?>
...
<input type="hidden" data-table="gen_maintenance" data-field="x_approver_action" name="x_approver_action" id="x_approver_action" value="<?php echo ew_HtmlEncode($gen_maintenance->approver_action->FormValue) ?>">
<?php } ?>
...
<?php if ($gen_maintenance->approver_comment->Visible) { // approver_comment ?>
	<div id="r_approver_comment" class="form-group">
		<label for="x_approver_comment" class="<?php echo $gen_maintenance_update->LeftColumnClass ?>"><div class="checkbox"><label>
<?php if ($gen_maintenance->CurrentAction <> "F") { ?>
<input type="checkbox" name="u_approver_comment" id="u_approver_comment" class="ewMultiSelect" value="1"<?php echo ($gen_maintenance->approver_comment->MultiUpdate == "1") ? " checked" : "" ?>>
<?php } else { ?>
<input type="checkbox" disabled<?php echo ($gen_maintenance->approver_comment->MultiUpdate == "1") ? " checked" : "" ?>>
<input type="hidden" name="u_approver_comment" id="u_approver_comment" value="<?php echo $gen_maintenance->approver_comment->MultiUpdate ?>">
<?php } ?>
 <?php echo $gen_maintenance->approver_comment->FldCaption() ?></label></div></label>
		<div class="<?php echo $gen_maintenance_update->RightColumnClass ?>"><div<?php echo $gen_maintenance->approver_comment->CellAttributes() ?>>
<?php if ($gen_maintenance->CurrentAction <> "F") { ?>
<span id="el_gen_maintenance_approver_comment">
<textarea data-table="gen_maintenance" data-field="x_approver_comment" name="x_approver_comment" id="x_approver_comment" cols="30" rows="4" placeholder="<?php echo ew_HtmlEncode($gen_maintenance->approver_comment->getPlaceHolder()) ?>"<?php echo $gen_maintenance->approver_comment->EditAttributes() ?>><?php echo $gen_maintenance->approver_comment->EditValue ?></textarea>
</span>
<?php } else { ?>
<span id="el_gen_maintenance_approver_comment">
...
<input type="hidden" data-table="gen_maintenance" data-field="x_approver_comment" name="x_approver_comment" id="x_approver_comment" value="<?php echo ew_HtmlEncode($gen_maintenance->approver_comment->FormValue) ?>">
<?php } ?>
...
<?php if ($gen_maintenance->approved_by->Visible) { // approved_by ?>
	<div id="r_approved_by" class="form-group">
		<label class="<?php echo $gen_maintenance_update->LeftColumnClass ?>"><div class="checkbox"><label>
<?php if ($gen_maintenance->CurrentAction <> "F") { ?>
<input type="checkbox" name="u_approved_by" id="u_approved_by" class="ewMultiSelect" value="1"<?php echo ($gen_maintenance->approved_by->MultiUpdate == "1") ? " checked" : "" ?>>
<?php } else { ?>
<input type="checkbox" disabled<?php echo ($gen_maintenance->approved_by->MultiUpdate == "1") ? " checked" : "" ?>>
<input type="hidden" name="u_approved_by" id="u_approved_by" value="<?php echo $gen_maintenance->approved_by->MultiUpdate ?>">
<?php } ?>
 <?php echo $gen_maintenance->approved_by->FldCaption() ?></label></div></label>
		<div class="<?php echo $gen_maintenance_update->RightColumnClass ?>"><div<?php echo $gen_maintenance->approved_by->CellAttributes() ?>>
<?php if ($gen_maintenance->CurrentAction <> "F") { ?>
...
<input type="hidden" data-table="gen_maintenance" data-field="x_approved_by" data-value-separator="<?php echo $gen_maintenance->approved_by->DisplayValueSeparatorAttribute() ?>" name="x_approved_by" id="x_approved_by" value="<?php echo ew_HtmlEncode($gen_maintenance->approved_by->CurrentValue) ?>"<?php echo $wrkonchange ?>>
<script type="text/javascript">
fgen_maintenanceupdate.CreateAutoSuggest({"id":"x_approved_by","forceSelect":false});
</script>
</span>
<?php } else { ?>
<span id="el_gen_maintenance_approved_by">
...
<input type="hidden" data-table="gen_maintenance" data-field="x_approved_by" name="x_approved_by" id="x_approved_by" value="<?php echo ew_HtmlEncode($gen_maintenance->approved_by->FormValue) ?>">
<?php } ?>
...
<?php if ($gen_maintenance->flag->Visible) { // flag ?>
	<div id="r_flag" class="form-group">
		<label for="x_flag" class="<?php echo $gen_maintenance_update->LeftColumnClass ?>"><div class="checkbox"><label>
<?php if ($gen_maintenance->CurrentAction <> "F") { ?>
<input type="checkbox" name="u_flag" id="u_flag" class="ewMultiSelect" value="1"<?php echo ($gen_maintenance->flag->MultiUpdate == "1") ? " checked" : "" ?>>
<?php } else { ?>
<input type="checkbox" disabled<?php echo ($gen_maintenance->flag->MultiUpdate == "1") ? " checked" : "" ?>>
<input type="hidden" name="u_flag" id="u_flag" value="<?php echo $gen_maintenance->flag->MultiUpdate ?>">
<?php } ?>
 <?php echo $gen_maintenance->flag->FldCaption() ?></label></div></label>
		<div class="<?php echo $gen_maintenance_update->RightColumnClass ?>"><div<?php echo $gen_maintenance->flag->CellAttributes() ?>>
<?php if ($gen_maintenance->CurrentAction <> "F") { ?>
...
<p class="form-control-static"><?php echo $gen_maintenance->flag->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="gen_maintenance" data-field="x_flag" name="x_flag" id="x_flag" value="<?php echo ew_HtmlEncode($gen_maintenance->flag->FormValue) ?>">
<?php } ?>
<?php echo $gen_maintenance->flag->CustomMsg ?></div></div>
	</div>
<?php } ?>
</div><!-- /page -->
<?php if (!$gen_maintenance_update->IsModal) { ?>
	<div class="form-group"><!-- buttons .form-group -->
		<div class="<?php echo $gen_maintenance_update->OffsetColumnClass ?>"><!-- buttons offset -->
<?php if ($gen_maintenance->CurrentAction <> "F") { // Confirm page ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $gen_maintenance_update->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
<?php } else { ?>
...
<script type="text/javascript">
fgen_maintenanceupdate.Init();
</script>
<?php
$gen_maintenance_update->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
$("#g_approver_action").hide();
$("#g_approver_action").prop("checked", true);
...
<?php
$gen_maintenance_update->Page_Terminate();
?>
