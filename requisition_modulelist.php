...
$requisition_module_list = NULL; // Initialize page object first

class crequisition_module_list extends crequisition_module {
	...
	// Page object name
	var $PageObjName = 'requisition_module_list';

	// Grid form hidden field names
	var $FormName = 'frequisition_modulelist';
	...
	function __construct() {
		...
		$this->AddUrl = "requisition_moduleadd.php";
		$this->InlineAddUrl = $this->PageUrl() . "a=add";
		$this->GridAddUrl = $this->PageUrl() . "a=gridadd";
		$this->GridEditUrl = $this->PageUrl() . "a=gridedit";
		$this->MultiDeleteUrl = "requisition_moduledelete.php";
		$this->MultiUpdateUrl = "requisition_moduleupdate.php";
		...
		$this->FilterOptions->TagClassName = "ewFilterOption frequisition_modulelistsrch";
	...
	function GetFilterList() {
		...
		if (EW_SEARCH_FILTER_OPTION == "Server" && isset($UserProfile))
			$sSavedFilterList = $UserProfile->GetSearchFilters(CurrentUserName(), "frequisition_modulelistsrch");
	...
	function ProcessFilterList() {
		...
		if (@$_POST["ajax"] == "savefilters") { // Save filter request (Ajax)
			...
			$UserProfile->SetSearchFilters(CurrentUserName(), "frequisition_modulelistsrch", $filters);
	...
	function RenderListOptions() {
		...
		if ($Security->CanView()) {
			if (ew_IsMobile())
				...
				$oListOpt->Body = "<a class=\"ewRowLink ewView\" title=\"" . $viewcaption . "\" data-table=\"requisition_module\" data-caption=\"" . $viewcaption . "\" href=\"javascript:void(0);\" onclick=\"ew_ModalDialogShow({lnk:this,url:'" . ew_HtmlEncode($this->ViewUrl) . "',btn:null});\">" . $Language->Phrase("ViewLink") . "</a>";
		} else {
			$oListOpt->Body = "";
		}
	...
	function SetupOtherOptions() {
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"frequisition_modulelistsrch\" href=\"#\">" . $Language->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"frequisition_modulelistsrch\" href=\"#\">" . $Language->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->ListActions->Items as $listaction) {
				if ($listaction->Select == EW_ACTION_MULTIPLE) {
					...
					$item->Body = "<a class=\"ewAction ewListAction\" title=\"" . ew_HtmlEncode($caption) . "\" data-caption=\"" . ew_HtmlEncode($caption) . "\" href=\"\" onclick=\"ew_SubmitAction(event,jQuery.extend({f:document.frequisition_modulelist}," . $listaction->ToJson(TRUE) . "));return false;\">" . $icon . "</a>";
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $Language->Phrase("SearchPanel") . "\" data-caption=\"" . $Language->Phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"frequisition_modulelistsrch\">" . $Language->Phrase("SearchLink") . "</button>";
		$item->Visible = TRUE;
		...
		$item->Body = "<a class=\"btn btn-default ewAdvancedSearch\" title=\"" . $Language->Phrase("AdvancedSearch") . "\" data-caption=\"" . $Language->Phrase("AdvancedSearch") . "\" href=\"requisition_modulesrch.php\">" . $Language->Phrase("AdvancedSearchBtn") . "</a>";
		$item->Visible = TRUE;

		// Search highlight button
		$item = &$this->SearchOptions->Add("searchhighlight");
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewHighlight active\" title=\"" . $Language->Phrase("Highlight") . "\" data-caption=\"" . $Language->Phrase("Highlight") . "\" data-toggle=\"button\" data-form=\"frequisition_modulelistsrch\" data-name=\"" . $this->HighlightName() . "\">" . $Language->Phrase("HighlightBtn") . "</button>";
	...
	function RenderRow() {
		...
		$this->rep_comment->ViewCustomAttributes = "";
			...
			if ($this->Export == "")
				$this->organization->ViewValue = $this->HighlightValue($this->organization);
	...
	function SetupExportOptions() {
		...
		$url = "";
		$item->Body = "<button id=\"emf_requisition_module\" class=\"ewExportLink ewEmail\" title=\"" . $Language->Phrase("ExportToEmailText") . "\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_requisition_module',hdr:ewLanguage.Phrase('ExportToEmailText'),f:document.frequisition_modulelist,sel:false" . $url . "});\">" . $Language->Phrase("ExportToEmail") . "</button>";
		$item->Visible = FALSE;
	...
	function Page_Load() {
		...
		if (CurrentPageID() == "list"){
			 $_SESSION['REQ_ID'] = generateREQKey();
		 }
	}
...
// Create page object
if (!isset($requisition_module_list)) $requisition_module_list = new crequisition_module_list();

// Page init
$requisition_module_list->Page_Init();

// Page main
$requisition_module_list->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$requisition_module_list->Page_Render();
?>
...
var CurrentForm = frequisition_modulelist = new ew_Form("frequisition_modulelist", "list");
frequisition_modulelist.FormKeyCountName = '<?php echo $requisition_module_list->FormKeyCountName ?>';

// Form_CustomValidate event
frequisition_modulelist.Form_CustomValidate = 
...
frequisition_modulelist.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
frequisition_modulelist.Lists["x_name"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_firstname","x_lastname","x_staffno",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"users"};
frequisition_modulelist.Lists["x_name"].Data = "<?php echo $requisition_module_list->name->LookupFilterQuery(FALSE, "list") ?>";
frequisition_modulelist.AutoSuggests["x_name"] = <?php echo json_encode(array("data" => "ajax=autosuggest&" . $requisition_module_list->name->LookupFilterQuery(TRUE, "list"))) ?>;
frequisition_modulelist.Lists["x_organization"] = {"LinkField":"x_branch_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_branch_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"branch"};
frequisition_modulelist.Lists["x_organization"].Data = "<?php echo $requisition_module_list->organization->LookupFilterQuery(FALSE, "list") ?>";
frequisition_modulelist.AutoSuggests["x_organization"] = <?php echo json_encode(array("data" => "ajax=autosuggest&" . $requisition_module_list->organization->LookupFilterQuery(TRUE, "list"))) ?>;
frequisition_modulelist.Lists["x_designation"] = {"LinkField":"x_code","Ajax":true,"AutoFill":false,"DisplayFields":["x_description","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"designation"};
frequisition_modulelist.Lists["x_designation"].Data = "<?php echo $requisition_module_list->designation->LookupFilterQuery(FALSE, "list") ?>";
frequisition_modulelist.AutoSuggests["x_designation"] = <?php echo json_encode(array("data" => "ajax=autosuggest&" . $requisition_module_list->designation->LookupFilterQuery(TRUE, "list"))) ?>;
frequisition_modulelist.Lists["x_department"] = {"LinkField":"x_department_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_department_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"depertment"};
frequisition_modulelist.Lists["x_department"].Data = "<?php echo $requisition_module_list->department->LookupFilterQuery(FALSE, "list") ?>";
frequisition_modulelist.AutoSuggests["x_department"] = <?php echo json_encode(array("data" => "ajax=autosuggest&" . $requisition_module_list->department->LookupFilterQuery(TRUE, "list"))) ?>;
frequisition_modulelist.Lists["x_status"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_description","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"status_ssf"};
frequisition_modulelist.Lists["x_status"].Data = "<?php echo $requisition_module_list->status->LookupFilterQuery(FALSE, "list") ?>";

// Form object for search
var CurrentSearchForm = frequisition_modulelistsrch = new ew_Form("frequisition_modulelistsrch");
</script>
<script type="text/javascript">
...
<?php if ($requisition_module->Export == "") { ?>
<div class="ewToolbar">
<?php if ($requisition_module_list->TotalRecs > 0 && $requisition_module_list->ExportOptions->Visible()) { ?>
<?php $requisition_module_list->ExportOptions->Render("body") ?>
<?php } ?>
<?php if ($requisition_module_list->SearchOptions->Visible()) { ?>
<?php $requisition_module_list->SearchOptions->Render("body") ?>
<?php } ?>
<?php if ($requisition_module_list->FilterOptions->Visible()) { ?>
<?php $requisition_module_list->FilterOptions->Render("body") ?>
<?php } ?>
<div class="clearfix"></div>
</div>
<?php } ?>
<?php
	$bSelectLimit = $requisition_module_list->UseSelectLimit;
	if ($bSelectLimit) {
		if ($requisition_module_list->TotalRecs <= 0)
			$requisition_module_list->TotalRecs = $requisition_module->ListRecordCount();
	} else {
		if (!$requisition_module_list->Recordset && ($requisition_module_list->Recordset = $requisition_module_list->LoadRecordset()))
			$requisition_module_list->TotalRecs = $requisition_module_list->Recordset->RecordCount();
	}
	$requisition_module_list->StartRec = 1;
	if ($requisition_module_list->DisplayRecs <= 0 || ($requisition_module->Export <> "" && $requisition_module->ExportAll)) // Display all records
		$requisition_module_list->DisplayRecs = $requisition_module_list->TotalRecs;
	if (!($requisition_module->Export <> "" && $requisition_module->ExportAll))
		$requisition_module_list->SetupStartRec(); // Set up start record position
	if ($bSelectLimit)
		$requisition_module_list->Recordset = $requisition_module_list->LoadRecordset($requisition_module_list->StartRec-1, $requisition_module_list->DisplayRecs);

	// Set no record found message
	if ($requisition_module->CurrentAction == "" && $requisition_module_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$requisition_module_list->setWarningMessage(ew_DeniedMsg());
		if ($requisition_module_list->SearchWhere == "0=101")
			$requisition_module_list->setWarningMessage($Language->Phrase("EnterSearchCriteria"));
		else
			$requisition_module_list->setWarningMessage($Language->Phrase("NoRecord"));
	}

	// Audit trail on search
	if ($requisition_module_list->AuditTrailOnSearch && $requisition_module_list->Command == "search" && !$requisition_module_list->RestoreSearch) {
		$searchparm = ew_ServerVar("QUERY_STRING");
		$searchsql = $requisition_module_list->getSessionWhere();
		$requisition_module_list->WriteAuditTrailOnSearch($searchparm, $searchsql);
	}
$requisition_module_list->RenderOtherOptions();
?>
<?php if ($Security->CanSearch()) { ?>
<?php if ($requisition_module->Export == "" && $requisition_module->CurrentAction == "") { ?>
<form name="frequisition_modulelistsrch" id="frequisition_modulelistsrch" class="form-inline ewForm ewExtSearchForm" action="<?php echo ew_CurrentPage() ?>">
<?php $SearchPanelClass = ($requisition_module_list->SearchWhere <> "") ? " in" : " in"; ?>
<div id="frequisition_modulelistsrch_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
<input type="hidden" name="cmd" value="search">
<input type="hidden" name="t" value="requisition_module">
	<div class="ewBasicSearch">
<div id="xsr_1" class="ewRow">
	<div class="ewQuickSearch input-group">
	<input type="text" name="<?php echo EW_TABLE_BASIC_SEARCH ?>" id="<?php echo EW_TABLE_BASIC_SEARCH ?>" class="form-control" value="<?php echo ew_HtmlEncode($requisition_module_list->BasicSearch->getKeyword()) ?>" placeholder="<?php echo ew_HtmlEncode($Language->Phrase("Search")) ?>">
	<input type="hidden" name="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" id="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" value="<?php echo ew_HtmlEncode($requisition_module_list->BasicSearch->getType()) ?>">
	<div class="input-group-btn">
		<button type="button" data-toggle="dropdown" class="btn btn-default"><span id="searchtype"><?php echo $requisition_module_list->BasicSearch->getTypeNameShort() ?></span><span class="caret"></span></button>
		<ul class="dropdown-menu pull-right" role="menu">
			<li<?php if ($requisition_module_list->BasicSearch->getType() == "") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this)"><?php echo $Language->Phrase("QuickSearchAuto") ?></a></li>
			<li<?php if ($requisition_module_list->BasicSearch->getType() == "=") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'=')"><?php echo $Language->Phrase("QuickSearchExact") ?></a></li>
			<li<?php if ($requisition_module_list->BasicSearch->getType() == "AND") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'AND')"><?php echo $Language->Phrase("QuickSearchAll") ?></a></li>
			<li<?php if ($requisition_module_list->BasicSearch->getType() == "OR") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'OR')"><?php echo $Language->Phrase("QuickSearchAny") ?></a></li>
		</ul>
...
<?php } ?>
<?php } ?>
<?php $requisition_module_list->ShowPageHeader(); ?>
<?php
$requisition_module_list->ShowMessage();
?>
<?php if ($requisition_module_list->TotalRecs > 0 || $requisition_module->CurrentAction <> "") { ?>
<div class="box ewBox ewGrid<?php if ($requisition_module_list->IsAddOrEdit()) { ?> ewGridAddEdit<?php } ?> requisition_module">
<?php if ($requisition_module->Export == "") { ?>
<div class="box-header ewGridUpperPanel">
<?php if ($requisition_module->CurrentAction <> "gridadd" && $requisition_module->CurrentAction <> "gridedit") { ?>
<form name="ewPagerForm" class="form-inline ewForm ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($requisition_module_list->Pager)) $requisition_module_list->Pager = new cPrevNextPager($requisition_module_list->StartRec, $requisition_module_list->DisplayRecs, $requisition_module_list->TotalRecs, $requisition_module_list->AutoHidePager) ?>
<?php if ($requisition_module_list->Pager->RecordCount > 0 && $requisition_module_list->Pager->Visible) { ?>
<div class="ewPager">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($requisition_module_list->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $requisition_module_list->PageUrl() ?>start=<?php echo $requisition_module_list->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($requisition_module_list->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $requisition_module_list->PageUrl() ?>start=<?php echo $requisition_module_list->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $requisition_module_list->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($requisition_module_list->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $requisition_module_list->PageUrl() ?>start=<?php echo $requisition_module_list->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($requisition_module_list->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $requisition_module_list->PageUrl() ?>start=<?php echo $requisition_module_list->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $requisition_module_list->Pager->PageCount ?></span>
</div>
<?php } ?>
<?php if ($requisition_module_list->Pager->RecordCount > 0) { ?>
<div class="ewPager ewRec">
	<span><?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $requisition_module_list->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $requisition_module_list->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $requisition_module_list->Pager->RecordCount ?></span>
</div>
<?php } ?>
<?php if ($requisition_module_list->TotalRecs > 0 && (!$requisition_module_list->AutoHidePageSizeSelector || $requisition_module_list->Pager->Visible)) { ?>
<div class="ewPager">
<input type="hidden" name="t" value="requisition_module">
<select name="<?php echo EW_TABLE_REC_PER_PAGE ?>" class="form-control input-sm ewTooltip" title="<?php echo $Language->Phrase("RecordsPerPage") ?>" onchange="this.form.submit();">
<option value="5"<?php if ($requisition_module_list->DisplayRecs == 5) { ?> selected<?php } ?>>5</option>
<option value="10"<?php if ($requisition_module_list->DisplayRecs == 10) { ?> selected<?php } ?>>10</option>
<option value="15"<?php if ($requisition_module_list->DisplayRecs == 15) { ?> selected<?php } ?>>15</option>
<option value="20"<?php if ($requisition_module_list->DisplayRecs == 20) { ?> selected<?php } ?>>20</option>
<option value="50"<?php if ($requisition_module_list->DisplayRecs == 50) { ?> selected<?php } ?>>50</option>
<option value="ALL"<?php if ($requisition_module->getRecordsPerPage() == -1) { ?> selected<?php } ?>><?php echo $Language->Phrase("AllRecords") ?></option>
</select>
</div>
<?php } ?>
</form>
...
<?php
	foreach ($requisition_module_list->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<div class="clearfix"></div>
...
<?php } ?>
<form name="frequisition_modulelist" id="frequisition_modulelist" class="form-inline ewForm ewListForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($requisition_module_list->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $requisition_module_list->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="requisition_module">
<div id="gmp_requisition_module" class="<?php if (ew_IsResponsiveLayout()) { ?>table-responsive <?php } ?>ewGridMiddlePanel">
<?php if ($requisition_module_list->TotalRecs > 0 || $requisition_module->CurrentAction == "gridedit") { ?>
<table id="tbl_requisition_modulelist" class="table ewTable">
<thead>
	<tr class="ewTableHeader">
<?php

// Header row
$requisition_module_list->RowType = EW_ROWTYPE_HEADER;

// Render list options
$requisition_module_list->RenderListOptions();

// Render list options (header, left)
$requisition_module_list->ListOptions->Render("header", "left");
?>
<?php if ($requisition_module->date->Visible) { // date ?>
	<?php if ($requisition_module->SortUrl($requisition_module->date) == "") { ?>
		<th data-name="date" class="<?php echo $requisition_module->date->HeaderCellClass() ?>"><div id="elh_requisition_module_date" class="requisition_module_date"><div class="ewTableHeaderCaption"><?php echo $requisition_module->date->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="date" class="<?php echo $requisition_module->date->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $requisition_module->SortUrl($requisition_module->date) ?>',1);"><div id="elh_requisition_module_date" class="requisition_module_date">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $requisition_module->date->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($requisition_module->date->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($requisition_module->date->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($requisition_module->reference->Visible) { // reference ?>
	<?php if ($requisition_module->SortUrl($requisition_module->reference) == "") { ?>
		<th data-name="reference" class="<?php echo $requisition_module->reference->HeaderCellClass() ?>"><div id="elh_requisition_module_reference" class="requisition_module_reference"><div class="ewTableHeaderCaption"><?php echo $requisition_module->reference->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="reference" class="<?php echo $requisition_module->reference->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $requisition_module->SortUrl($requisition_module->reference) ?>',1);"><div id="elh_requisition_module_reference" class="requisition_module_reference">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $requisition_module->reference->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($requisition_module->reference->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($requisition_module->reference->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($requisition_module->outward_location->Visible) { // outward_location ?>
	<?php if ($requisition_module->SortUrl($requisition_module->outward_location) == "") { ?>
		<th data-name="outward_location" class="<?php echo $requisition_module->outward_location->HeaderCellClass() ?>"><div id="elh_requisition_module_outward_location" class="requisition_module_outward_location"><div class="ewTableHeaderCaption"><?php echo $requisition_module->outward_location->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="outward_location" class="<?php echo $requisition_module->outward_location->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $requisition_module->SortUrl($requisition_module->outward_location) ?>',1);"><div id="elh_requisition_module_outward_location" class="requisition_module_outward_location">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $requisition_module->outward_location->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($requisition_module->outward_location->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($requisition_module->outward_location->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($requisition_module->delivery_point->Visible) { // delivery_point ?>
	<?php if ($requisition_module->SortUrl($requisition_module->delivery_point) == "") { ?>
		<th data-name="delivery_point" class="<?php echo $requisition_module->delivery_point->HeaderCellClass() ?>"><div id="elh_requisition_module_delivery_point" class="requisition_module_delivery_point"><div class="ewTableHeaderCaption"><?php echo $requisition_module->delivery_point->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="delivery_point" class="<?php echo $requisition_module->delivery_point->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $requisition_module->SortUrl($requisition_module->delivery_point) ?>',1);"><div id="elh_requisition_module_delivery_point" class="requisition_module_delivery_point">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $requisition_module->delivery_point->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($requisition_module->delivery_point->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($requisition_module->delivery_point->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($requisition_module->name->Visible) { // name ?>
	<?php if ($requisition_module->SortUrl($requisition_module->name) == "") { ?>
		<th data-name="name" class="<?php echo $requisition_module->name->HeaderCellClass() ?>"><div id="elh_requisition_module_name" class="requisition_module_name"><div class="ewTableHeaderCaption"><?php echo $requisition_module->name->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="name" class="<?php echo $requisition_module->name->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $requisition_module->SortUrl($requisition_module->name) ?>',1);"><div id="elh_requisition_module_name" class="requisition_module_name">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $requisition_module->name->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($requisition_module->name->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($requisition_module->name->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($requisition_module->organization->Visible) { // organization ?>
	<?php if ($requisition_module->SortUrl($requisition_module->organization) == "") { ?>
		<th data-name="organization" class="<?php echo $requisition_module->organization->HeaderCellClass() ?>"><div id="elh_requisition_module_organization" class="requisition_module_organization"><div class="ewTableHeaderCaption"><?php echo $requisition_module->organization->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="organization" class="<?php echo $requisition_module->organization->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $requisition_module->SortUrl($requisition_module->organization) ?>',1);"><div id="elh_requisition_module_organization" class="requisition_module_organization">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $requisition_module->organization->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($requisition_module->organization->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($requisition_module->organization->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($requisition_module->designation->Visible) { // designation ?>
	<?php if ($requisition_module->SortUrl($requisition_module->designation) == "") { ?>
		<th data-name="designation" class="<?php echo $requisition_module->designation->HeaderCellClass() ?>"><div id="elh_requisition_module_designation" class="requisition_module_designation"><div class="ewTableHeaderCaption"><?php echo $requisition_module->designation->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="designation" class="<?php echo $requisition_module->designation->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $requisition_module->SortUrl($requisition_module->designation) ?>',1);"><div id="elh_requisition_module_designation" class="requisition_module_designation">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $requisition_module->designation->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($requisition_module->designation->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($requisition_module->designation->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($requisition_module->department->Visible) { // department ?>
	<?php if ($requisition_module->SortUrl($requisition_module->department) == "") { ?>
		<th data-name="department" class="<?php echo $requisition_module->department->HeaderCellClass() ?>"><div id="elh_requisition_module_department" class="requisition_module_department"><div class="ewTableHeaderCaption"><?php echo $requisition_module->department->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="department" class="<?php echo $requisition_module->department->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $requisition_module->SortUrl($requisition_module->department) ?>',1);"><div id="elh_requisition_module_department" class="requisition_module_department">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $requisition_module->department->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($requisition_module->department->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($requisition_module->department->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($requisition_module->item_description->Visible) { // item_description ?>
	<?php if ($requisition_module->SortUrl($requisition_module->item_description) == "") { ?>
		<th data-name="item_description" class="<?php echo $requisition_module->item_description->HeaderCellClass() ?>"><div id="elh_requisition_module_item_description" class="requisition_module_item_description"><div class="ewTableHeaderCaption"><?php echo $requisition_module->item_description->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="item_description" class="<?php echo $requisition_module->item_description->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $requisition_module->SortUrl($requisition_module->item_description) ?>',1);"><div id="elh_requisition_module_item_description" class="requisition_module_item_description">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $requisition_module->item_description->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($requisition_module->item_description->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($requisition_module->item_description->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($requisition_module->driver_name->Visible) { // driver_name ?>
	<?php if ($requisition_module->SortUrl($requisition_module->driver_name) == "") { ?>
		<th data-name="driver_name" class="<?php echo $requisition_module->driver_name->HeaderCellClass() ?>"><div id="elh_requisition_module_driver_name" class="requisition_module_driver_name"><div class="ewTableHeaderCaption"><?php echo $requisition_module->driver_name->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="driver_name" class="<?php echo $requisition_module->driver_name->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $requisition_module->SortUrl($requisition_module->driver_name) ?>',1);"><div id="elh_requisition_module_driver_name" class="requisition_module_driver_name">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $requisition_module->driver_name->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($requisition_module->driver_name->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($requisition_module->driver_name->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($requisition_module->vehicle_no->Visible) { // vehicle_no ?>
	<?php if ($requisition_module->SortUrl($requisition_module->vehicle_no) == "") { ?>
		<th data-name="vehicle_no" class="<?php echo $requisition_module->vehicle_no->HeaderCellClass() ?>"><div id="elh_requisition_module_vehicle_no" class="requisition_module_vehicle_no"><div class="ewTableHeaderCaption"><?php echo $requisition_module->vehicle_no->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="vehicle_no" class="<?php echo $requisition_module->vehicle_no->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $requisition_module->SortUrl($requisition_module->vehicle_no) ?>',1);"><div id="elh_requisition_module_vehicle_no" class="requisition_module_vehicle_no">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $requisition_module->vehicle_no->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($requisition_module->vehicle_no->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($requisition_module->vehicle_no->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($requisition_module->status->Visible) { // status ?>
	<?php if ($requisition_module->SortUrl($requisition_module->status) == "") { ?>
		<th data-name="status" class="<?php echo $requisition_module->status->HeaderCellClass() ?>"><div id="elh_requisition_module_status" class="requisition_module_status"><div class="ewTableHeaderCaption"><?php echo $requisition_module->status->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="status" class="<?php echo $requisition_module->status->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $requisition_module->SortUrl($requisition_module->status) ?>',1);"><div id="elh_requisition_module_status" class="requisition_module_status">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $requisition_module->status->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($requisition_module->status->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($requisition_module->status->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php
...
$requisition_module_list->ListOptions->Render("header", "right");
?>
	</tr>
</thead>
<tbody>
<?php
if ($requisition_module->ExportAll && $requisition_module->Export <> "") {
	$requisition_module_list->StopRec = $requisition_module_list->TotalRecs;
} else {

	// Set the last record to display
	if ($requisition_module_list->TotalRecs > $requisition_module_list->StartRec + $requisition_module_list->DisplayRecs - 1)
		$requisition_module_list->StopRec = $requisition_module_list->StartRec + $requisition_module_list->DisplayRecs - 1;
	else
		$requisition_module_list->StopRec = $requisition_module_list->TotalRecs;
}
$requisition_module_list->RecCnt = $requisition_module_list->StartRec - 1;
if ($requisition_module_list->Recordset && !$requisition_module_list->Recordset->EOF) {
	$requisition_module_list->Recordset->MoveFirst();
	$bSelectLimit = $requisition_module_list->UseSelectLimit;
	if (!$bSelectLimit && $requisition_module_list->StartRec > 1)
		$requisition_module_list->Recordset->Move($requisition_module_list->StartRec - 1);
} elseif (!$requisition_module->AllowAddDeleteRow && $requisition_module_list->StopRec == 0) {
	$requisition_module_list->StopRec = $requisition_module->GridAddRowCount;
}

// Initialize aggregate
$requisition_module->RowType = EW_ROWTYPE_AGGREGATEINIT;
$requisition_module->ResetAttrs();
$requisition_module_list->RenderRow();
while ($requisition_module_list->RecCnt < $requisition_module_list->StopRec) {
	$requisition_module_list->RecCnt++;
	if (intval($requisition_module_list->RecCnt) >= intval($requisition_module_list->StartRec)) {
		$requisition_module_list->RowCnt++;

		// Set up key count
		$requisition_module_list->KeyCount = $requisition_module_list->RowIndex;

		// Init row class and style
		$requisition_module->ResetAttrs();
		$requisition_module->CssClass = "";
		if ($requisition_module->CurrentAction == "gridadd") {
		} else {
			$requisition_module_list->LoadRowValues($requisition_module_list->Recordset); // Load row values
		}
		$requisition_module->RowType = EW_ROWTYPE_VIEW; // Render view

		// Set up row id / data-rowindex
		$requisition_module->RowAttrs = array_merge($requisition_module->RowAttrs, array('data-rowindex'=>$requisition_module_list->RowCnt, 'id'=>'r' . $requisition_module_list->RowCnt . '_requisition_module', 'data-rowtype'=>$requisition_module->RowType));
		...
		$requisition_module_list->RenderListOptions();
?>
	<tr<?php echo $requisition_module->RowAttributes() ?>>
<?php

// Render list options (body, left)
$requisition_module_list->ListOptions->Render("body", "left", $requisition_module_list->RowCnt);
?>
	<?php if ($requisition_module->date->Visible) { // date ?>
		<td data-name="date"<?php echo $requisition_module->date->CellAttributes() ?>>
<span id="el<?php echo $requisition_module_list->RowCnt ?>_requisition_module_date" class="requisition_module_date">
<span<?php echo $requisition_module->date->ViewAttributes() ?>>
<?php echo $requisition_module->date->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($requisition_module->reference->Visible) { // reference ?>
		<td data-name="reference"<?php echo $requisition_module->reference->CellAttributes() ?>>
<span id="el<?php echo $requisition_module_list->RowCnt ?>_requisition_module_reference" class="requisition_module_reference">
<span<?php echo $requisition_module->reference->ViewAttributes() ?>>
<?php echo $requisition_module->reference->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($requisition_module->outward_location->Visible) { // outward_location ?>
		<td data-name="outward_location"<?php echo $requisition_module->outward_location->CellAttributes() ?>>
<span id="el<?php echo $requisition_module_list->RowCnt ?>_requisition_module_outward_location" class="requisition_module_outward_location">
<span<?php echo $requisition_module->outward_location->ViewAttributes() ?>>
<?php echo $requisition_module->outward_location->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($requisition_module->delivery_point->Visible) { // delivery_point ?>
		<td data-name="delivery_point"<?php echo $requisition_module->delivery_point->CellAttributes() ?>>
<span id="el<?php echo $requisition_module_list->RowCnt ?>_requisition_module_delivery_point" class="requisition_module_delivery_point">
<span<?php echo $requisition_module->delivery_point->ViewAttributes() ?>>
<?php echo $requisition_module->delivery_point->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($requisition_module->name->Visible) { // name ?>
		<td data-name="name"<?php echo $requisition_module->name->CellAttributes() ?>>
<span id="el<?php echo $requisition_module_list->RowCnt ?>_requisition_module_name" class="requisition_module_name">
<span<?php echo $requisition_module->name->ViewAttributes() ?>>
<?php echo $requisition_module->name->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($requisition_module->organization->Visible) { // organization ?>
		<td data-name="organization"<?php echo $requisition_module->organization->CellAttributes() ?>>
<span id="el<?php echo $requisition_module_list->RowCnt ?>_requisition_module_organization" class="requisition_module_organization">
<span<?php echo $requisition_module->organization->ViewAttributes() ?>>
<?php echo $requisition_module->organization->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($requisition_module->designation->Visible) { // designation ?>
		<td data-name="designation"<?php echo $requisition_module->designation->CellAttributes() ?>>
<span id="el<?php echo $requisition_module_list->RowCnt ?>_requisition_module_designation" class="requisition_module_designation">
<span<?php echo $requisition_module->designation->ViewAttributes() ?>>
<?php echo $requisition_module->designation->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($requisition_module->department->Visible) { // department ?>
		<td data-name="department"<?php echo $requisition_module->department->CellAttributes() ?>>
<span id="el<?php echo $requisition_module_list->RowCnt ?>_requisition_module_department" class="requisition_module_department">
<span<?php echo $requisition_module->department->ViewAttributes() ?>>
<?php echo $requisition_module->department->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($requisition_module->item_description->Visible) { // item_description ?>
		<td data-name="item_description"<?php echo $requisition_module->item_description->CellAttributes() ?>>
<span id="el<?php echo $requisition_module_list->RowCnt ?>_requisition_module_item_description" class="requisition_module_item_description">
<span<?php echo $requisition_module->item_description->ViewAttributes() ?>>
<?php echo $requisition_module->item_description->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($requisition_module->driver_name->Visible) { // driver_name ?>
		<td data-name="driver_name"<?php echo $requisition_module->driver_name->CellAttributes() ?>>
<span id="el<?php echo $requisition_module_list->RowCnt ?>_requisition_module_driver_name" class="requisition_module_driver_name">
<span<?php echo $requisition_module->driver_name->ViewAttributes() ?>>
<?php echo $requisition_module->driver_name->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($requisition_module->vehicle_no->Visible) { // vehicle_no ?>
		<td data-name="vehicle_no"<?php echo $requisition_module->vehicle_no->CellAttributes() ?>>
<span id="el<?php echo $requisition_module_list->RowCnt ?>_requisition_module_vehicle_no" class="requisition_module_vehicle_no">
<span<?php echo $requisition_module->vehicle_no->ViewAttributes() ?>>
<?php echo $requisition_module->vehicle_no->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($requisition_module->status->Visible) { // status ?>
		<td data-name="status"<?php echo $requisition_module->status->CellAttributes() ?>>
<span id="el<?php echo $requisition_module_list->RowCnt ?>_requisition_module_status" class="requisition_module_status">
<span<?php echo $requisition_module->status->ViewAttributes() ?>>
<?php echo $requisition_module->status->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
<?php
...
$requisition_module_list->ListOptions->Render("body", "right", $requisition_module_list->RowCnt);
?>
	</tr>
<?php
	}
	if ($requisition_module->CurrentAction <> "gridadd")
		$requisition_module_list->Recordset->MoveNext();
}
?>
</tbody>
</table>
<?php } ?>
<?php if ($requisition_module->CurrentAction == "") { ?>
...
// Close recordset
if ($requisition_module_list->Recordset)
	$requisition_module_list->Recordset->Close();
?>
</div>
<?php } ?>
<?php if ($requisition_module_list->TotalRecs == 0 && $requisition_module->CurrentAction == "") { // Show other options ?>
<div class="ewListOtherOptions">
<?php
	foreach ($requisition_module_list->OtherOptions as &$option) {
		$option->ButtonClass = "";
...
<?php if ($requisition_module->Export == "") { ?>
<script type="text/javascript">
frequisition_modulelistsrch.FilterList = <?php echo $requisition_module_list->GetFilterList() ?>;
frequisition_modulelistsrch.Init();
frequisition_modulelist.Init();
</script>
<?php } ?>
<?php
$requisition_module_list->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($requisition_module->Export == "") { ?>
...
<?php
$requisition_module_list->Page_Terminate();
?>
