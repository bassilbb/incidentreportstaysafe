...
$loan_application_edit = NULL; // Initialize page object first

class cloan_application_edit extends cloan_application {
	...
	// Page object name
	var $PageObjName = 'loan_application_edit';
	...
	function LoadFormValues() {
		...
		if (!$this->correction_date->FldIsDetailKey) {
			$this->correction_date->setFormValue($objForm->GetValue("x_correction_date"));
			$this->correction_date->CurrentValue = ew_UnFormatDateTime($this->correction_date->CurrentValue, 0);
		}
		if (!$this->correction_action->FldIsDetailKey) {
			$this->correction_action->setFormValue($objForm->GetValue("x_correction_action"));
		}
		if (!$this->correction_comment->FldIsDetailKey) {
			$this->correction_comment->setFormValue($objForm->GetValue("x_correction_comment"));
		}
		if (!$this->corrected_by->FldIsDetailKey) {
			$this->corrected_by->setFormValue($objForm->GetValue("x_corrected_by"));
		}
	...
	function RestoreFormValues() {
		...
		$this->correction_date->CurrentValue = ew_UnFormatDateTime($this->correction_date->CurrentValue, 0);
		$this->correction_action->CurrentValue = $this->correction_action->FormValue;
		$this->correction_comment->CurrentValue = $this->correction_comment->FormValue;
		$this->corrected_by->CurrentValue = $this->corrected_by->FormValue;
	}
	...
	function RenderRow() {
		...
		$this->corrected_by->ViewCustomAttributes = "";
			...
			if ($this->CurrentAction == "I" && !$this->EventCancelled) ew_RenderUploadField($this->applicant_passport);
			...
			if ($this->CurrentAction == "I" && !$this->EventCancelled) ew_RenderUploadField($this->guarantor_passport);
			...
			$this->correction_date->EditValue = ew_HtmlEncode(ew_FormatDateTime($this->correction_date->CurrentValue, 8));
			...
			$this->correction_comment->EditValue = ew_HtmlEncode($this->correction_comment->CurrentValue);
			...
			$this->corrected_by->EditValue = ew_HtmlEncode($this->corrected_by->CurrentValue);
			if (strval($this->corrected_by->CurrentValue) <> "") {
				$sFilterWrk = "`id`" . ew_SearchString("=", $this->corrected_by->CurrentValue, EW_DATATYPE_NUMBER, "");
			$sSqlWrk = "SELECT `id`, `firstname` AS `DispFld`, `lastname` AS `Disp2Fld`, '' AS `Disp3Fld`, '' AS `Disp4Fld` FROM `users`";
			$sWhereWrk = "";
			...
			if ($sWhereWrk <> "") $sSqlWrk .= " WHERE " . $sWhereWrk;
				...
				if ($rswrk && !$rswrk->EOF) { // Lookup values found
					...
					$arwrk[1] = ew_HtmlEncode($rswrk->fields('DispFld'));
					$arwrk[2] = ew_HtmlEncode($rswrk->fields('Disp2Fld'));
					$this->corrected_by->EditValue = $this->corrected_by->DisplayValue($arwrk);
					$rswrk->Close();
				} else {
					$this->corrected_by->EditValue = ew_HtmlEncode($this->corrected_by->CurrentValue);
				}
			} else {
				$this->corrected_by->EditValue = NULL;
			}
	...
	function ValidateForm() {
		...
		if (!$this->correction_comment->FldIsDetailKey && !is_null($this->correction_comment->FormValue) && $this->correction_comment->FormValue == "") {
			ew_AddMessage($gsFormError, str_replace("%s", $this->correction_comment->FldCaption(), $this->correction_comment->ReqErrMsg));
		}
		if (!$this->corrected_by->FldIsDetailKey && !is_null($this->corrected_by->FormValue) && $this->corrected_by->FormValue == "") {
			ew_AddMessage($gsFormError, str_replace("%s", $this->corrected_by->FldCaption(), $this->corrected_by->ReqErrMsg));
		}
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// refernce_id
			$this->refernce_id->SetDbValueDef($rsnew, $this->refernce_id->CurrentValue, NULL, $this->refernce_id->ReadOnly);

			// employee_name
			$this->employee_name->SetDbValueDef($rsnew, $this->employee_name->CurrentValue, NULL, $this->employee_name->ReadOnly);
			...
			// pension
			$this->pension->SetDbValueDef($rsnew, $this->pension->CurrentValue, NULL, $this->pension->ReadOnly);

			// loan_amount
			$this->loan_amount->SetDbValueDef($rsnew, $this->loan_amount->CurrentValue, NULL, $this->loan_amount->ReadOnly);

			// amount_inwords
			$this->amount_inwords->SetDbValueDef($rsnew, $this->amount_inwords->CurrentValue, NULL, $this->amount_inwords->ReadOnly);

			// purpose
			$this->purpose->SetDbValueDef($rsnew, $this->purpose->CurrentValue, NULL, $this->purpose->ReadOnly);

			// repayment_period
			$this->repayment_period->SetDbValueDef($rsnew, $this->repayment_period->CurrentValue, NULL, $this->repayment_period->ReadOnly);

			// salary_permonth
			$this->salary_permonth->SetDbValueDef($rsnew, $this->salary_permonth->CurrentValue, NULL, $this->salary_permonth->ReadOnly);

			// previous_loan
			$this->previous_loan->SetDbValueDef($rsnew, $this->previous_loan->CurrentValue, NULL, $this->previous_loan->ReadOnly);

			// date_collected
			$this->date_collected->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->date_collected->CurrentValue, 0), NULL, $this->date_collected->ReadOnly);

			// date_liquidated
			$this->date_liquidated->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->date_liquidated->CurrentValue, 0), NULL, $this->date_liquidated->ReadOnly);
			...
			if ($this->applicant_passport->Visible && !$this->applicant_passport->ReadOnly && !$this->applicant_passport->Upload->KeepFile) {
				$this->applicant_passport->Upload->DbValue = $rsold['applicant_passport']; // Get original value
			...
			// guarantor_name
			$this->guarantor_name->SetDbValueDef($rsnew, $this->guarantor_name->CurrentValue, NULL, $this->guarantor_name->ReadOnly);

			// guarantor_address
			$this->guarantor_address->SetDbValueDef($rsnew, $this->guarantor_address->CurrentValue, NULL, $this->guarantor_address->ReadOnly);

			// guarantor_mobile
			$this->guarantor_mobile->SetDbValueDef($rsnew, $this->guarantor_mobile->CurrentValue, NULL, $this->guarantor_mobile->ReadOnly);

			// guarantor_department
			$this->guarantor_department->SetDbValueDef($rsnew, $this->guarantor_department->CurrentValue, NULL, $this->guarantor_department->ReadOnly);
			...
			// employers_name
			$this->employers_name->SetDbValueDef($rsnew, $this->employers_name->CurrentValue, NULL, $this->employers_name->ReadOnly);

			// employers_address
			$this->employers_address->SetDbValueDef($rsnew, $this->employers_address->CurrentValue, NULL, $this->employers_address->ReadOnly);
			...
			if ($this->guarantor_passport->Visible && !$this->guarantor_passport->ReadOnly && !$this->guarantor_passport->Upload->KeepFile) {
				$this->guarantor_passport->Upload->DbValue = $rsold['guarantor_passport']; // Get original value
			...
			// recommended_date
			$this->recommended_date->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->recommended_date->CurrentValue, 14), NULL, $this->recommended_date->ReadOnly);

			// document_checklist
			$this->document_checklist->SetDbValueDef($rsnew, $this->document_checklist->CurrentValue, NULL, $this->document_checklist->ReadOnly);

			// recommender_action
			$this->recommender_action->SetDbValueDef($rsnew, $this->recommender_action->CurrentValue, NULL, $this->recommender_action->ReadOnly);

			// recommender_comment
			$this->recommender_comment->SetDbValueDef($rsnew, $this->recommender_comment->CurrentValue, NULL, $this->recommender_comment->ReadOnly);

			// recommended_by
			$this->recommended_by->SetDbValueDef($rsnew, $this->recommended_by->CurrentValue, NULL, $this->recommended_by->ReadOnly);

			// application_status
			$this->application_status->SetDbValueDef($rsnew, $this->application_status->CurrentValue, NULL, $this->application_status->ReadOnly);

			// approved_amount
			$this->approved_amount->SetDbValueDef($rsnew, $this->approved_amount->CurrentValue, NULL, $this->approved_amount->ReadOnly);

			// duration_approved
			$this->duration_approved->SetDbValueDef($rsnew, $this->duration_approved->CurrentValue, NULL, $this->duration_approved->ReadOnly);

			// approval_date
			$this->approval_date->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->approval_date->CurrentValue, 17), NULL, $this->approval_date->ReadOnly);
			...
			// correction_date
			$this->correction_date->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->correction_date->CurrentValue, 0), NULL, $this->correction_date->ReadOnly);

			// correction_action
			$this->correction_action->SetDbValueDef($rsnew, $this->correction_action->CurrentValue, NULL, $this->correction_action->ReadOnly);

			// correction_comment
			$this->correction_comment->SetDbValueDef($rsnew, $this->correction_comment->CurrentValue, NULL, $this->correction_comment->ReadOnly);

			// corrected_by
			$this->corrected_by->SetDbValueDef($rsnew, $this->corrected_by->CurrentValue, NULL, $this->corrected_by->ReadOnly);
			if ($this->applicant_passport->Visible && !$this->applicant_passport->Upload->KeepFile) {
				$OldFiles = ew_Empty($this->applicant_passport->Upload->DbValue) ? array() : explode(EW_MULTIPLE_UPLOAD_SEPARATOR, strval($this->applicant_passport->Upload->DbValue));
				if (!ew_Empty($this->applicant_passport->Upload->FileName)) {
					$NewFiles = explode(EW_MULTIPLE_UPLOAD_SEPARATOR, strval($this->applicant_passport->Upload->FileName));
					...
					$this->applicant_passport->SetDbValueDef($rsnew, $this->applicant_passport->Upload->FileName, NULL, $this->applicant_passport->ReadOnly);
				}
			}
			if ($this->guarantor_passport->Visible && !$this->guarantor_passport->Upload->KeepFile) {
				$OldFiles = ew_Empty($this->guarantor_passport->Upload->DbValue) ? array() : explode(EW_MULTIPLE_UPLOAD_SEPARATOR, strval($this->guarantor_passport->Upload->DbValue));
				if (!ew_Empty($this->guarantor_passport->Upload->FileName)) {
					$NewFiles = explode(EW_MULTIPLE_UPLOAD_SEPARATOR, strval($this->guarantor_passport->Upload->FileName));
					$NewFileCount = count($NewFiles);
					for ($i = 0; $i < $NewFileCount; $i++) {
					...
					$this->guarantor_passport->SetDbValueDef($rsnew, $this->guarantor_passport->Upload->FileName, NULL, $this->guarantor_passport->ReadOnly);
				}
			}
			...
			if ($bUpdateRow) {
				...
				if ($EditRow) {
					if ($this->applicant_passport->Visible && !$this->applicant_passport->Upload->KeepFile) {
						$OldFiles = ew_Empty($this->applicant_passport->Upload->DbValue) ? array() : explode(EW_MULTIPLE_UPLOAD_SEPARATOR, strval($this->applicant_passport->Upload->DbValue));
						if (!ew_Empty($this->applicant_passport->Upload->FileName)) {
							$NewFiles = explode(EW_MULTIPLE_UPLOAD_SEPARATOR, $this->applicant_passport->Upload->FileName);
							$NewFiles2 = explode(EW_MULTIPLE_UPLOAD_SEPARATOR, $rsnew['applicant_passport']);
							$NewFileCount = count($NewFiles);
							for ($i = 0; $i < $NewFileCount; $i++) {
								$fldvar = ($this->applicant_passport->Upload->Index < 0) ? $this->applicant_passport->FldVar : substr($this->applicant_passport->FldVar, 0, 1) . $this->applicant_passport->Upload->Index . substr($this->applicant_passport->FldVar, 1);
								if ($NewFiles[$i] <> "") {
					...
					if ($this->guarantor_passport->Visible && !$this->guarantor_passport->Upload->KeepFile) {
						$OldFiles = ew_Empty($this->guarantor_passport->Upload->DbValue) ? array() : explode(EW_MULTIPLE_UPLOAD_SEPARATOR, strval($this->guarantor_passport->Upload->DbValue));
						if (!ew_Empty($this->guarantor_passport->Upload->FileName)) {
							$NewFiles = explode(EW_MULTIPLE_UPLOAD_SEPARATOR, $this->guarantor_passport->Upload->FileName);
							$NewFiles2 = explode(EW_MULTIPLE_UPLOAD_SEPARATOR, $rsnew['guarantor_passport']);
							$NewFileCount = count($NewFiles);
							for ($i = 0; $i < $NewFileCount; $i++) {
								$fldvar = ($this->guarantor_passport->Upload->Index < 0) ? $this->guarantor_passport->FldVar : substr($this->guarantor_passport->FldVar, 0, 1) . $this->guarantor_passport->Upload->Index . substr($this->guarantor_passport->FldVar, 1);
								if ($NewFiles[$i] <> "") {
	...
	function SetupAutoSuggestFilters($fld, $pageId = null) {
		...
		case "x_corrected_by":
			$sSqlWrk = "";
			$sSqlWrk = "SELECT `id`, `firstname` AS `DispFld`, `lastname` AS `Disp2Fld` FROM `users`";
			$sWhereWrk = "`firstname` LIKE '{query_value}%' OR CONCAT(COALESCE(`firstname`, ''),'" . ew_ValueSeparator(1, $this->corrected_by) . "',COALESCE(`lastname`,'')) LIKE '{query_value}%'";
			$fld->LookupFilters = array();
			$fld->LookupFilters += array("s" => $sSqlWrk, "d" => "");
			$sSqlWrk = "";
			$this->Lookup_Selecting($this->corrected_by, $sWhereWrk); // Call Lookup Selecting
...
// Create page object
if (!isset($loan_application_edit)) $loan_application_edit = new cloan_application_edit();

// Page init
$loan_application_edit->Page_Init();

// Page main
$loan_application_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$loan_application_edit->Page_Render();
?>
...
var CurrentForm = floan_applicationedit = new ew_Form("floan_applicationedit", "edit");

// Validate form
floan_applicationedit.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			elm = this.GetElements("x" + infix + "_correction_comment");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $loan_application->correction_comment->FldCaption(), $loan_application->correction_comment->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_corrected_by");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $loan_application->corrected_by->FldCaption(), $loan_application->corrected_by->ReqErrMsg)) ?>");
...
floan_applicationedit.Form_CustomValidate = 
...
floan_applicationedit.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;
...
floan_applicationedit.Lists["x_repayment_period"].Data = "<?php echo $loan_application_edit->repayment_period->LookupFilterQuery(FALSE, "edit") ?>";
floan_applicationedit.Lists["x_guarantor_department"] = {"LinkField":"x_department_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_department_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"depertment"};
floan_applicationedit.Lists["x_guarantor_department"].Data = "<?php echo $loan_application_edit->guarantor_department->LookupFilterQuery(FALSE, "edit") ?>";
floan_applicationedit.Lists["x_bank_name"] = {"LinkField":"x_code","Ajax":true,"AutoFill":false,"DisplayFields":["x_description","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"banks_list"};
floan_applicationedit.Lists["x_bank_name"].Data = "<?php echo $loan_application_edit->bank_name->LookupFilterQuery(FALSE, "edit") ?>";
floan_applicationedit.Lists["x_status"] = {"LinkField":"x_code","Ajax":true,"AutoFill":false,"DisplayFields":["x_description","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"loan_status"};
floan_applicationedit.Lists["x_status"].Data = "<?php echo $loan_application_edit->status->LookupFilterQuery(FALSE, "edit") ?>";
floan_applicationedit.AutoSuggests["x_status"] = <?php echo json_encode(array("data" => "ajax=autosuggest&" . $loan_application_edit->status->LookupFilterQuery(TRUE, "edit"))) ?>;
floan_applicationedit.Lists["x_initiator_action"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
floan_applicationedit.Lists["x_initiator_action"].Options = <?php echo json_encode($loan_application_edit->initiator_action->Options()) ?>;
floan_applicationedit.Lists["x_document_checklist[]"] = {"LinkField":"x_code","Ajax":true,"AutoFill":false,"DisplayFields":["x_discription","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"document_checklist"};
floan_applicationedit.Lists["x_document_checklist[]"].Data = "<?php echo $loan_application_edit->document_checklist->LookupFilterQuery(FALSE, "edit") ?>";
floan_applicationedit.Lists["x_recommender_action"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
floan_applicationedit.Lists["x_recommender_action"].Options = <?php echo json_encode($loan_application_edit->recommender_action->Options()) ?>;
floan_applicationedit.Lists["x_recommended_by"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_firstname","x_lastname","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"users"};
floan_applicationedit.Lists["x_recommended_by"].Data = "<?php echo $loan_application_edit->recommended_by->LookupFilterQuery(FALSE, "edit") ?>";
floan_applicationedit.AutoSuggests["x_recommended_by"] = <?php echo json_encode(array("data" => "ajax=autosuggest&" . $loan_application_edit->recommended_by->LookupFilterQuery(TRUE, "edit"))) ?>;
floan_applicationedit.Lists["x_application_status"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
floan_applicationedit.Lists["x_application_status"].Options = <?php echo json_encode($loan_application_edit->application_status->Options()) ?>;
floan_applicationedit.Lists["x_duration_approved"] = {"LinkField":"x_code","Ajax":true,"AutoFill":false,"DisplayFields":["x_description","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"duration_months"};
floan_applicationedit.Lists["x_duration_approved"].Data = "<?php echo $loan_application_edit->duration_approved->LookupFilterQuery(FALSE, "edit") ?>";
floan_applicationedit.Lists["x_approval_action"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
floan_applicationedit.Lists["x_approval_action"].Options = <?php echo json_encode($loan_application_edit->approval_action->Options()) ?>;
floan_applicationedit.Lists["x_approved_by"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_firstname","x_lastname","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"users"};
floan_applicationedit.Lists["x_approved_by"].Data = "<?php echo $loan_application_edit->approved_by->LookupFilterQuery(FALSE, "edit") ?>";
floan_applicationedit.Lists["x_correction_action"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
floan_applicationedit.Lists["x_correction_action"].Options = <?php echo json_encode($loan_application_edit->correction_action->Options()) ?>;
floan_applicationedit.Lists["x_corrected_by"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_firstname","x_lastname","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"users"};
floan_applicationedit.Lists["x_corrected_by"].Data = "<?php echo $loan_application_edit->corrected_by->LookupFilterQuery(FALSE, "edit") ?>";
floan_applicationedit.AutoSuggests["x_corrected_by"] = <?php echo json_encode(array("data" => "ajax=autosuggest&" . $loan_application_edit->corrected_by->LookupFilterQuery(TRUE, "edit"))) ?>;
...
$('#x_recommended_by').attr('readonly',true);
$('#x_status').attr('readonly',true);
$('#x_recommended_date').attr('readonly',true);
$('#x_approval_date').attr('readonly',true);
</script>
<?php $loan_application_edit->ShowPageHeader(); ?>
<?php
$loan_application_edit->ShowMessage();
?>
<?php if (!$loan_application_edit->IsModal) { ?>
<?php if ($loan_application->CurrentAction <> "F") { // Confirm page ?>
<form name="ewPagerForm" class="form-horizontal ewForm ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($loan_application_edit->Pager)) $loan_application_edit->Pager = new cPrevNextPager($loan_application_edit->StartRec, $loan_application_edit->DisplayRecs, $loan_application_edit->TotalRecs, $loan_application_edit->AutoHidePager) ?>
<?php if ($loan_application_edit->Pager->RecordCount > 0 && $loan_application_edit->Pager->Visible) { ?>
<div class="ewPager">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($loan_application_edit->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $loan_application_edit->PageUrl() ?>start=<?php echo $loan_application_edit->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($loan_application_edit->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $loan_application_edit->PageUrl() ?>start=<?php echo $loan_application_edit->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $loan_application_edit->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($loan_application_edit->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $loan_application_edit->PageUrl() ?>start=<?php echo $loan_application_edit->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($loan_application_edit->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $loan_application_edit->PageUrl() ?>start=<?php echo $loan_application_edit->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $loan_application_edit->Pager->PageCount ?></span>
</div>
<?php } ?>
...
<?php } ?>
<?php } ?>
<form name="floan_applicationedit" id="floan_applicationedit" class="<?php echo $loan_application_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($loan_application_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $loan_application_edit->Token ?>">
<?php } ?>
...
<?php } ?>
<input type="hidden" name="modal" value="<?php echo intval($loan_application_edit->IsModal) ?>">
<div class="ewMultiPage"><!-- multi-page -->
<div class="nav-tabs-custom" id="loan_application_edit"><!-- multi-page .nav-tabs-custom -->
	<ul class="nav<?php echo $loan_application_edit->MultiPages->NavStyle() ?>">
		<li<?php echo $loan_application_edit->MultiPages->TabStyle("1") ?>><a href="#tab_loan_application1" data-toggle="tab"><?php echo $loan_application->PageCaption(1) ?></a></li>
		<li<?php echo $loan_application_edit->MultiPages->TabStyle("2") ?>><a href="#tab_loan_application2" data-toggle="tab"><?php echo $loan_application->PageCaption(2) ?></a></li>
		<li<?php echo $loan_application_edit->MultiPages->TabStyle("3") ?>><a href="#tab_loan_application3" data-toggle="tab"><?php echo $loan_application->PageCaption(3) ?></a></li>
	</ul>
	<div class="tab-content"><!-- multi-page .nav-tabs-custom .tab-content -->
		<div class="tab-pane<?php echo $loan_application_edit->MultiPages->PageStyle("1") ?>" id="tab_loan_application1"><!-- multi-page .tab-pane -->
...
<p class="form-control-static"><?php echo $loan_application->date_initiated->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="loan_application" data-field="x_date_initiated" data-page="1" name="x_date_initiated" id="x_date_initiated" value="<?php echo ew_HtmlEncode($loan_application->date_initiated->FormValue) ?>">
<?php } ?>
...
<p class="form-control-static"><?php echo $loan_application->refernce_id->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="loan_application" data-field="x_refernce_id" data-page="1" name="x_refernce_id" id="x_refernce_id" value="<?php echo ew_HtmlEncode($loan_application->refernce_id->FormValue) ?>">
<?php } ?>
...
<p class="form-control-static"><?php echo $loan_application->employee_name->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="loan_application" data-field="x_employee_name" data-page="1" name="x_employee_name" id="x_employee_name" value="<?php echo ew_HtmlEncode($loan_application->employee_name->FormValue) ?>">
<?php } ?>
...
<p class="form-control-static"><?php echo $loan_application->address->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="loan_application" data-field="x_address" data-page="1" name="x_address" id="x_address" value="<?php echo ew_HtmlEncode($loan_application->address->FormValue) ?>">
<?php } ?>
...
<p class="form-control-static"><?php echo $loan_application->mobile->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="loan_application" data-field="x_mobile" data-page="1" name="x_mobile" id="x_mobile" value="<?php echo ew_HtmlEncode($loan_application->mobile->FormValue) ?>">
<?php } ?>
...
<p class="form-control-static"><?php echo $loan_application->department->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="loan_application" data-field="x_department" data-page="1" name="x_department" id="x_department" value="<?php echo ew_HtmlEncode($loan_application->department->FormValue) ?>">
<?php } ?>
...
<?php if ($loan_application->pension->Visible) { // pension ?>
	<div id="r_pension" class="form-group">
		<label id="elh_loan_application_pension" for="x_pension" class="<?php echo $loan_application_edit->LeftColumnClass ?>"><?php echo $loan_application->pension->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $loan_application_edit->RightColumnClass ?>"><div<?php echo $loan_application->pension->CellAttributes() ?>>
<?php if ($loan_application->CurrentAction <> "F") { ?>
...
<p class="form-control-static"><?php echo $loan_application->pension->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="loan_application" data-field="x_pension" data-page="1" name="x_pension" id="x_pension" value="<?php echo ew_HtmlEncode($loan_application->pension->FormValue) ?>">
<?php } ?>
...
<p class="form-control-static"><?php echo $loan_application->loan_amount->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="loan_application" data-field="x_loan_amount" data-page="1" name="x_loan_amount" id="x_loan_amount" value="<?php echo ew_HtmlEncode($loan_application->loan_amount->FormValue) ?>">
<?php } ?>
...
<p class="form-control-static"><?php echo $loan_application->amount_inwords->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="loan_application" data-field="x_amount_inwords" data-page="1" name="x_amount_inwords" id="x_amount_inwords" value="<?php echo ew_HtmlEncode($loan_application->amount_inwords->FormValue) ?>">
<?php } ?>
...
<p class="form-control-static"><?php echo $loan_application->purpose->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="loan_application" data-field="x_purpose" data-page="1" name="x_purpose" id="x_purpose" value="<?php echo ew_HtmlEncode($loan_application->purpose->FormValue) ?>">
<?php } ?>
...
<?php if ($loan_application->repayment_period->Visible) { // repayment_period ?>
	<div id="r_repayment_period" class="form-group">
		<label id="elh_loan_application_repayment_period" for="x_repayment_period" class="<?php echo $loan_application_edit->LeftColumnClass ?>"><?php echo $loan_application->repayment_period->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $loan_application_edit->RightColumnClass ?>"><div<?php echo $loan_application->repayment_period->CellAttributes() ?>>
<?php if ($loan_application->CurrentAction <> "F") { ?>
...
<p class="form-control-static"><?php echo $loan_application->repayment_period->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="loan_application" data-field="x_repayment_period" data-page="1" name="x_repayment_period" id="x_repayment_period" value="<?php echo ew_HtmlEncode($loan_application->repayment_period->FormValue) ?>">
<?php } ?>
...
<p class="form-control-static"><?php echo $loan_application->salary_permonth->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="loan_application" data-field="x_salary_permonth" data-page="1" name="x_salary_permonth" id="x_salary_permonth" value="<?php echo ew_HtmlEncode($loan_application->salary_permonth->FormValue) ?>">
<?php } ?>
...
<?php if ($loan_application->previous_loan->Visible) { // previous_loan ?>
	<div id="r_previous_loan" class="form-group">
		<label id="elh_loan_application_previous_loan" for="x_previous_loan" class="<?php echo $loan_application_edit->LeftColumnClass ?>"><?php echo $loan_application->previous_loan->FldCaption() ?></label>
		<div class="<?php echo $loan_application_edit->RightColumnClass ?>"><div<?php echo $loan_application->previous_loan->CellAttributes() ?>>
<?php if ($loan_application->CurrentAction <> "F") { ?>
...
<p class="form-control-static"><?php echo $loan_application->previous_loan->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="loan_application" data-field="x_previous_loan" data-page="1" name="x_previous_loan" id="x_previous_loan" value="<?php echo ew_HtmlEncode($loan_application->previous_loan->FormValue) ?>">
<?php } ?>
...
<?php if ($loan_application->date_collected->Visible) { // date_collected ?>
	<div id="r_date_collected" class="form-group">
		<label id="elh_loan_application_date_collected" for="x_date_collected" class="<?php echo $loan_application_edit->LeftColumnClass ?>"><?php echo $loan_application->date_collected->FldCaption() ?></label>
		<div class="<?php echo $loan_application_edit->RightColumnClass ?>"><div<?php echo $loan_application->date_collected->CellAttributes() ?>>
<?php if ($loan_application->CurrentAction <> "F") { ?>
...
<script type="text/javascript">
ew_CreateDateTimePicker("floan_applicationedit", "x_date_collected", {"ignoreReadonly":true,"useCurrent":false,"format":0});
</script>
<?php } ?>
</span>
<?php } else { ?>
...
<p class="form-control-static"><?php echo $loan_application->date_collected->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="loan_application" data-field="x_date_collected" data-page="1" name="x_date_collected" id="x_date_collected" value="<?php echo ew_HtmlEncode($loan_application->date_collected->FormValue) ?>">
<?php } ?>
...
<?php if ($loan_application->date_liquidated->Visible) { // date_liquidated ?>
	<div id="r_date_liquidated" class="form-group">
		<label id="elh_loan_application_date_liquidated" for="x_date_liquidated" class="<?php echo $loan_application_edit->LeftColumnClass ?>"><?php echo $loan_application->date_liquidated->FldCaption() ?></label>
		<div class="<?php echo $loan_application_edit->RightColumnClass ?>"><div<?php echo $loan_application->date_liquidated->CellAttributes() ?>>
<?php if ($loan_application->CurrentAction <> "F") { ?>
...
<script type="text/javascript">
ew_CreateDateTimePicker("floan_applicationedit", "x_date_liquidated", {"ignoreReadonly":true,"useCurrent":false,"format":0});
</script>
<?php } ?>
</span>
<?php } else { ?>
...
<p class="form-control-static"><?php echo $loan_application->date_liquidated->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="loan_application" data-field="x_date_liquidated" data-page="1" name="x_date_liquidated" id="x_date_liquidated" value="<?php echo ew_HtmlEncode($loan_application->date_liquidated->FormValue) ?>">
<?php } ?>
...
<?php if ($loan_application->balance_remaining->Visible) { // balance_remaining ?>
	<div id="r_balance_remaining" class="form-group">
		<label id="elh_loan_application_balance_remaining" for="x_balance_remaining" class="<?php echo $loan_application_edit->LeftColumnClass ?>"><?php echo $loan_application->balance_remaining->FldCaption() ?></label>
		<div class="<?php echo $loan_application_edit->RightColumnClass ?>"><div<?php echo $loan_application->balance_remaining->CellAttributes() ?>>
<?php if ($loan_application->CurrentAction <> "F") { ?>
...
<p class="form-control-static"><?php echo $loan_application->balance_remaining->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="loan_application" data-field="x_balance_remaining" data-page="1" name="x_balance_remaining" id="x_balance_remaining" value="<?php echo ew_HtmlEncode($loan_application->balance_remaining->FormValue) ?>">
<?php } ?>
...
<p class="form-control-static"><?php echo $loan_application->applicant_date->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="loan_application" data-field="x_applicant_date" data-page="1" name="x_applicant_date" id="x_applicant_date" value="<?php echo ew_HtmlEncode($loan_application->applicant_date->FormValue) ?>">
<?php } ?>
...
<span title="<?php echo $loan_application->applicant_passport->FldTitle() ? $loan_application->applicant_passport->FldTitle() : $Language->Phrase("ChooseFiles") ?>" class="btn btn-default btn-sm fileinput-button ewTooltip<?php if ($loan_application->applicant_passport->ReadOnly || $loan_application->applicant_passport->Disabled) echo " hide"; ?>" data-trigger="hover">
	<span><?php echo $Language->Phrase("ChooseFileBtn") ?></span>
	<input type="file" title=" " data-table="loan_application" data-field="x_applicant_passport" data-page="1" name="x_applicant_passport" id="x_applicant_passport" multiple="multiple"<?php echo $loan_application->applicant_passport->EditAttributes() ?>>
</span>
...
<?php if (@$_POST["fa_x_applicant_passport"] == "0") { ?>
<input type="hidden" name="fa_x_applicant_passport" id= "fa_x_applicant_passport" value="0">
<?php } else { ?>
<input type="hidden" name="fa_x_applicant_passport" id= "fa_x_applicant_passport" value="1">
<?php } ?>
...
<input type="hidden" name="fc_x_applicant_passport" id= "fc_x_applicant_passport" value="<?php echo $loan_application->applicant_passport->UploadMaxFileCount ?>">
</div>
...
<?php } ?>
		...
		<div class="tab-pane<?php echo $loan_application_edit->MultiPages->PageStyle("2") ?>" id="tab_loan_application2"><!-- multi-page .tab-pane -->
<div class="ewEditDiv"><!-- page* -->
...
<p class="form-control-static"><?php echo $loan_application->guarantor_name->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="loan_application" data-field="x_guarantor_name" data-page="2" name="x_guarantor_name" id="x_guarantor_name" value="<?php echo ew_HtmlEncode($loan_application->guarantor_name->FormValue) ?>">
<?php } ?>
...
<p class="form-control-static"><?php echo $loan_application->guarantor_address->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="loan_application" data-field="x_guarantor_address" data-page="2" name="x_guarantor_address" id="x_guarantor_address" value="<?php echo ew_HtmlEncode($loan_application->guarantor_address->FormValue) ?>">
<?php } ?>
...
<p class="form-control-static"><?php echo $loan_application->guarantor_mobile->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="loan_application" data-field="x_guarantor_mobile" data-page="2" name="x_guarantor_mobile" id="x_guarantor_mobile" value="<?php echo ew_HtmlEncode($loan_application->guarantor_mobile->FormValue) ?>">
<?php } ?>
...
<p class="form-control-static"><?php echo $loan_application->guarantor_department->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="loan_application" data-field="x_guarantor_department" data-page="2" name="x_guarantor_department" id="x_guarantor_department" value="<?php echo ew_HtmlEncode($loan_application->guarantor_department->FormValue) ?>">
<?php } ?>
...
<p class="form-control-static"><?php echo $loan_application->account_no->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="loan_application" data-field="x_account_no" data-page="2" name="x_account_no" id="x_account_no" value="<?php echo ew_HtmlEncode($loan_application->account_no->FormValue) ?>">
<?php } ?>
...
<p class="form-control-static"><?php echo $loan_application->bank_name->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="loan_application" data-field="x_bank_name" data-page="2" name="x_bank_name" id="x_bank_name" value="<?php echo ew_HtmlEncode($loan_application->bank_name->FormValue) ?>">
<?php } ?>
...
<p class="form-control-static"><?php echo $loan_application->employers_name->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="loan_application" data-field="x_employers_name" data-page="2" name="x_employers_name" id="x_employers_name" value="<?php echo ew_HtmlEncode($loan_application->employers_name->FormValue) ?>">
<?php } ?>
...
<p class="form-control-static"><?php echo $loan_application->employers_address->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="loan_application" data-field="x_employers_address" data-page="2" name="x_employers_address" id="x_employers_address" value="<?php echo ew_HtmlEncode($loan_application->employers_address->FormValue) ?>">
<?php } ?>
...
<p class="form-control-static"><?php echo $loan_application->employers_mobile->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="loan_application" data-field="x_employers_mobile" data-page="2" name="x_employers_mobile" id="x_employers_mobile" value="<?php echo ew_HtmlEncode($loan_application->employers_mobile->FormValue) ?>">
<?php } ?>
...
<p class="form-control-static"><?php echo $loan_application->guarantor_date->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="loan_application" data-field="x_guarantor_date" data-page="2" name="x_guarantor_date" id="x_guarantor_date" value="<?php echo ew_HtmlEncode($loan_application->guarantor_date->FormValue) ?>">
<?php } ?>
...
<span title="<?php echo $loan_application->guarantor_passport->FldTitle() ? $loan_application->guarantor_passport->FldTitle() : $Language->Phrase("ChooseFiles") ?>" class="btn btn-default btn-sm fileinput-button ewTooltip<?php if ($loan_application->guarantor_passport->ReadOnly || $loan_application->guarantor_passport->Disabled) echo " hide"; ?>" data-trigger="hover">
	<span><?php echo $Language->Phrase("ChooseFileBtn") ?></span>
	<input type="file" title=" " data-table="loan_application" data-field="x_guarantor_passport" data-page="2" name="x_guarantor_passport" id="x_guarantor_passport" multiple="multiple"<?php echo $loan_application->guarantor_passport->EditAttributes() ?>>
</span>
...
<?php if (@$_POST["fa_x_guarantor_passport"] == "0") { ?>
<input type="hidden" name="fa_x_guarantor_passport" id= "fa_x_guarantor_passport" value="0">
<?php } else { ?>
<input type="hidden" name="fa_x_guarantor_passport" id= "fa_x_guarantor_passport" value="1">
<?php } ?>
...
<input type="hidden" name="fc_x_guarantor_passport" id= "fc_x_guarantor_passport" value="<?php echo $loan_application->guarantor_passport->UploadMaxFileCount ?>">
</div>
...
<?php } ?>
		...
		<div class="tab-pane<?php echo $loan_application_edit->MultiPages->PageStyle("3") ?>" id="tab_loan_application3"><!-- multi-page .tab-pane -->
<div class="ewEditDiv"><!-- page* -->
...
<script type="text/javascript">
floan_applicationedit.CreateAutoSuggest({"id":"x_status","forceSelect":false});
</script>
</span>
<?php } else { ?>
<span id="el_loan_application_status">
<span<?php echo $loan_application->status->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $loan_application->status->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="loan_application" data-field="x_status" data-page="3" name="x_status" id="x_status" value="<?php echo ew_HtmlEncode($loan_application->status->FormValue) ?>">
<?php } ?>
...
<p class="form-control-static"><?php echo $loan_application->initiator_action->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="loan_application" data-field="x_initiator_action" data-page="3" name="x_initiator_action" id="x_initiator_action" value="<?php echo ew_HtmlEncode($loan_application->initiator_action->FormValue) ?>">
<?php } ?>
...
<p class="form-control-static"><?php echo $loan_application->initiator_comment->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="loan_application" data-field="x_initiator_comment" data-page="3" name="x_initiator_comment" id="x_initiator_comment" value="<?php echo ew_HtmlEncode($loan_application->initiator_comment->FormValue) ?>">
<?php } ?>
...
<p class="form-control-static"><?php echo $loan_application->recommended_date->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="loan_application" data-field="x_recommended_date" data-page="3" name="x_recommended_date" id="x_recommended_date" value="<?php echo ew_HtmlEncode($loan_application->recommended_date->FormValue) ?>">
<?php } ?>
...
<p class="form-control-static"><?php echo $loan_application->document_checklist->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="loan_application" data-field="x_document_checklist" data-page="3" name="x_document_checklist" id="x_document_checklist" value="<?php echo ew_HtmlEncode($loan_application->document_checklist->FormValue) ?>">
<?php } ?>
...
<p class="form-control-static"><?php echo $loan_application->recommender_action->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="loan_application" data-field="x_recommender_action" data-page="3" name="x_recommender_action" id="x_recommender_action" value="<?php echo ew_HtmlEncode($loan_application->recommender_action->FormValue) ?>">
<?php } ?>
...
<p class="form-control-static"><?php echo $loan_application->recommender_comment->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="loan_application" data-field="x_recommender_comment" data-page="3" name="x_recommender_comment" id="x_recommender_comment" value="<?php echo ew_HtmlEncode($loan_application->recommender_comment->FormValue) ?>">
<?php } ?>
...
<script type="text/javascript">
floan_applicationedit.CreateAutoSuggest({"id":"x_recommended_by","forceSelect":false});
</script>
</span>
<?php } else { ?>
<span id="el_loan_application_recommended_by">
<span<?php echo $loan_application->recommended_by->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $loan_application->recommended_by->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="loan_application" data-field="x_recommended_by" data-page="3" name="x_recommended_by" id="x_recommended_by" value="<?php echo ew_HtmlEncode($loan_application->recommended_by->FormValue) ?>">
<?php } ?>
...
<p class="form-control-static"><?php echo $loan_application->application_status->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="loan_application" data-field="x_application_status" data-page="3" name="x_application_status" id="x_application_status" value="<?php echo ew_HtmlEncode($loan_application->application_status->FormValue) ?>">
<?php } ?>
...
<p class="form-control-static"><?php echo $loan_application->approved_amount->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="loan_application" data-field="x_approved_amount" data-page="3" name="x_approved_amount" id="x_approved_amount" value="<?php echo ew_HtmlEncode($loan_application->approved_amount->FormValue) ?>">
<?php } ?>
...
<p class="form-control-static"><?php echo $loan_application->duration_approved->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="loan_application" data-field="x_duration_approved" data-page="3" name="x_duration_approved" id="x_duration_approved" value="<?php echo ew_HtmlEncode($loan_application->duration_approved->FormValue) ?>">
<?php } ?>
...
<?php if ($loan_application->approval_date->Visible) { // approval_date ?>
	<div id="r_approval_date" class="form-group">
		<label id="elh_loan_application_approval_date" for="x_approval_date" class="<?php echo $loan_application_edit->LeftColumnClass ?>"><?php echo $loan_application->approval_date->FldCaption() ?></label>
		<div class="<?php echo $loan_application_edit->RightColumnClass ?>"><div<?php echo $loan_application->approval_date->CellAttributes() ?>>
<?php if ($loan_application->CurrentAction <> "F") { ?>
...
<p class="form-control-static"><?php echo $loan_application->approval_date->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="loan_application" data-field="x_approval_date" data-page="3" name="x_approval_date" id="x_approval_date" value="<?php echo ew_HtmlEncode($loan_application->approval_date->FormValue) ?>">
<?php } ?>
...
<p class="form-control-static"><?php echo $loan_application->approval_action->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="loan_application" data-field="x_approval_action" data-page="3" name="x_approval_action" id="x_approval_action" value="<?php echo ew_HtmlEncode($loan_application->approval_action->FormValue) ?>">
<?php } ?>
...
<p class="form-control-static"><?php echo $loan_application->approval_comment->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="loan_application" data-field="x_approval_comment" data-page="3" name="x_approval_comment" id="x_approval_comment" value="<?php echo ew_HtmlEncode($loan_application->approval_comment->FormValue) ?>">
<?php } ?>
...
<p class="form-control-static"><?php echo $loan_application->approved_by->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="loan_application" data-field="x_approved_by" data-page="3" name="x_approved_by" id="x_approved_by" value="<?php echo ew_HtmlEncode($loan_application->approved_by->FormValue) ?>">
<?php } ?>
<?php echo $loan_application->approved_by->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($loan_application->correction_date->Visible) { // correction_date ?>
	<div id="r_correction_date" class="form-group">
		<label id="elh_loan_application_correction_date" for="x_correction_date" class="<?php echo $loan_application_edit->LeftColumnClass ?>"><?php echo $loan_application->correction_date->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $loan_application_edit->RightColumnClass ?>"><div<?php echo $loan_application->correction_date->CellAttributes() ?>>
<?php if ($loan_application->CurrentAction <> "F") { ?>
<span id="el_loan_application_correction_date">
<input type="text" data-table="loan_application" data-field="x_correction_date" data-page="3" name="x_correction_date" id="x_correction_date" placeholder="<?php echo ew_HtmlEncode($loan_application->correction_date->getPlaceHolder()) ?>" value="<?php echo $loan_application->correction_date->EditValue ?>"<?php echo $loan_application->correction_date->EditAttributes() ?>>
</span>
<?php } else { ?>
<span id="el_loan_application_correction_date">
<span<?php echo $loan_application->correction_date->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $loan_application->correction_date->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="loan_application" data-field="x_correction_date" data-page="3" name="x_correction_date" id="x_correction_date" value="<?php echo ew_HtmlEncode($loan_application->correction_date->FormValue) ?>">
<?php } ?>
<?php echo $loan_application->correction_date->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($loan_application->correction_action->Visible) { // correction_action ?>
	<div id="r_correction_action" class="form-group">
		<label id="elh_loan_application_correction_action" class="<?php echo $loan_application_edit->LeftColumnClass ?>"><?php echo $loan_application->correction_action->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $loan_application_edit->RightColumnClass ?>"><div<?php echo $loan_application->correction_action->CellAttributes() ?>>
<?php if ($loan_application->CurrentAction <> "F") { ?>
<span id="el_loan_application_correction_action">
<div id="tp_x_correction_action" class="ewTemplate"><input type="radio" data-table="loan_application" data-field="x_correction_action" data-page="3" data-value-separator="<?php echo $loan_application->correction_action->DisplayValueSeparatorAttribute() ?>" name="x_correction_action" id="x_correction_action" value="{value}"<?php echo $loan_application->correction_action->EditAttributes() ?>></div>
<div id="dsl_x_correction_action" data-repeatcolumn="5" class="ewItemList" style="display: none;"><div>
<?php echo $loan_application->correction_action->RadioButtonListHtml(FALSE, "x_correction_action", 3) ?>
</div></div>
</span>
<?php } else { ?>
<span id="el_loan_application_correction_action">
<span<?php echo $loan_application->correction_action->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $loan_application->correction_action->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="loan_application" data-field="x_correction_action" data-page="3" name="x_correction_action" id="x_correction_action" value="<?php echo ew_HtmlEncode($loan_application->correction_action->FormValue) ?>">
<?php } ?>
<?php echo $loan_application->correction_action->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($loan_application->correction_comment->Visible) { // correction_comment ?>
	<div id="r_correction_comment" class="form-group">
		<label id="elh_loan_application_correction_comment" for="x_correction_comment" class="<?php echo $loan_application_edit->LeftColumnClass ?>"><?php echo $loan_application->correction_comment->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $loan_application_edit->RightColumnClass ?>"><div<?php echo $loan_application->correction_comment->CellAttributes() ?>>
<?php if ($loan_application->CurrentAction <> "F") { ?>
<span id="el_loan_application_correction_comment">
<textarea data-table="loan_application" data-field="x_correction_comment" data-page="3" name="x_correction_comment" id="x_correction_comment" cols="30" rows="4" placeholder="<?php echo ew_HtmlEncode($loan_application->correction_comment->getPlaceHolder()) ?>"<?php echo $loan_application->correction_comment->EditAttributes() ?>><?php echo $loan_application->correction_comment->EditValue ?></textarea>
</span>
<?php } else { ?>
<span id="el_loan_application_correction_comment">
<span<?php echo $loan_application->correction_comment->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $loan_application->correction_comment->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="loan_application" data-field="x_correction_comment" data-page="3" name="x_correction_comment" id="x_correction_comment" value="<?php echo ew_HtmlEncode($loan_application->correction_comment->FormValue) ?>">
<?php } ?>
<?php echo $loan_application->correction_comment->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($loan_application->corrected_by->Visible) { // corrected_by ?>
	<div id="r_corrected_by" class="form-group">
		<label id="elh_loan_application_corrected_by" class="<?php echo $loan_application_edit->LeftColumnClass ?>"><?php echo $loan_application->corrected_by->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $loan_application_edit->RightColumnClass ?>"><div<?php echo $loan_application->corrected_by->CellAttributes() ?>>
<?php if ($loan_application->CurrentAction <> "F") { ?>
<span id="el_loan_application_corrected_by">
<?php
$wrkonchange = trim(" " . @$loan_application->corrected_by->EditAttrs["onchange"]);
if ($wrkonchange <> "") $wrkonchange = " onchange=\"" . ew_JsEncode2($wrkonchange) . "\"";
$loan_application->corrected_by->EditAttrs["onchange"] = "";
?>
<span id="as_x_corrected_by" style="white-space: nowrap; z-index: 8510">
	<input type="text" name="sv_x_corrected_by" id="sv_x_corrected_by" value="<?php echo $loan_application->corrected_by->EditValue ?>" size="30" placeholder="<?php echo ew_HtmlEncode($loan_application->corrected_by->getPlaceHolder()) ?>" data-placeholder="<?php echo ew_HtmlEncode($loan_application->corrected_by->getPlaceHolder()) ?>"<?php echo $loan_application->corrected_by->EditAttributes() ?>>
</span>
<input type="hidden" data-table="loan_application" data-field="x_corrected_by" data-page="3" data-value-separator="<?php echo $loan_application->corrected_by->DisplayValueSeparatorAttribute() ?>" name="x_corrected_by" id="x_corrected_by" value="<?php echo ew_HtmlEncode($loan_application->corrected_by->CurrentValue) ?>"<?php echo $wrkonchange ?>>
<script type="text/javascript">
floan_applicationedit.CreateAutoSuggest({"id":"x_corrected_by","forceSelect":false});
</script>
</span>
<?php } else { ?>
<span id="el_loan_application_corrected_by">
<span<?php echo $loan_application->corrected_by->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $loan_application->corrected_by->ViewValue ?></p></span>
</span>
<input type="hidden" data-table="loan_application" data-field="x_corrected_by" data-page="3" name="x_corrected_by" id="x_corrected_by" value="<?php echo ew_HtmlEncode($loan_application->corrected_by->FormValue) ?>">
<?php } ?>
<?php echo $loan_application->corrected_by->CustomMsg ?></div></div>
	</div>
<?php } ?>
</div><!-- /page* -->
		</div><!-- /multi-page .tab-pane -->
...
<input type="hidden" data-table="loan_application" data-field="x_code" name="x_code" id="x_code" value="<?php echo ew_HtmlEncode($loan_application->code->CurrentValue) ?>">
<?php if (!$loan_application_edit->IsModal) { ?>
<div class="form-group"><!-- buttons .form-group -->
	<div class="<?php echo $loan_application_edit->OffsetColumnClass ?>"><!-- buttons offset -->
<?php if ($loan_application->CurrentAction <> "F") { // Confirm page ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $loan_application_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
<?php } else { ?>
...
<script type="text/javascript">
floan_applicationedit.Init();
</script>
<?php
$loan_application_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$loan_application_edit->Page_Terminate();
?>
