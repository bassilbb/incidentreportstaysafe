...
<?php include_once "maintenance_reportinfo.php" ?>
...
$maintenance_report_list = NULL; // Initialize page object first

class cmaintenance_report_list extends cmaintenance_report {
	...
	// Table name
	var $TableName = 'maintenance_report';

	// Page object name
	var $PageObjName = 'maintenance_report_list';

	// Grid form hidden field names
	var $FormName = 'fmaintenance_reportlist';
	...
	function __construct() {
		...
		// Table object (maintenance_report)
		if (!isset($GLOBALS["maintenance_report"]) || get_class($GLOBALS["maintenance_report"]) == "cmaintenance_report") {
			$GLOBALS["maintenance_report"] = &$this;
			$GLOBALS["Table"] = &$GLOBALS["maintenance_report"];
		}
		...
		$this->AddUrl = "maintenance_reportadd.php";
		$this->InlineAddUrl = $this->PageUrl() . "a=add";
		$this->GridAddUrl = $this->PageUrl() . "a=gridadd";
		$this->GridEditUrl = $this->PageUrl() . "a=gridedit";
		$this->MultiDeleteUrl = "maintenance_reportdelete.php";
		$this->MultiUpdateUrl = "maintenance_reportupdate.php";
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fmaintenance_reportlistsrch";
	...
	function Page_Terminate($url = "") {
		...
		// Export
		global $EW_EXPORT, $maintenance_report;
		if ($this->CustomExport <> "" && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, $EW_EXPORT)) {
				$sContent = ob_get_contents();
			if ($gsExportFile == "") $gsExportFile = $this->TableVar;
			$class = $EW_EXPORT[$this->CustomExport];
			if (class_exists($class)) {
				$doc = new $class($maintenance_report);
				$doc->Text = $sContent;
	...
	function GetFilterList() {
		...
		if (EW_SEARCH_FILTER_OPTION == "Server" && isset($UserProfile))
			$sSavedFilterList = $UserProfile->GetSearchFilters(CurrentUserName(), "fmaintenance_reportlistsrch");
	...
	function ProcessFilterList() {
		...
		if (@$_POST["ajax"] == "savefilters") { // Save filter request (Ajax)
			...
			$UserProfile->SetSearchFilters(CurrentUserName(), "fmaintenance_reportlistsrch", $filters);
	...
	function RenderListOptions() {
		...
		if ($Security->CanView()) {
			if (ew_IsMobile())
				...
				$oListOpt->Body = "<a class=\"ewRowLink ewView\" title=\"" . $viewcaption . "\" data-table=\"maintenance_report\" data-caption=\"" . $viewcaption . "\" href=\"javascript:void(0);\" onclick=\"ew_ModalDialogShow({lnk:this,url:'" . ew_HtmlEncode($this->ViewUrl) . "',btn:null});\">" . $Language->Phrase("ViewLink") . "</a>";
		} else {
			$oListOpt->Body = "";
		}
	...
	function SetupOtherOptions() {
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"fmaintenance_reportlistsrch\" href=\"#\">" . $Language->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"fmaintenance_reportlistsrch\" href=\"#\">" . $Language->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->ListActions->Items as $listaction) {
				if ($listaction->Select == EW_ACTION_MULTIPLE) {
					...
					$item->Body = "<a class=\"ewAction ewListAction\" title=\"" . ew_HtmlEncode($caption) . "\" data-caption=\"" . ew_HtmlEncode($caption) . "\" href=\"\" onclick=\"ew_SubmitAction(event,jQuery.extend({f:document.fmaintenance_reportlist}," . $listaction->ToJson(TRUE) . "));return false;\">" . $icon . "</a>";
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $Language->Phrase("SearchPanel") . "\" data-caption=\"" . $Language->Phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fmaintenance_reportlistsrch\">" . $Language->Phrase("SearchLink") . "</button>";
		$item->Visible = TRUE;
	...
	function ValidateSearch() {
		...
		if (!EW_SERVER_VALIDATE)
			return TRUE;
		if (!ew_CheckEuroDate($this->date_initiated->AdvancedSearch->SearchValue)) {
			ew_AddMessage($gsSearchError, $this->date_initiated->FldErrMsg());
		}
		if (!ew_CheckEuroDate($this->date_initiated->AdvancedSearch->SearchValue2)) {
			ew_AddMessage($gsSearchError, $this->date_initiated->FldErrMsg());
		}
	...
	function SetupExportOptions() {
		...
		$url = "";
		$item->Body = "<button id=\"emf_maintenance_report\" class=\"ewExportLink ewEmail\" title=\"" . $Language->Phrase("ExportToEmailText") . "\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_maintenance_report',hdr:ewLanguage.Phrase('ExportToEmailText'),f:document.fmaintenance_reportlist,sel:false" . $url . "});\">" . $Language->Phrase("ExportToEmail") . "</button>";
		$item->Visible = FALSE;
...
// Create page object
if (!isset($maintenance_report_list)) $maintenance_report_list = new cmaintenance_report_list();

// Page init
$maintenance_report_list->Page_Init();

// Page main
$maintenance_report_list->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$maintenance_report_list->Page_Render();
?>
<?php include_once "header.php" ?>
<?php if ($maintenance_report->Export == "") { ?>
<script type="text/javascript">

// Form object
var CurrentPageID = EW_PAGE_ID = "list";
var CurrentForm = fmaintenance_reportlist = new ew_Form("fmaintenance_reportlist", "list");
fmaintenance_reportlist.FormKeyCountName = '<?php echo $maintenance_report_list->FormKeyCountName ?>';

// Form_CustomValidate event
fmaintenance_reportlist.Form_CustomValidate = 
...
fmaintenance_reportlist.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fmaintenance_reportlist.Lists["x_staff_id"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_staffno","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"users"};
fmaintenance_reportlist.Lists["x_staff_id"].Data = "<?php echo $maintenance_report_list->staff_id->LookupFilterQuery(FALSE, "list") ?>";
fmaintenance_reportlist.AutoSuggests["x_staff_id"] = <?php echo json_encode(array("data" => "ajax=autosuggest&" . $maintenance_report_list->staff_id->LookupFilterQuery(TRUE, "list"))) ?>;
fmaintenance_reportlist.Lists["x_staff_name"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_firstname","x_lastname","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"users"};
fmaintenance_reportlist.Lists["x_staff_name"].Data = "<?php echo $maintenance_report_list->staff_name->LookupFilterQuery(FALSE, "list") ?>";
fmaintenance_reportlist.AutoSuggests["x_staff_name"] = <?php echo json_encode(array("data" => "ajax=autosuggest&" . $maintenance_report_list->staff_name->LookupFilterQuery(TRUE, "list"))) ?>;
fmaintenance_reportlist.Lists["x_department"] = {"LinkField":"x_department_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_department_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"depertment"};
fmaintenance_reportlist.Lists["x_department"].Data = "<?php echo $maintenance_report_list->department->LookupFilterQuery(FALSE, "list") ?>";
fmaintenance_reportlist.AutoSuggests["x_department"] = <?php echo json_encode(array("data" => "ajax=autosuggest&" . $maintenance_report_list->department->LookupFilterQuery(TRUE, "list"))) ?>;
fmaintenance_reportlist.Lists["x_branch"] = {"LinkField":"x_branch_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_branch_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"branch"};
fmaintenance_reportlist.Lists["x_branch"].Data = "<?php echo $maintenance_report_list->branch->LookupFilterQuery(FALSE, "list") ?>";
fmaintenance_reportlist.AutoSuggests["x_branch"] = <?php echo json_encode(array("data" => "ajax=autosuggest&" . $maintenance_report_list->branch->LookupFilterQuery(TRUE, "list"))) ?>;
fmaintenance_reportlist.Lists["x_buildings"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_description","","",""],"ParentFields":[],"ChildFields":["x_floors"],"FilterFields":[],"Options":[],"Template":"","LinkTable":"buildings"};
fmaintenance_reportlist.Lists["x_buildings"].Data = "<?php echo $maintenance_report_list->buildings->LookupFilterQuery(FALSE, "list") ?>";
fmaintenance_reportlist.Lists["x_floors"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_description","","",""],"ParentFields":[],"ChildFields":["x_items[]"],"FilterFields":[],"Options":[],"Template":"","LinkTable":"floors"};
fmaintenance_reportlist.Lists["x_floors"].Data = "<?php echo $maintenance_report_list->floors->LookupFilterQuery(FALSE, "list") ?>";
fmaintenance_reportlist.Lists["x_items[]"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_description","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"items"};
fmaintenance_reportlist.Lists["x_items[]"].Data = "<?php echo $maintenance_report_list->items->LookupFilterQuery(FALSE, "list") ?>";
fmaintenance_reportlist.Lists["x_priority"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_description","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"incident_category"};
fmaintenance_reportlist.Lists["x_priority"].Data = "<?php echo $maintenance_report_list->priority->LookupFilterQuery(FALSE, "list") ?>";
fmaintenance_reportlist.Lists["x_status"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_description","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"maintained_status"};
fmaintenance_reportlist.Lists["x_status"].Data = "<?php echo $maintenance_report_list->status->LookupFilterQuery(FALSE, "list") ?>";

// Form object for search
var CurrentSearchForm = fmaintenance_reportlistsrch = new ew_Form("fmaintenance_reportlistsrch");

// Validate function for search
fmaintenance_reportlistsrch.Validate = function(fobj) {
	...
	if (elm && !ew_CheckEuroDate(elm.value))
		return this.OnError(elm, "<?php echo ew_JsEncode2($maintenance_report->date_initiated->FldErrMsg()) ?>");
...
// Form_CustomValidate event
fmaintenance_reportlistsrch.Form_CustomValidate = 
 function(fobj) { // DO NOT CHANGE THIS LINE!
...
// Use JavaScript validation or not
fmaintenance_reportlistsrch.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
fmaintenance_reportlistsrch.Lists["x_buildings"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_description","","",""],"ParentFields":[],"ChildFields":["x_floors"],"FilterFields":[],"Options":[],"Template":"","LinkTable":"buildings"};
fmaintenance_reportlistsrch.Lists["x_buildings"].Data = "<?php echo $maintenance_report_list->buildings->LookupFilterQuery(FALSE, "extbs") ?>";
fmaintenance_reportlistsrch.Lists["x_floors"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_description","","",""],"ParentFields":["x_buildings"],"ChildFields":["x_items[]"],"FilterFields":["x_buildings_id"],"Options":[],"Template":"","LinkTable":"floors"};
fmaintenance_reportlistsrch.Lists["x_floors"].Data = "<?php echo $maintenance_report_list->floors->LookupFilterQuery(FALSE, "extbs") ?>";
fmaintenance_reportlistsrch.Lists["x_items[]"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_description","","",""],"ParentFields":["x_floors"],"ChildFields":[],"FilterFields":["x_floor_id"],"Options":[],"Template":"","LinkTable":"items"};
fmaintenance_reportlistsrch.Lists["x_items[]"].Data = "<?php echo $maintenance_report_list->items->LookupFilterQuery(FALSE, "extbs") ?>";
fmaintenance_reportlistsrch.Lists["x_status"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_description","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"maintained_status"};
fmaintenance_reportlistsrch.Lists["x_status"].Data = "<?php echo $maintenance_report_list->status->LookupFilterQuery(FALSE, "extbs") ?>";
</script>
<script type="text/javascript">
...
<?php if ($maintenance_report->Export == "") { ?>
<div class="ewToolbar">
<?php if ($maintenance_report_list->TotalRecs > 0 && $maintenance_report_list->ExportOptions->Visible()) { ?>
<?php $maintenance_report_list->ExportOptions->Render("body") ?>
<?php } ?>
<?php if ($maintenance_report_list->SearchOptions->Visible()) { ?>
<?php $maintenance_report_list->SearchOptions->Render("body") ?>
<?php } ?>
<?php if ($maintenance_report_list->FilterOptions->Visible()) { ?>
<?php $maintenance_report_list->FilterOptions->Render("body") ?>
<?php } ?>
<div class="clearfix"></div>
</div>
<?php } ?>
<?php
	$bSelectLimit = $maintenance_report_list->UseSelectLimit;
	if ($bSelectLimit) {
		if ($maintenance_report_list->TotalRecs <= 0)
			$maintenance_report_list->TotalRecs = $maintenance_report->ListRecordCount();
	} else {
		if (!$maintenance_report_list->Recordset && ($maintenance_report_list->Recordset = $maintenance_report_list->LoadRecordset()))
			$maintenance_report_list->TotalRecs = $maintenance_report_list->Recordset->RecordCount();
	}
	$maintenance_report_list->StartRec = 1;
	if ($maintenance_report_list->DisplayRecs <= 0 || ($maintenance_report->Export <> "" && $maintenance_report->ExportAll)) // Display all records
		$maintenance_report_list->DisplayRecs = $maintenance_report_list->TotalRecs;
	if (!($maintenance_report->Export <> "" && $maintenance_report->ExportAll))
		$maintenance_report_list->SetupStartRec(); // Set up start record position
	if ($bSelectLimit)
		$maintenance_report_list->Recordset = $maintenance_report_list->LoadRecordset($maintenance_report_list->StartRec-1, $maintenance_report_list->DisplayRecs);

	// Set no record found message
	if ($maintenance_report->CurrentAction == "" && $maintenance_report_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$maintenance_report_list->setWarningMessage(ew_DeniedMsg());
		if ($maintenance_report_list->SearchWhere == "0=101")
			$maintenance_report_list->setWarningMessage($Language->Phrase("EnterSearchCriteria"));
		else
			$maintenance_report_list->setWarningMessage($Language->Phrase("NoRecord"));
	}
$maintenance_report_list->RenderOtherOptions();
?>
<?php if ($Security->CanSearch()) { ?>
<?php if ($maintenance_report->Export == "" && $maintenance_report->CurrentAction == "") { ?>
<form name="fmaintenance_reportlistsrch" id="fmaintenance_reportlistsrch" class="form-inline ewForm ewExtSearchForm" action="<?php echo ew_CurrentPage() ?>">
<?php $SearchPanelClass = ($maintenance_report_list->SearchWhere <> "") ? " in" : " in"; ?>
<div id="fmaintenance_reportlistsrch_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
<input type="hidden" name="cmd" value="search">
<input type="hidden" name="t" value="maintenance_report">
	<div class="ewBasicSearch">
<?php
if ($gsSearchError == "")
	$maintenance_report_list->LoadAdvancedSearch(); // Load advanced search

// Render for search
$maintenance_report->RowType = EW_ROWTYPE_SEARCH;

// Render row
$maintenance_report->ResetAttrs();
$maintenance_report_list->RenderRow();
?>
<div id="xsr_1" class="ewRow">
<?php if ($maintenance_report->date_initiated->Visible) { // date_initiated ?>
	<div id="xsc_date_initiated" class="ewCell form-group">
		<label for="x_date_initiated" class="ewSearchCaption ewLabel"><?php echo $maintenance_report->date_initiated->FldCaption() ?></label>
...
<input type="text" data-table="maintenance_report" data-field="x_date_initiated" data-format="7" name="x_date_initiated" id="x_date_initiated" size="25" placeholder="<?php echo ew_HtmlEncode($maintenance_report->date_initiated->getPlaceHolder()) ?>" value="<?php echo $maintenance_report->date_initiated->EditValue ?>"<?php echo $maintenance_report->date_initiated->EditAttributes() ?>>
<?php if (!$maintenance_report->date_initiated->ReadOnly && !$maintenance_report->date_initiated->Disabled && !isset($maintenance_report->date_initiated->EditAttrs["readonly"]) && !isset($maintenance_report->date_initiated->EditAttrs["disabled"])) { ?>
<script type="text/javascript">
ew_CreateDateTimePicker("fmaintenance_reportlistsrch", "x_date_initiated", {"ignoreReadonly":true,"useCurrent":false,"format":7});
</script>
<?php } ?>
</span>
...
<input type="text" data-table="maintenance_report" data-field="x_date_initiated" data-format="7" name="y_date_initiated" id="y_date_initiated" size="25" placeholder="<?php echo ew_HtmlEncode($maintenance_report->date_initiated->getPlaceHolder()) ?>" value="<?php echo $maintenance_report->date_initiated->EditValue2 ?>"<?php echo $maintenance_report->date_initiated->EditAttributes() ?>>
<?php if (!$maintenance_report->date_initiated->ReadOnly && !$maintenance_report->date_initiated->Disabled && !isset($maintenance_report->date_initiated->EditAttrs["readonly"]) && !isset($maintenance_report->date_initiated->EditAttrs["disabled"])) { ?>
<script type="text/javascript">
ew_CreateDateTimePicker("fmaintenance_reportlistsrch", "y_date_initiated", {"ignoreReadonly":true,"useCurrent":false,"format":7});
</script>
<?php } ?>
</span>
	</div>
...
<?php if ($maintenance_report->reference_id->Visible) { // reference_id ?>
	<div id="xsc_reference_id" class="ewCell form-group">
		<label for="x_reference_id" class="ewSearchCaption ewLabel"><?php echo $maintenance_report->reference_id->FldCaption() ?></label>
		...
		<span class="ewSearchField">
<input type="text" data-table="maintenance_report" data-field="x_reference_id" name="x_reference_id" id="x_reference_id" size="25" maxlength="50" placeholder="<?php echo ew_HtmlEncode($maintenance_report->reference_id->getPlaceHolder()) ?>" value="<?php echo $maintenance_report->reference_id->EditValue ?>"<?php echo $maintenance_report->reference_id->EditAttributes() ?>>
</span>
	</div>
<?php } ?>
</div>
<div id="xsr_3" class="ewRow">
<?php if ($maintenance_report->buildings->Visible) { // buildings ?>
	<div id="xsc_buildings" class="ewCell form-group">
		<label for="x_buildings" class="ewSearchCaption ewLabel"><?php echo $maintenance_report->buildings->FldCaption() ?></label>
...
<?php if ($maintenance_report->floors->Visible) { // floors ?>
	<div id="xsc_floors" class="ewCell form-group">
		<label for="x_floors" class="ewSearchCaption ewLabel"><?php echo $maintenance_report->floors->FldCaption() ?></label>
...
<?php if ($maintenance_report->items->Visible) { // items ?>
	<div id="xsc_items" class="ewCell form-group">
		<label class="ewSearchCaption ewLabel"><?php echo $maintenance_report->items->FldCaption() ?></label>
...
<?php if ($maintenance_report->status->Visible) { // status ?>
	<div id="xsc_status" class="ewCell form-group">
		<label for="x_status" class="ewSearchCaption ewLabel"><?php echo $maintenance_report->status->FldCaption() ?></label>
		<span class="ewSearchOperator"><?php echo $Language->Phrase("=") ?><input type="hidden" name="z_status" id="z_status" value="="></span>
		<span class="ewSearchField">
<select data-table="maintenance_report" data-field="x_status" data-value-separator="<?php echo $maintenance_report->status->DisplayValueSeparatorAttribute() ?>" id="x_status" name="x_status"<?php echo $maintenance_report->status->EditAttributes() ?>>
<?php echo $maintenance_report->status->SelectOptionListHtml("x_status") ?>
</select>
</span>
	</div>
<?php } ?>
...
<div id="xsr_7" class="ewRow">
	...
	<input type="text" name="<?php echo EW_TABLE_BASIC_SEARCH ?>" id="<?php echo EW_TABLE_BASIC_SEARCH ?>" class="form-control" value="<?php echo ew_HtmlEncode($maintenance_report_list->BasicSearch->getKeyword()) ?>" placeholder="<?php echo ew_HtmlEncode($Language->Phrase("Search")) ?>">
	<input type="hidden" name="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" id="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" value="<?php echo ew_HtmlEncode($maintenance_report_list->BasicSearch->getType()) ?>">
	<div class="input-group-btn">
		<button type="button" data-toggle="dropdown" class="btn btn-default"><span id="searchtype"><?php echo $maintenance_report_list->BasicSearch->getTypeNameShort() ?></span><span class="caret"></span></button>
		<ul class="dropdown-menu pull-right" role="menu">
			<li<?php if ($maintenance_report_list->BasicSearch->getType() == "") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this)"><?php echo $Language->Phrase("QuickSearchAuto") ?></a></li>
			<li<?php if ($maintenance_report_list->BasicSearch->getType() == "=") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'=')"><?php echo $Language->Phrase("QuickSearchExact") ?></a></li>
			<li<?php if ($maintenance_report_list->BasicSearch->getType() == "AND") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'AND')"><?php echo $Language->Phrase("QuickSearchAll") ?></a></li>
			<li<?php if ($maintenance_report_list->BasicSearch->getType() == "OR") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'OR')"><?php echo $Language->Phrase("QuickSearchAny") ?></a></li>
		</ul>
...
<?php } ?>
<?php } ?>
<?php $maintenance_report_list->ShowPageHeader(); ?>
<?php
$maintenance_report_list->ShowMessage();
?>
<?php if ($maintenance_report_list->TotalRecs > 0 || $maintenance_report->CurrentAction <> "") { ?>
<div class="box ewBox ewGrid<?php if ($maintenance_report_list->IsAddOrEdit()) { ?> ewGridAddEdit<?php } ?> maintenance_report">
<?php if ($maintenance_report->Export == "") { ?>
<div class="box-header ewGridUpperPanel">
<?php if ($maintenance_report->CurrentAction <> "gridadd" && $maintenance_report->CurrentAction <> "gridedit") { ?>
<form name="ewPagerForm" class="form-inline ewForm ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($maintenance_report_list->Pager)) $maintenance_report_list->Pager = new cPrevNextPager($maintenance_report_list->StartRec, $maintenance_report_list->DisplayRecs, $maintenance_report_list->TotalRecs, $maintenance_report_list->AutoHidePager) ?>
<?php if ($maintenance_report_list->Pager->RecordCount > 0 && $maintenance_report_list->Pager->Visible) { ?>
<div class="ewPager">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($maintenance_report_list->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $maintenance_report_list->PageUrl() ?>start=<?php echo $maintenance_report_list->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($maintenance_report_list->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $maintenance_report_list->PageUrl() ?>start=<?php echo $maintenance_report_list->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $maintenance_report_list->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($maintenance_report_list->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $maintenance_report_list->PageUrl() ?>start=<?php echo $maintenance_report_list->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($maintenance_report_list->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $maintenance_report_list->PageUrl() ?>start=<?php echo $maintenance_report_list->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $maintenance_report_list->Pager->PageCount ?></span>
</div>
<?php } ?>
<?php if ($maintenance_report_list->Pager->RecordCount > 0) { ?>
<div class="ewPager ewRec">
	<span><?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $maintenance_report_list->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $maintenance_report_list->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $maintenance_report_list->Pager->RecordCount ?></span>
</div>
<?php } ?>
<?php if ($maintenance_report_list->TotalRecs > 0 && (!$maintenance_report_list->AutoHidePageSizeSelector || $maintenance_report_list->Pager->Visible)) { ?>
<div class="ewPager">
<input type="hidden" name="t" value="maintenance_report">
<select name="<?php echo EW_TABLE_REC_PER_PAGE ?>" class="form-control input-sm ewTooltip" title="<?php echo $Language->Phrase("RecordsPerPage") ?>" onchange="this.form.submit();">
<option value="5"<?php if ($maintenance_report_list->DisplayRecs == 5) { ?> selected<?php } ?>>5</option>
<option value="10"<?php if ($maintenance_report_list->DisplayRecs == 10) { ?> selected<?php } ?>>10</option>
<option value="15"<?php if ($maintenance_report_list->DisplayRecs == 15) { ?> selected<?php } ?>>15</option>
<option value="20"<?php if ($maintenance_report_list->DisplayRecs == 20) { ?> selected<?php } ?>>20</option>
<option value="50"<?php if ($maintenance_report_list->DisplayRecs == 50) { ?> selected<?php } ?>>50</option>
<option value="ALL"<?php if ($maintenance_report->getRecordsPerPage() == -1) { ?> selected<?php } ?>><?php echo $Language->Phrase("AllRecords") ?></option>
</select>
</div>
<?php } ?>
</form>
...
<?php
	foreach ($maintenance_report_list->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<div class="clearfix"></div>
...
<?php } ?>
<form name="fmaintenance_reportlist" id="fmaintenance_reportlist" class="form-inline ewForm ewListForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($maintenance_report_list->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $maintenance_report_list->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="maintenance_report">
<div id="gmp_maintenance_report" class="<?php if (ew_IsResponsiveLayout()) { ?>table-responsive <?php } ?>ewGridMiddlePanel">
<?php if ($maintenance_report_list->TotalRecs > 0 || $maintenance_report->CurrentAction == "gridedit") { ?>
<table id="tbl_maintenance_reportlist" class="table ewTable">
<thead>
	<tr class="ewTableHeader">
<?php

// Header row
$maintenance_report_list->RowType = EW_ROWTYPE_HEADER;

// Render list options
$maintenance_report_list->RenderListOptions();

// Render list options (header, left)
$maintenance_report_list->ListOptions->Render("header", "left");
?>
<?php if ($maintenance_report->date_initiated->Visible) { // date_initiated ?>
	<?php if ($maintenance_report->SortUrl($maintenance_report->date_initiated) == "") { ?>
		<th data-name="date_initiated" class="<?php echo $maintenance_report->date_initiated->HeaderCellClass() ?>"><div id="elh_maintenance_report_date_initiated" class="maintenance_report_date_initiated"><div class="ewTableHeaderCaption"><?php echo $maintenance_report->date_initiated->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="date_initiated" class="<?php echo $maintenance_report->date_initiated->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $maintenance_report->SortUrl($maintenance_report->date_initiated) ?>',1);"><div id="elh_maintenance_report_date_initiated" class="maintenance_report_date_initiated">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $maintenance_report->date_initiated->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($maintenance_report->date_initiated->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($maintenance_report->date_initiated->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($maintenance_report->reference_id->Visible) { // reference_id ?>
	<?php if ($maintenance_report->SortUrl($maintenance_report->reference_id) == "") { ?>
		<th data-name="reference_id" class="<?php echo $maintenance_report->reference_id->HeaderCellClass() ?>"><div id="elh_maintenance_report_reference_id" class="maintenance_report_reference_id"><div class="ewTableHeaderCaption"><?php echo $maintenance_report->reference_id->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="reference_id" class="<?php echo $maintenance_report->reference_id->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $maintenance_report->SortUrl($maintenance_report->reference_id) ?>',1);"><div id="elh_maintenance_report_reference_id" class="maintenance_report_reference_id">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $maintenance_report->reference_id->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($maintenance_report->reference_id->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($maintenance_report->reference_id->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($maintenance_report->staff_id->Visible) { // staff_id ?>
	<?php if ($maintenance_report->SortUrl($maintenance_report->staff_id) == "") { ?>
		<th data-name="staff_id" class="<?php echo $maintenance_report->staff_id->HeaderCellClass() ?>"><div id="elh_maintenance_report_staff_id" class="maintenance_report_staff_id"><div class="ewTableHeaderCaption"><?php echo $maintenance_report->staff_id->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="staff_id" class="<?php echo $maintenance_report->staff_id->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $maintenance_report->SortUrl($maintenance_report->staff_id) ?>',1);"><div id="elh_maintenance_report_staff_id" class="maintenance_report_staff_id">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $maintenance_report->staff_id->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($maintenance_report->staff_id->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($maintenance_report->staff_id->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($maintenance_report->staff_name->Visible) { // staff_name ?>
	<?php if ($maintenance_report->SortUrl($maintenance_report->staff_name) == "") { ?>
		<th data-name="staff_name" class="<?php echo $maintenance_report->staff_name->HeaderCellClass() ?>"><div id="elh_maintenance_report_staff_name" class="maintenance_report_staff_name"><div class="ewTableHeaderCaption"><?php echo $maintenance_report->staff_name->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="staff_name" class="<?php echo $maintenance_report->staff_name->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $maintenance_report->SortUrl($maintenance_report->staff_name) ?>',1);"><div id="elh_maintenance_report_staff_name" class="maintenance_report_staff_name">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $maintenance_report->staff_name->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($maintenance_report->staff_name->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($maintenance_report->staff_name->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($maintenance_report->department->Visible) { // department ?>
	<?php if ($maintenance_report->SortUrl($maintenance_report->department) == "") { ?>
		<th data-name="department" class="<?php echo $maintenance_report->department->HeaderCellClass() ?>"><div id="elh_maintenance_report_department" class="maintenance_report_department"><div class="ewTableHeaderCaption"><?php echo $maintenance_report->department->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="department" class="<?php echo $maintenance_report->department->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $maintenance_report->SortUrl($maintenance_report->department) ?>',1);"><div id="elh_maintenance_report_department" class="maintenance_report_department">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $maintenance_report->department->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($maintenance_report->department->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($maintenance_report->department->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($maintenance_report->branch->Visible) { // branch ?>
	<?php if ($maintenance_report->SortUrl($maintenance_report->branch) == "") { ?>
		<th data-name="branch" class="<?php echo $maintenance_report->branch->HeaderCellClass() ?>"><div id="elh_maintenance_report_branch" class="maintenance_report_branch"><div class="ewTableHeaderCaption"><?php echo $maintenance_report->branch->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="branch" class="<?php echo $maintenance_report->branch->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $maintenance_report->SortUrl($maintenance_report->branch) ?>',1);"><div id="elh_maintenance_report_branch" class="maintenance_report_branch">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $maintenance_report->branch->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($maintenance_report->branch->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($maintenance_report->branch->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($maintenance_report->buildings->Visible) { // buildings ?>
	<?php if ($maintenance_report->SortUrl($maintenance_report->buildings) == "") { ?>
		<th data-name="buildings" class="<?php echo $maintenance_report->buildings->HeaderCellClass() ?>"><div id="elh_maintenance_report_buildings" class="maintenance_report_buildings"><div class="ewTableHeaderCaption"><?php echo $maintenance_report->buildings->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="buildings" class="<?php echo $maintenance_report->buildings->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $maintenance_report->SortUrl($maintenance_report->buildings) ?>',1);"><div id="elh_maintenance_report_buildings" class="maintenance_report_buildings">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $maintenance_report->buildings->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($maintenance_report->buildings->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($maintenance_report->buildings->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($maintenance_report->floors->Visible) { // floors ?>
	<?php if ($maintenance_report->SortUrl($maintenance_report->floors) == "") { ?>
		<th data-name="floors" class="<?php echo $maintenance_report->floors->HeaderCellClass() ?>"><div id="elh_maintenance_report_floors" class="maintenance_report_floors"><div class="ewTableHeaderCaption"><?php echo $maintenance_report->floors->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="floors" class="<?php echo $maintenance_report->floors->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $maintenance_report->SortUrl($maintenance_report->floors) ?>',1);"><div id="elh_maintenance_report_floors" class="maintenance_report_floors">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $maintenance_report->floors->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($maintenance_report->floors->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($maintenance_report->floors->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($maintenance_report->items->Visible) { // items ?>
	<?php if ($maintenance_report->SortUrl($maintenance_report->items) == "") { ?>
		<th data-name="items" class="<?php echo $maintenance_report->items->HeaderCellClass() ?>"><div id="elh_maintenance_report_items" class="maintenance_report_items"><div class="ewTableHeaderCaption"><?php echo $maintenance_report->items->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="items" class="<?php echo $maintenance_report->items->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $maintenance_report->SortUrl($maintenance_report->items) ?>',1);"><div id="elh_maintenance_report_items" class="maintenance_report_items">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $maintenance_report->items->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($maintenance_report->items->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($maintenance_report->items->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($maintenance_report->priority->Visible) { // priority ?>
	<?php if ($maintenance_report->SortUrl($maintenance_report->priority) == "") { ?>
		<th data-name="priority" class="<?php echo $maintenance_report->priority->HeaderCellClass() ?>"><div id="elh_maintenance_report_priority" class="maintenance_report_priority"><div class="ewTableHeaderCaption"><?php echo $maintenance_report->priority->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="priority" class="<?php echo $maintenance_report->priority->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $maintenance_report->SortUrl($maintenance_report->priority) ?>',1);"><div id="elh_maintenance_report_priority" class="maintenance_report_priority">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $maintenance_report->priority->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($maintenance_report->priority->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($maintenance_report->priority->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($maintenance_report->description->Visible) { // description ?>
	<?php if ($maintenance_report->SortUrl($maintenance_report->description) == "") { ?>
		<th data-name="description" class="<?php echo $maintenance_report->description->HeaderCellClass() ?>"><div id="elh_maintenance_report_description" class="maintenance_report_description"><div class="ewTableHeaderCaption"><?php echo $maintenance_report->description->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="description" class="<?php echo $maintenance_report->description->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $maintenance_report->SortUrl($maintenance_report->description) ?>',1);"><div id="elh_maintenance_report_description" class="maintenance_report_description">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $maintenance_report->description->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($maintenance_report->description->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($maintenance_report->description->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($maintenance_report->status->Visible) { // status ?>
	<?php if ($maintenance_report->SortUrl($maintenance_report->status) == "") { ?>
		<th data-name="status" class="<?php echo $maintenance_report->status->HeaderCellClass() ?>"><div id="elh_maintenance_report_status" class="maintenance_report_status"><div class="ewTableHeaderCaption"><?php echo $maintenance_report->status->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="status" class="<?php echo $maintenance_report->status->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $maintenance_report->SortUrl($maintenance_report->status) ?>',1);"><div id="elh_maintenance_report_status" class="maintenance_report_status">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $maintenance_report->status->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($maintenance_report->status->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($maintenance_report->status->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php
...
$maintenance_report_list->ListOptions->Render("header", "right");
?>
	</tr>
</thead>
<tbody>
<?php
if ($maintenance_report->ExportAll && $maintenance_report->Export <> "") {
	$maintenance_report_list->StopRec = $maintenance_report_list->TotalRecs;
} else {

	// Set the last record to display
	if ($maintenance_report_list->TotalRecs > $maintenance_report_list->StartRec + $maintenance_report_list->DisplayRecs - 1)
		$maintenance_report_list->StopRec = $maintenance_report_list->StartRec + $maintenance_report_list->DisplayRecs - 1;
	else
		$maintenance_report_list->StopRec = $maintenance_report_list->TotalRecs;
}
$maintenance_report_list->RecCnt = $maintenance_report_list->StartRec - 1;
if ($maintenance_report_list->Recordset && !$maintenance_report_list->Recordset->EOF) {
	$maintenance_report_list->Recordset->MoveFirst();
	$bSelectLimit = $maintenance_report_list->UseSelectLimit;
	if (!$bSelectLimit && $maintenance_report_list->StartRec > 1)
		$maintenance_report_list->Recordset->Move($maintenance_report_list->StartRec - 1);
} elseif (!$maintenance_report->AllowAddDeleteRow && $maintenance_report_list->StopRec == 0) {
	$maintenance_report_list->StopRec = $maintenance_report->GridAddRowCount;
}

// Initialize aggregate
$maintenance_report->RowType = EW_ROWTYPE_AGGREGATEINIT;
$maintenance_report->ResetAttrs();
$maintenance_report_list->RenderRow();
while ($maintenance_report_list->RecCnt < $maintenance_report_list->StopRec) {
	$maintenance_report_list->RecCnt++;
	if (intval($maintenance_report_list->RecCnt) >= intval($maintenance_report_list->StartRec)) {
		$maintenance_report_list->RowCnt++;

		// Set up key count
		$maintenance_report_list->KeyCount = $maintenance_report_list->RowIndex;

		// Init row class and style
		$maintenance_report->ResetAttrs();
		$maintenance_report->CssClass = "";
		if ($maintenance_report->CurrentAction == "gridadd") {
		} else {
			$maintenance_report_list->LoadRowValues($maintenance_report_list->Recordset); // Load row values
		}
		$maintenance_report->RowType = EW_ROWTYPE_VIEW; // Render view

		// Set up row id / data-rowindex
		$maintenance_report->RowAttrs = array_merge($maintenance_report->RowAttrs, array('data-rowindex'=>$maintenance_report_list->RowCnt, 'id'=>'r' . $maintenance_report_list->RowCnt . '_maintenance_report', 'data-rowtype'=>$maintenance_report->RowType));
		...
		$maintenance_report_list->RenderListOptions();
?>
	<tr<?php echo $maintenance_report->RowAttributes() ?>>
<?php

// Render list options (body, left)
$maintenance_report_list->ListOptions->Render("body", "left", $maintenance_report_list->RowCnt);
?>
	<?php if ($maintenance_report->date_initiated->Visible) { // date_initiated ?>
		<td data-name="date_initiated"<?php echo $maintenance_report->date_initiated->CellAttributes() ?>>
<span id="el<?php echo $maintenance_report_list->RowCnt ?>_maintenance_report_date_initiated" class="maintenance_report_date_initiated">
<span<?php echo $maintenance_report->date_initiated->ViewAttributes() ?>>
<?php echo $maintenance_report->date_initiated->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($maintenance_report->reference_id->Visible) { // reference_id ?>
		<td data-name="reference_id"<?php echo $maintenance_report->reference_id->CellAttributes() ?>>
<span id="el<?php echo $maintenance_report_list->RowCnt ?>_maintenance_report_reference_id" class="maintenance_report_reference_id">
<span<?php echo $maintenance_report->reference_id->ViewAttributes() ?>>
<?php echo $maintenance_report->reference_id->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($maintenance_report->staff_id->Visible) { // staff_id ?>
		<td data-name="staff_id"<?php echo $maintenance_report->staff_id->CellAttributes() ?>>
<span id="el<?php echo $maintenance_report_list->RowCnt ?>_maintenance_report_staff_id" class="maintenance_report_staff_id">
<span<?php echo $maintenance_report->staff_id->ViewAttributes() ?>>
<?php echo $maintenance_report->staff_id->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($maintenance_report->staff_name->Visible) { // staff_name ?>
		<td data-name="staff_name"<?php echo $maintenance_report->staff_name->CellAttributes() ?>>
<span id="el<?php echo $maintenance_report_list->RowCnt ?>_maintenance_report_staff_name" class="maintenance_report_staff_name">
<span<?php echo $maintenance_report->staff_name->ViewAttributes() ?>>
<?php echo $maintenance_report->staff_name->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($maintenance_report->department->Visible) { // department ?>
		<td data-name="department"<?php echo $maintenance_report->department->CellAttributes() ?>>
<span id="el<?php echo $maintenance_report_list->RowCnt ?>_maintenance_report_department" class="maintenance_report_department">
<span<?php echo $maintenance_report->department->ViewAttributes() ?>>
<?php echo $maintenance_report->department->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($maintenance_report->branch->Visible) { // branch ?>
		<td data-name="branch"<?php echo $maintenance_report->branch->CellAttributes() ?>>
<span id="el<?php echo $maintenance_report_list->RowCnt ?>_maintenance_report_branch" class="maintenance_report_branch">
<span<?php echo $maintenance_report->branch->ViewAttributes() ?>>
<?php echo $maintenance_report->branch->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($maintenance_report->buildings->Visible) { // buildings ?>
		<td data-name="buildings"<?php echo $maintenance_report->buildings->CellAttributes() ?>>
<span id="el<?php echo $maintenance_report_list->RowCnt ?>_maintenance_report_buildings" class="maintenance_report_buildings">
<span<?php echo $maintenance_report->buildings->ViewAttributes() ?>>
<?php echo $maintenance_report->buildings->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($maintenance_report->floors->Visible) { // floors ?>
		<td data-name="floors"<?php echo $maintenance_report->floors->CellAttributes() ?>>
<span id="el<?php echo $maintenance_report_list->RowCnt ?>_maintenance_report_floors" class="maintenance_report_floors">
<span<?php echo $maintenance_report->floors->ViewAttributes() ?>>
<?php echo $maintenance_report->floors->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($maintenance_report->items->Visible) { // items ?>
		<td data-name="items"<?php echo $maintenance_report->items->CellAttributes() ?>>
<span id="el<?php echo $maintenance_report_list->RowCnt ?>_maintenance_report_items" class="maintenance_report_items">
<span<?php echo $maintenance_report->items->ViewAttributes() ?>>
<?php echo $maintenance_report->items->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($maintenance_report->priority->Visible) { // priority ?>
		<td data-name="priority"<?php echo $maintenance_report->priority->CellAttributes() ?>>
<span id="el<?php echo $maintenance_report_list->RowCnt ?>_maintenance_report_priority" class="maintenance_report_priority">
<span<?php echo $maintenance_report->priority->ViewAttributes() ?>>
<?php echo $maintenance_report->priority->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($maintenance_report->description->Visible) { // description ?>
		<td data-name="description"<?php echo $maintenance_report->description->CellAttributes() ?>>
<span id="el<?php echo $maintenance_report_list->RowCnt ?>_maintenance_report_description" class="maintenance_report_description">
<span<?php echo $maintenance_report->description->ViewAttributes() ?>>
<?php echo $maintenance_report->description->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($maintenance_report->status->Visible) { // status ?>
		<td data-name="status"<?php echo $maintenance_report->status->CellAttributes() ?>>
<span id="el<?php echo $maintenance_report_list->RowCnt ?>_maintenance_report_status" class="maintenance_report_status">
<span<?php echo $maintenance_report->status->ViewAttributes() ?>>
<?php echo $maintenance_report->status->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
<?php
...
$maintenance_report_list->ListOptions->Render("body", "right", $maintenance_report_list->RowCnt);
?>
	</tr>
<?php
	}
	if ($maintenance_report->CurrentAction <> "gridadd")
		$maintenance_report_list->Recordset->MoveNext();
}
?>
</tbody>
</table>
<?php } ?>
<?php if ($maintenance_report->CurrentAction == "") { ?>
...
// Close recordset
if ($maintenance_report_list->Recordset)
	$maintenance_report_list->Recordset->Close();
?>
</div>
<?php } ?>
<?php if ($maintenance_report_list->TotalRecs == 0 && $maintenance_report->CurrentAction == "") { // Show other options ?>
<div class="ewListOtherOptions">
<?php
	foreach ($maintenance_report_list->OtherOptions as &$option) {
		$option->ButtonClass = "";
...
<?php if ($maintenance_report->Export == "") { ?>
<script type="text/javascript">
fmaintenance_reportlistsrch.FilterList = <?php echo $maintenance_report_list->GetFilterList() ?>;
fmaintenance_reportlistsrch.Init();
fmaintenance_reportlist.Init();
</script>
<?php } ?>
<?php
$maintenance_report_list->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($maintenance_report->Export == "") { ?>
...
<?php
$maintenance_report_list->Page_Terminate();
?>
