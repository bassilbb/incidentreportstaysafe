...
class ccategory_edit extends ccategory {
	...
	function Page_Main() {
		...
		switch ($this->CurrentAction) {
			...
			Case "U": // Update
				...
				if (ew_GetPageName($sReturnUrl) == "categorylist.php")
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// code_id
			$this->code_id->SetDbValueDef($rsnew, $this->code_id->CurrentValue, NULL, $this->code_id->ReadOnly);
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $category->description->FldCaption(), $category->description->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_code_id");
			if (elm && !ew_CheckInteger(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($category->code_id->FldErrMsg()) ?>");
...
fcategoryedit.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;
...
<?php if (!isset($category_edit->Pager)) $category_edit->Pager = new cPrevNextPager($category_edit->StartRec, $category_edit->DisplayRecs, $category_edit->TotalRecs, $category_edit->AutoHidePager) ?>
...
<!--first page button-->
	...
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $category_edit->PageUrl() ?>start=<?php echo $category_edit->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($category_edit->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $category_edit->PageUrl() ?>start=<?php echo $category_edit->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $category_edit->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($category_edit->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $category_edit->PageUrl() ?>start=<?php echo $category_edit->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($category_edit->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $category_edit->PageUrl() ?>start=<?php echo $category_edit->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $category_edit->Pager->PageCount ?></span>
</div>
<?php } ?>
...
<input type="hidden" name="modal" value="<?php echo intval($category_edit->IsModal) ?>">
<div class="ewEditDiv"><!-- page* -->
<?php if ($category->category_id->Visible) { // category_id ?>
	<div id="r_category_id" class="form-group">
		<label id="elh_category_category_id" class="<?php echo $category_edit->LeftColumnClass ?>"><?php echo $category->category_id->FldCaption() ?></label>
		<div class="<?php echo $category_edit->RightColumnClass ?>"><div<?php echo $category->category_id->CellAttributes() ?>>
...
<?php if ($category->description->Visible) { // description ?>
	<div id="r_description" class="form-group">
		<label id="elh_category_description" for="x_description" class="<?php echo $category_edit->LeftColumnClass ?>"><?php echo $category->description->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
...
<input type="text" data-table="category" data-field="x_description" name="x_description" id="x_description" size="30" maxlength="50" placeholder="<?php echo ew_HtmlEncode($category->description->getPlaceHolder()) ?>" value="<?php echo $category->description->EditValue ?>"<?php echo $category->description->EditAttributes() ?>>
</span>
...
<?php if ($category->code_id->Visible) { // code_id ?>
	<div id="r_code_id" class="form-group">
		<label id="elh_category_code_id" for="x_code_id" class="<?php echo $category_edit->LeftColumnClass ?>"><?php echo $category->code_id->FldCaption() ?></label>
		<div class="<?php echo $category_edit->RightColumnClass ?>"><div<?php echo $category->code_id->CellAttributes() ?>>
<span id="el_category_code_id">
<input type="text" data-table="category" data-field="x_code_id" name="x_code_id" id="x_code_id" size="30" placeholder="<?php echo ew_HtmlEncode($category->code_id->getPlaceHolder()) ?>" value="<?php echo $category->code_id->EditValue ?>"<?php echo $category->code_id->EditAttributes() ?>>
</span>
<?php echo $category->code_id->CustomMsg ?></div></div>
	</div>
<?php } ?>
</div><!-- /page* -->
<?php if (!$category_edit->IsModal) { ?>
