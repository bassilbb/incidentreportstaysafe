...
$inventory_staysafe_report_list = NULL; // Initialize page object first

class cinventory_staysafe_report_list extends cinventory_staysafe_report {
	...
	// Page object name
	var $PageObjName = 'inventory_staysafe_report_list';

	// Grid form hidden field names
	var $FormName = 'finventory_staysafe_reportlist';
	...
	function __construct() {
		...
		$this->AddUrl = "inventory_staysafe_reportadd.php";
		$this->InlineAddUrl = $this->PageUrl() . "a=add";
		$this->GridAddUrl = $this->PageUrl() . "a=gridadd";
		$this->GridEditUrl = $this->PageUrl() . "a=gridedit";
		$this->MultiDeleteUrl = "inventory_staysafe_reportdelete.php";
		$this->MultiUpdateUrl = "inventory_staysafe_reportupdate.php";
		...
		$this->FilterOptions->TagClassName = "ewFilterOption finventory_staysafe_reportlistsrch";
	...
	function GetFilterList() {
		...
		if (EW_SEARCH_FILTER_OPTION == "Server" && isset($UserProfile))
			$sSavedFilterList = $UserProfile->GetSearchFilters(CurrentUserName(), "finventory_staysafe_reportlistsrch");
	...
	function ProcessFilterList() {
		...
		if (@$_POST["ajax"] == "savefilters") { // Save filter request (Ajax)
			...
			$UserProfile->SetSearchFilters(CurrentUserName(), "finventory_staysafe_reportlistsrch", $filters);
	...
	function SetupOtherOptions() {
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"finventory_staysafe_reportlistsrch\" href=\"#\">" . $Language->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"finventory_staysafe_reportlistsrch\" href=\"#\">" . $Language->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->ListActions->Items as $listaction) {
				if ($listaction->Select == EW_ACTION_MULTIPLE) {
					...
					$item->Body = "<a class=\"ewAction ewListAction\" title=\"" . ew_HtmlEncode($caption) . "\" data-caption=\"" . ew_HtmlEncode($caption) . "\" href=\"\" onclick=\"ew_SubmitAction(event,jQuery.extend({f:document.finventory_staysafe_reportlist}," . $listaction->ToJson(TRUE) . "));return false;\">" . $icon . "</a>";
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $Language->Phrase("SearchPanel") . "\" data-caption=\"" . $Language->Phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"finventory_staysafe_reportlistsrch\">" . $Language->Phrase("SearchLink") . "</button>";
		$item->Visible = TRUE;
		...
		$item->Body = "<a class=\"btn btn-default ewAdvancedSearch\" title=\"" . $Language->Phrase("AdvancedSearch") . "\" data-caption=\"" . $Language->Phrase("AdvancedSearch") . "\" href=\"inventory_staysafe_reportsrch.php\">" . $Language->Phrase("AdvancedSearchBtn") . "</a>";
		$item->Visible = TRUE;

		// Search highlight button
		$item = &$this->SearchOptions->Add("searchhighlight");
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewHighlight active\" title=\"" . $Language->Phrase("Highlight") . "\" data-caption=\"" . $Language->Phrase("Highlight") . "\" data-toggle=\"button\" data-form=\"finventory_staysafe_reportlistsrch\" data-name=\"" . $this->HighlightName() . "\">" . $Language->Phrase("HighlightBtn") . "</button>";
	...
	function SetupExportOptions() {
		...
		$url = "";
		$item->Body = "<button id=\"emf_inventory_staysafe_report\" class=\"ewExportLink ewEmail\" title=\"" . $Language->Phrase("ExportToEmailText") . "\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_inventory_staysafe_report',hdr:ewLanguage.Phrase('ExportToEmailText'),f:document.finventory_staysafe_reportlist,sel:false" . $url . "});\">" . $Language->Phrase("ExportToEmail") . "</button>";
		$item->Visible = FALSE;
...
// Create page object
if (!isset($inventory_staysafe_report_list)) $inventory_staysafe_report_list = new cinventory_staysafe_report_list();

// Page init
$inventory_staysafe_report_list->Page_Init();

// Page main
$inventory_staysafe_report_list->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$inventory_staysafe_report_list->Page_Render();
?>
<?php include_once "header.php" ?>
<?php if ($inventory_staysafe_report->Export == "") { ?>
<script type="text/javascript">

// Form object
var CurrentPageID = EW_PAGE_ID = "list";
var CurrentForm = finventory_staysafe_reportlist = new ew_Form("finventory_staysafe_reportlist", "list");
finventory_staysafe_reportlist.FormKeyCountName = '<?php echo $inventory_staysafe_report_list->FormKeyCountName ?>';

// Form_CustomValidate event
finventory_staysafe_reportlist.Form_CustomValidate = 
...
finventory_staysafe_reportlist.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
finventory_staysafe_reportlist.Lists["x_material_name"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_material_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"inventory_staysafe"};
finventory_staysafe_reportlist.Lists["x_material_name"].Data = "<?php echo $inventory_staysafe_report_list->material_name->LookupFilterQuery(FALSE, "list") ?>";
finventory_staysafe_reportlist.AutoSuggests["x_material_name"] = <?php echo json_encode(array("data" => "ajax=autosuggest&" . $inventory_staysafe_report_list->material_name->LookupFilterQuery(TRUE, "list"))) ?>;
finventory_staysafe_reportlist.Lists["x_statuss"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_description","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"statuss"};
finventory_staysafe_reportlist.Lists["x_statuss"].Data = "<?php echo $inventory_staysafe_report_list->statuss->LookupFilterQuery(FALSE, "list") ?>";
finventory_staysafe_reportlist.Lists["x_approved_by"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_firstname","x_lastname","x_staffno",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"users"};
finventory_staysafe_reportlist.Lists["x_approved_by"].Data = "<?php echo $inventory_staysafe_report_list->approved_by->LookupFilterQuery(FALSE, "list") ?>";
finventory_staysafe_reportlist.AutoSuggests["x_approved_by"] = <?php echo json_encode(array("data" => "ajax=autosuggest&" . $inventory_staysafe_report_list->approved_by->LookupFilterQuery(TRUE, "list"))) ?>;
finventory_staysafe_reportlist.Lists["x_verified_by"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_firstname","x_lastname","x_staffno",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"users"};
finventory_staysafe_reportlist.Lists["x_verified_by"].Data = "<?php echo $inventory_staysafe_report_list->verified_by->LookupFilterQuery(FALSE, "list") ?>";
finventory_staysafe_reportlist.AutoSuggests["x_verified_by"] = <?php echo json_encode(array("data" => "ajax=autosuggest&" . $inventory_staysafe_report_list->verified_by->LookupFilterQuery(TRUE, "list"))) ?>;

// Form object for search
var CurrentSearchForm = finventory_staysafe_reportlistsrch = new ew_Form("finventory_staysafe_reportlistsrch");

// Validate function for search
finventory_staysafe_reportlistsrch.Validate = function(fobj) {
...
// Form_CustomValidate event
finventory_staysafe_reportlistsrch.Form_CustomValidate = 
 function(fobj) { // DO NOT CHANGE THIS LINE!
...
// Use JavaScript validation or not
finventory_staysafe_reportlistsrch.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
finventory_staysafe_reportlistsrch.Lists["x_material_name"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_material_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"inventory_staysafe"};
finventory_staysafe_reportlistsrch.Lists["x_material_name"].Data = "<?php echo $inventory_staysafe_report_list->material_name->LookupFilterQuery(FALSE, "extbs") ?>";
finventory_staysafe_reportlistsrch.AutoSuggests["x_material_name"] = <?php echo json_encode(array("data" => "ajax=autosuggest&" . $inventory_staysafe_report_list->material_name->LookupFilterQuery(TRUE, "extbs"))) ?>;
finventory_staysafe_reportlistsrch.Lists["x_statuss"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_description","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"statuss"};
finventory_staysafe_reportlistsrch.Lists["x_statuss"].Data = "<?php echo $inventory_staysafe_report_list->statuss->LookupFilterQuery(FALSE, "extbs") ?>";
</script>
<script type="text/javascript">
...
<?php if ($inventory_staysafe_report->Export == "") { ?>
<div class="ewToolbar">
<?php if ($inventory_staysafe_report_list->TotalRecs > 0 && $inventory_staysafe_report_list->ExportOptions->Visible()) { ?>
<?php $inventory_staysafe_report_list->ExportOptions->Render("body") ?>
<?php } ?>
<?php if ($inventory_staysafe_report_list->SearchOptions->Visible()) { ?>
<?php $inventory_staysafe_report_list->SearchOptions->Render("body") ?>
<?php } ?>
<?php if ($inventory_staysafe_report_list->FilterOptions->Visible()) { ?>
<?php $inventory_staysafe_report_list->FilterOptions->Render("body") ?>
<?php } ?>
<div class="clearfix"></div>
</div>
<?php } ?>
<?php
	$bSelectLimit = $inventory_staysafe_report_list->UseSelectLimit;
	if ($bSelectLimit) {
		if ($inventory_staysafe_report_list->TotalRecs <= 0)
			$inventory_staysafe_report_list->TotalRecs = $inventory_staysafe_report->ListRecordCount();
	} else {
		if (!$inventory_staysafe_report_list->Recordset && ($inventory_staysafe_report_list->Recordset = $inventory_staysafe_report_list->LoadRecordset()))
			$inventory_staysafe_report_list->TotalRecs = $inventory_staysafe_report_list->Recordset->RecordCount();
	}
	$inventory_staysafe_report_list->StartRec = 1;
	if ($inventory_staysafe_report_list->DisplayRecs <= 0 || ($inventory_staysafe_report->Export <> "" && $inventory_staysafe_report->ExportAll)) // Display all records
		$inventory_staysafe_report_list->DisplayRecs = $inventory_staysafe_report_list->TotalRecs;
	if (!($inventory_staysafe_report->Export <> "" && $inventory_staysafe_report->ExportAll))
		$inventory_staysafe_report_list->SetupStartRec(); // Set up start record position
	if ($bSelectLimit)
		$inventory_staysafe_report_list->Recordset = $inventory_staysafe_report_list->LoadRecordset($inventory_staysafe_report_list->StartRec-1, $inventory_staysafe_report_list->DisplayRecs);

	// Set no record found message
	if ($inventory_staysafe_report->CurrentAction == "" && $inventory_staysafe_report_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$inventory_staysafe_report_list->setWarningMessage(ew_DeniedMsg());
		if ($inventory_staysafe_report_list->SearchWhere == "0=101")
			$inventory_staysafe_report_list->setWarningMessage($Language->Phrase("EnterSearchCriteria"));
		else
			$inventory_staysafe_report_list->setWarningMessage($Language->Phrase("NoRecord"));
	}

	// Audit trail on search
	if ($inventory_staysafe_report_list->AuditTrailOnSearch && $inventory_staysafe_report_list->Command == "search" && !$inventory_staysafe_report_list->RestoreSearch) {
		$searchparm = ew_ServerVar("QUERY_STRING");
		$searchsql = $inventory_staysafe_report_list->getSessionWhere();
		$inventory_staysafe_report_list->WriteAuditTrailOnSearch($searchparm, $searchsql);
	}
$inventory_staysafe_report_list->RenderOtherOptions();
?>
<?php if ($Security->CanSearch()) { ?>
<?php if ($inventory_staysafe_report->Export == "" && $inventory_staysafe_report->CurrentAction == "") { ?>
<form name="finventory_staysafe_reportlistsrch" id="finventory_staysafe_reportlistsrch" class="form-inline ewForm ewExtSearchForm" action="<?php echo ew_CurrentPage() ?>">
<?php $SearchPanelClass = ($inventory_staysafe_report_list->SearchWhere <> "") ? " in" : " in"; ?>
<div id="finventory_staysafe_reportlistsrch_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
<input type="hidden" name="cmd" value="search">
<input type="hidden" name="t" value="inventory_staysafe_report">
	<div class="ewBasicSearch">
<?php
if ($gsSearchError == "")
	$inventory_staysafe_report_list->LoadAdvancedSearch(); // Load advanced search

// Render for search
$inventory_staysafe_report->RowType = EW_ROWTYPE_SEARCH;

// Render row
$inventory_staysafe_report->ResetAttrs();
$inventory_staysafe_report_list->RenderRow();
?>
<div id="xsr_1" class="ewRow">
<?php if ($inventory_staysafe_report->date_recieved->Visible) { // date_recieved ?>
	<div id="xsc_date_recieved" class="ewCell form-group">
		<label for="x_date_recieved" class="ewSearchCaption ewLabel"><?php echo $inventory_staysafe_report->date_recieved->FldCaption() ?></label>
...
<script type="text/javascript">
ew_CreateDateTimePicker("finventory_staysafe_reportlistsrch", "x_date_recieved", {"ignoreReadonly":true,"useCurrent":false,"format":0});
</script>
<?php } ?>
</span>
...
<?php if (!$inventory_staysafe_report->date_recieved->ReadOnly && !$inventory_staysafe_report->date_recieved->Disabled && !isset($inventory_staysafe_report->date_recieved->EditAttrs["readonly"]) && !isset($inventory_staysafe_report->date_recieved->EditAttrs["disabled"])) { ?>
<script type="text/javascript">
ew_CreateDateTimePicker("finventory_staysafe_reportlistsrch", "y_date_recieved", {"ignoreReadonly":true,"useCurrent":false,"format":0});
</script>
<?php } ?>
</span>
	</div>
...
<?php if ($inventory_staysafe_report->reference_id->Visible) { // reference_id ?>
	<div id="xsc_reference_id" class="ewCell form-group">
		<label for="x_reference_id" class="ewSearchCaption ewLabel"><?php echo $inventory_staysafe_report->reference_id->FldCaption() ?></label>
...
<?php if ($inventory_staysafe_report->material_name->Visible) { // material_name ?>
	<div id="xsc_material_name" class="ewCell form-group">
		<label class="ewSearchCaption ewLabel"><?php echo $inventory_staysafe_report->material_name->FldCaption() ?></label>
...
<script type="text/javascript">
finventory_staysafe_reportlistsrch.CreateAutoSuggest({"id":"x_material_name","forceSelect":false});
</script>
</span>
	</div>
<?php } ?>
...
<?php if ($inventory_staysafe_report->statuss->Visible) { // statuss ?>
	<div id="xsc_statuss" class="ewCell form-group">
		<label for="x_statuss" class="ewSearchCaption ewLabel"><?php echo $inventory_staysafe_report->statuss->FldCaption() ?></label>
...
<div id="xsr_5" class="ewRow">
	...
	<input type="text" name="<?php echo EW_TABLE_BASIC_SEARCH ?>" id="<?php echo EW_TABLE_BASIC_SEARCH ?>" class="form-control" value="<?php echo ew_HtmlEncode($inventory_staysafe_report_list->BasicSearch->getKeyword()) ?>" placeholder="<?php echo ew_HtmlEncode($Language->Phrase("Search")) ?>">
	<input type="hidden" name="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" id="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" value="<?php echo ew_HtmlEncode($inventory_staysafe_report_list->BasicSearch->getType()) ?>">
	<div class="input-group-btn">
		<button type="button" data-toggle="dropdown" class="btn btn-default"><span id="searchtype"><?php echo $inventory_staysafe_report_list->BasicSearch->getTypeNameShort() ?></span><span class="caret"></span></button>
		<ul class="dropdown-menu pull-right" role="menu">
			<li<?php if ($inventory_staysafe_report_list->BasicSearch->getType() == "") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this)"><?php echo $Language->Phrase("QuickSearchAuto") ?></a></li>
			<li<?php if ($inventory_staysafe_report_list->BasicSearch->getType() == "=") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'=')"><?php echo $Language->Phrase("QuickSearchExact") ?></a></li>
			<li<?php if ($inventory_staysafe_report_list->BasicSearch->getType() == "AND") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'AND')"><?php echo $Language->Phrase("QuickSearchAll") ?></a></li>
			<li<?php if ($inventory_staysafe_report_list->BasicSearch->getType() == "OR") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'OR')"><?php echo $Language->Phrase("QuickSearchAny") ?></a></li>
		</ul>
...
<?php } ?>
<?php } ?>
<?php $inventory_staysafe_report_list->ShowPageHeader(); ?>
<?php
$inventory_staysafe_report_list->ShowMessage();
?>
<?php if ($inventory_staysafe_report_list->TotalRecs > 0 || $inventory_staysafe_report->CurrentAction <> "") { ?>
<div class="box ewBox ewGrid<?php if ($inventory_staysafe_report_list->IsAddOrEdit()) { ?> ewGridAddEdit<?php } ?> inventory_staysafe_report">
<?php if ($inventory_staysafe_report->Export == "") { ?>
<div class="box-header ewGridUpperPanel">
<?php if ($inventory_staysafe_report->CurrentAction <> "gridadd" && $inventory_staysafe_report->CurrentAction <> "gridedit") { ?>
<form name="ewPagerForm" class="form-inline ewForm ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($inventory_staysafe_report_list->Pager)) $inventory_staysafe_report_list->Pager = new cPrevNextPager($inventory_staysafe_report_list->StartRec, $inventory_staysafe_report_list->DisplayRecs, $inventory_staysafe_report_list->TotalRecs, $inventory_staysafe_report_list->AutoHidePager) ?>
<?php if ($inventory_staysafe_report_list->Pager->RecordCount > 0 && $inventory_staysafe_report_list->Pager->Visible) { ?>
<div class="ewPager">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($inventory_staysafe_report_list->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $inventory_staysafe_report_list->PageUrl() ?>start=<?php echo $inventory_staysafe_report_list->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($inventory_staysafe_report_list->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $inventory_staysafe_report_list->PageUrl() ?>start=<?php echo $inventory_staysafe_report_list->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $inventory_staysafe_report_list->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($inventory_staysafe_report_list->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $inventory_staysafe_report_list->PageUrl() ?>start=<?php echo $inventory_staysafe_report_list->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($inventory_staysafe_report_list->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $inventory_staysafe_report_list->PageUrl() ?>start=<?php echo $inventory_staysafe_report_list->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $inventory_staysafe_report_list->Pager->PageCount ?></span>
</div>
<?php } ?>
<?php if ($inventory_staysafe_report_list->Pager->RecordCount > 0) { ?>
<div class="ewPager ewRec">
	<span><?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $inventory_staysafe_report_list->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $inventory_staysafe_report_list->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $inventory_staysafe_report_list->Pager->RecordCount ?></span>
</div>
<?php } ?>
<?php if ($inventory_staysafe_report_list->TotalRecs > 0 && (!$inventory_staysafe_report_list->AutoHidePageSizeSelector || $inventory_staysafe_report_list->Pager->Visible)) { ?>
<div class="ewPager">
<input type="hidden" name="t" value="inventory_staysafe_report">
<select name="<?php echo EW_TABLE_REC_PER_PAGE ?>" class="form-control input-sm ewTooltip" title="<?php echo $Language->Phrase("RecordsPerPage") ?>" onchange="this.form.submit();">
<option value="5"<?php if ($inventory_staysafe_report_list->DisplayRecs == 5) { ?> selected<?php } ?>>5</option>
<option value="10"<?php if ($inventory_staysafe_report_list->DisplayRecs == 10) { ?> selected<?php } ?>>10</option>
<option value="15"<?php if ($inventory_staysafe_report_list->DisplayRecs == 15) { ?> selected<?php } ?>>15</option>
<option value="20"<?php if ($inventory_staysafe_report_list->DisplayRecs == 20) { ?> selected<?php } ?>>20</option>
<option value="50"<?php if ($inventory_staysafe_report_list->DisplayRecs == 50) { ?> selected<?php } ?>>50</option>
<option value="ALL"<?php if ($inventory_staysafe_report->getRecordsPerPage() == -1) { ?> selected<?php } ?>><?php echo $Language->Phrase("AllRecords") ?></option>
</select>
</div>
<?php } ?>
</form>
...
<?php
	foreach ($inventory_staysafe_report_list->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<div class="clearfix"></div>
...
<?php } ?>
<form name="finventory_staysafe_reportlist" id="finventory_staysafe_reportlist" class="form-inline ewForm ewListForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($inventory_staysafe_report_list->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $inventory_staysafe_report_list->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="inventory_staysafe_report">
<div id="gmp_inventory_staysafe_report" class="<?php if (ew_IsResponsiveLayout()) { ?>table-responsive <?php } ?>ewGridMiddlePanel">
<?php if ($inventory_staysafe_report_list->TotalRecs > 0 || $inventory_staysafe_report->CurrentAction == "gridedit") { ?>
<table id="tbl_inventory_staysafe_reportlist" class="table ewTable">
<thead>
	<tr class="ewTableHeader">
<?php

// Header row
$inventory_staysafe_report_list->RowType = EW_ROWTYPE_HEADER;

// Render list options
$inventory_staysafe_report_list->RenderListOptions();

// Render list options (header, left)
$inventory_staysafe_report_list->ListOptions->Render("header", "left");
?>
<?php if ($inventory_staysafe_report->id->Visible) { // id ?>
	<?php if ($inventory_staysafe_report->SortUrl($inventory_staysafe_report->id) == "") { ?>
		<th data-name="id" class="<?php echo $inventory_staysafe_report->id->HeaderCellClass() ?>"><div id="elh_inventory_staysafe_report_id" class="inventory_staysafe_report_id"><div class="ewTableHeaderCaption"><?php echo $inventory_staysafe_report->id->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="id" class="<?php echo $inventory_staysafe_report->id->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $inventory_staysafe_report->SortUrl($inventory_staysafe_report->id) ?>',1);"><div id="elh_inventory_staysafe_report_id" class="inventory_staysafe_report_id">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $inventory_staysafe_report->id->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($inventory_staysafe_report->id->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($inventory_staysafe_report->id->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($inventory_staysafe_report->date_recieved->Visible) { // date_recieved ?>
	<?php if ($inventory_staysafe_report->SortUrl($inventory_staysafe_report->date_recieved) == "") { ?>
		<th data-name="date_recieved" class="<?php echo $inventory_staysafe_report->date_recieved->HeaderCellClass() ?>"><div id="elh_inventory_staysafe_report_date_recieved" class="inventory_staysafe_report_date_recieved"><div class="ewTableHeaderCaption"><?php echo $inventory_staysafe_report->date_recieved->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="date_recieved" class="<?php echo $inventory_staysafe_report->date_recieved->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $inventory_staysafe_report->SortUrl($inventory_staysafe_report->date_recieved) ?>',1);"><div id="elh_inventory_staysafe_report_date_recieved" class="inventory_staysafe_report_date_recieved">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $inventory_staysafe_report->date_recieved->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($inventory_staysafe_report->date_recieved->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($inventory_staysafe_report->date_recieved->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($inventory_staysafe_report->reference_id->Visible) { // reference_id ?>
	<?php if ($inventory_staysafe_report->SortUrl($inventory_staysafe_report->reference_id) == "") { ?>
		<th data-name="reference_id" class="<?php echo $inventory_staysafe_report->reference_id->HeaderCellClass() ?>"><div id="elh_inventory_staysafe_report_reference_id" class="inventory_staysafe_report_reference_id"><div class="ewTableHeaderCaption"><?php echo $inventory_staysafe_report->reference_id->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="reference_id" class="<?php echo $inventory_staysafe_report->reference_id->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $inventory_staysafe_report->SortUrl($inventory_staysafe_report->reference_id) ?>',1);"><div id="elh_inventory_staysafe_report_reference_id" class="inventory_staysafe_report_reference_id">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $inventory_staysafe_report->reference_id->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($inventory_staysafe_report->reference_id->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($inventory_staysafe_report->reference_id->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($inventory_staysafe_report->material_name->Visible) { // material_name ?>
	<?php if ($inventory_staysafe_report->SortUrl($inventory_staysafe_report->material_name) == "") { ?>
		<th data-name="material_name" class="<?php echo $inventory_staysafe_report->material_name->HeaderCellClass() ?>"><div id="elh_inventory_staysafe_report_material_name" class="inventory_staysafe_report_material_name"><div class="ewTableHeaderCaption"><?php echo $inventory_staysafe_report->material_name->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="material_name" class="<?php echo $inventory_staysafe_report->material_name->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $inventory_staysafe_report->SortUrl($inventory_staysafe_report->material_name) ?>',1);"><div id="elh_inventory_staysafe_report_material_name" class="inventory_staysafe_report_material_name">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $inventory_staysafe_report->material_name->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($inventory_staysafe_report->material_name->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($inventory_staysafe_report->material_name->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($inventory_staysafe_report->type->Visible) { // type ?>
	<?php if ($inventory_staysafe_report->SortUrl($inventory_staysafe_report->type) == "") { ?>
		<th data-name="type" class="<?php echo $inventory_staysafe_report->type->HeaderCellClass() ?>"><div id="elh_inventory_staysafe_report_type" class="inventory_staysafe_report_type"><div class="ewTableHeaderCaption"><?php echo $inventory_staysafe_report->type->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="type" class="<?php echo $inventory_staysafe_report->type->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $inventory_staysafe_report->SortUrl($inventory_staysafe_report->type) ?>',1);"><div id="elh_inventory_staysafe_report_type" class="inventory_staysafe_report_type">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $inventory_staysafe_report->type->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($inventory_staysafe_report->type->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($inventory_staysafe_report->type->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($inventory_staysafe_report->capacity->Visible) { // capacity ?>
	<?php if ($inventory_staysafe_report->SortUrl($inventory_staysafe_report->capacity) == "") { ?>
		<th data-name="capacity" class="<?php echo $inventory_staysafe_report->capacity->HeaderCellClass() ?>"><div id="elh_inventory_staysafe_report_capacity" class="inventory_staysafe_report_capacity"><div class="ewTableHeaderCaption"><?php echo $inventory_staysafe_report->capacity->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="capacity" class="<?php echo $inventory_staysafe_report->capacity->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $inventory_staysafe_report->SortUrl($inventory_staysafe_report->capacity) ?>',1);"><div id="elh_inventory_staysafe_report_capacity" class="inventory_staysafe_report_capacity">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $inventory_staysafe_report->capacity->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($inventory_staysafe_report->capacity->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($inventory_staysafe_report->capacity->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($inventory_staysafe_report->quantity->Visible) { // quantity ?>
	<?php if ($inventory_staysafe_report->SortUrl($inventory_staysafe_report->quantity) == "") { ?>
		<th data-name="quantity" class="<?php echo $inventory_staysafe_report->quantity->HeaderCellClass() ?>"><div id="elh_inventory_staysafe_report_quantity" class="inventory_staysafe_report_quantity"><div class="ewTableHeaderCaption"><?php echo $inventory_staysafe_report->quantity->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="quantity" class="<?php echo $inventory_staysafe_report->quantity->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $inventory_staysafe_report->SortUrl($inventory_staysafe_report->quantity) ?>',1);"><div id="elh_inventory_staysafe_report_quantity" class="inventory_staysafe_report_quantity">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $inventory_staysafe_report->quantity->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($inventory_staysafe_report->quantity->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($inventory_staysafe_report->quantity->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($inventory_staysafe_report->statuss->Visible) { // statuss ?>
	<?php if ($inventory_staysafe_report->SortUrl($inventory_staysafe_report->statuss) == "") { ?>
		<th data-name="statuss" class="<?php echo $inventory_staysafe_report->statuss->HeaderCellClass() ?>"><div id="elh_inventory_staysafe_report_statuss" class="inventory_staysafe_report_statuss"><div class="ewTableHeaderCaption"><?php echo $inventory_staysafe_report->statuss->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="statuss" class="<?php echo $inventory_staysafe_report->statuss->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $inventory_staysafe_report->SortUrl($inventory_staysafe_report->statuss) ?>',1);"><div id="elh_inventory_staysafe_report_statuss" class="inventory_staysafe_report_statuss">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $inventory_staysafe_report->statuss->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($inventory_staysafe_report->statuss->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($inventory_staysafe_report->statuss->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($inventory_staysafe_report->approved_by->Visible) { // approved_by ?>
	<?php if ($inventory_staysafe_report->SortUrl($inventory_staysafe_report->approved_by) == "") { ?>
		<th data-name="approved_by" class="<?php echo $inventory_staysafe_report->approved_by->HeaderCellClass() ?>"><div id="elh_inventory_staysafe_report_approved_by" class="inventory_staysafe_report_approved_by"><div class="ewTableHeaderCaption"><?php echo $inventory_staysafe_report->approved_by->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="approved_by" class="<?php echo $inventory_staysafe_report->approved_by->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $inventory_staysafe_report->SortUrl($inventory_staysafe_report->approved_by) ?>',1);"><div id="elh_inventory_staysafe_report_approved_by" class="inventory_staysafe_report_approved_by">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $inventory_staysafe_report->approved_by->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($inventory_staysafe_report->approved_by->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($inventory_staysafe_report->approved_by->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($inventory_staysafe_report->verified_by->Visible) { // verified_by ?>
	<?php if ($inventory_staysafe_report->SortUrl($inventory_staysafe_report->verified_by) == "") { ?>
		<th data-name="verified_by" class="<?php echo $inventory_staysafe_report->verified_by->HeaderCellClass() ?>"><div id="elh_inventory_staysafe_report_verified_by" class="inventory_staysafe_report_verified_by"><div class="ewTableHeaderCaption"><?php echo $inventory_staysafe_report->verified_by->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="verified_by" class="<?php echo $inventory_staysafe_report->verified_by->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $inventory_staysafe_report->SortUrl($inventory_staysafe_report->verified_by) ?>',1);"><div id="elh_inventory_staysafe_report_verified_by" class="inventory_staysafe_report_verified_by">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $inventory_staysafe_report->verified_by->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($inventory_staysafe_report->verified_by->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($inventory_staysafe_report->verified_by->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php
...
$inventory_staysafe_report_list->ListOptions->Render("header", "right");
?>
	</tr>
</thead>
<tbody>
<?php
if ($inventory_staysafe_report->ExportAll && $inventory_staysafe_report->Export <> "") {
	$inventory_staysafe_report_list->StopRec = $inventory_staysafe_report_list->TotalRecs;
} else {

	// Set the last record to display
	if ($inventory_staysafe_report_list->TotalRecs > $inventory_staysafe_report_list->StartRec + $inventory_staysafe_report_list->DisplayRecs - 1)
		$inventory_staysafe_report_list->StopRec = $inventory_staysafe_report_list->StartRec + $inventory_staysafe_report_list->DisplayRecs - 1;
	else
		$inventory_staysafe_report_list->StopRec = $inventory_staysafe_report_list->TotalRecs;
}
$inventory_staysafe_report_list->RecCnt = $inventory_staysafe_report_list->StartRec - 1;
if ($inventory_staysafe_report_list->Recordset && !$inventory_staysafe_report_list->Recordset->EOF) {
	$inventory_staysafe_report_list->Recordset->MoveFirst();
	$bSelectLimit = $inventory_staysafe_report_list->UseSelectLimit;
	if (!$bSelectLimit && $inventory_staysafe_report_list->StartRec > 1)
		$inventory_staysafe_report_list->Recordset->Move($inventory_staysafe_report_list->StartRec - 1);
} elseif (!$inventory_staysafe_report->AllowAddDeleteRow && $inventory_staysafe_report_list->StopRec == 0) {
	$inventory_staysafe_report_list->StopRec = $inventory_staysafe_report->GridAddRowCount;
}

// Initialize aggregate
$inventory_staysafe_report->RowType = EW_ROWTYPE_AGGREGATEINIT;
$inventory_staysafe_report->ResetAttrs();
$inventory_staysafe_report_list->RenderRow();
while ($inventory_staysafe_report_list->RecCnt < $inventory_staysafe_report_list->StopRec) {
	$inventory_staysafe_report_list->RecCnt++;
	if (intval($inventory_staysafe_report_list->RecCnt) >= intval($inventory_staysafe_report_list->StartRec)) {
		$inventory_staysafe_report_list->RowCnt++;

		// Set up key count
		$inventory_staysafe_report_list->KeyCount = $inventory_staysafe_report_list->RowIndex;

		// Init row class and style
		$inventory_staysafe_report->ResetAttrs();
		$inventory_staysafe_report->CssClass = "";
		if ($inventory_staysafe_report->CurrentAction == "gridadd") {
		} else {
			$inventory_staysafe_report_list->LoadRowValues($inventory_staysafe_report_list->Recordset); // Load row values
		}
		$inventory_staysafe_report->RowType = EW_ROWTYPE_VIEW; // Render view

		// Set up row id / data-rowindex
		$inventory_staysafe_report->RowAttrs = array_merge($inventory_staysafe_report->RowAttrs, array('data-rowindex'=>$inventory_staysafe_report_list->RowCnt, 'id'=>'r' . $inventory_staysafe_report_list->RowCnt . '_inventory_staysafe_report', 'data-rowtype'=>$inventory_staysafe_report->RowType));
		...
		$inventory_staysafe_report_list->RenderListOptions();
?>
	<tr<?php echo $inventory_staysafe_report->RowAttributes() ?>>
<?php

// Render list options (body, left)
$inventory_staysafe_report_list->ListOptions->Render("body", "left", $inventory_staysafe_report_list->RowCnt);
?>
	<?php if ($inventory_staysafe_report->id->Visible) { // id ?>
		<td data-name="id"<?php echo $inventory_staysafe_report->id->CellAttributes() ?>>
<span id="el<?php echo $inventory_staysafe_report_list->RowCnt ?>_inventory_staysafe_report_id" class="inventory_staysafe_report_id">
<span<?php echo $inventory_staysafe_report->id->ViewAttributes() ?>>
<?php echo $inventory_staysafe_report->id->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($inventory_staysafe_report->date_recieved->Visible) { // date_recieved ?>
		<td data-name="date_recieved"<?php echo $inventory_staysafe_report->date_recieved->CellAttributes() ?>>
<span id="el<?php echo $inventory_staysafe_report_list->RowCnt ?>_inventory_staysafe_report_date_recieved" class="inventory_staysafe_report_date_recieved">
<span<?php echo $inventory_staysafe_report->date_recieved->ViewAttributes() ?>>
<?php echo $inventory_staysafe_report->date_recieved->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($inventory_staysafe_report->reference_id->Visible) { // reference_id ?>
		<td data-name="reference_id"<?php echo $inventory_staysafe_report->reference_id->CellAttributes() ?>>
<span id="el<?php echo $inventory_staysafe_report_list->RowCnt ?>_inventory_staysafe_report_reference_id" class="inventory_staysafe_report_reference_id">
<span<?php echo $inventory_staysafe_report->reference_id->ViewAttributes() ?>>
<?php echo $inventory_staysafe_report->reference_id->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($inventory_staysafe_report->material_name->Visible) { // material_name ?>
		<td data-name="material_name"<?php echo $inventory_staysafe_report->material_name->CellAttributes() ?>>
<span id="el<?php echo $inventory_staysafe_report_list->RowCnt ?>_inventory_staysafe_report_material_name" class="inventory_staysafe_report_material_name">
<span<?php echo $inventory_staysafe_report->material_name->ViewAttributes() ?>>
<?php echo $inventory_staysafe_report->material_name->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($inventory_staysafe_report->type->Visible) { // type ?>
		<td data-name="type"<?php echo $inventory_staysafe_report->type->CellAttributes() ?>>
<span id="el<?php echo $inventory_staysafe_report_list->RowCnt ?>_inventory_staysafe_report_type" class="inventory_staysafe_report_type">
<span<?php echo $inventory_staysafe_report->type->ViewAttributes() ?>>
<?php echo $inventory_staysafe_report->type->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($inventory_staysafe_report->capacity->Visible) { // capacity ?>
		<td data-name="capacity"<?php echo $inventory_staysafe_report->capacity->CellAttributes() ?>>
<span id="el<?php echo $inventory_staysafe_report_list->RowCnt ?>_inventory_staysafe_report_capacity" class="inventory_staysafe_report_capacity">
<span<?php echo $inventory_staysafe_report->capacity->ViewAttributes() ?>>
<?php echo $inventory_staysafe_report->capacity->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($inventory_staysafe_report->quantity->Visible) { // quantity ?>
		<td data-name="quantity"<?php echo $inventory_staysafe_report->quantity->CellAttributes() ?>>
<span id="el<?php echo $inventory_staysafe_report_list->RowCnt ?>_inventory_staysafe_report_quantity" class="inventory_staysafe_report_quantity">
<span<?php echo $inventory_staysafe_report->quantity->ViewAttributes() ?>>
<?php echo $inventory_staysafe_report->quantity->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($inventory_staysafe_report->statuss->Visible) { // statuss ?>
		<td data-name="statuss"<?php echo $inventory_staysafe_report->statuss->CellAttributes() ?>>
<span id="el<?php echo $inventory_staysafe_report_list->RowCnt ?>_inventory_staysafe_report_statuss" class="inventory_staysafe_report_statuss">
<span<?php echo $inventory_staysafe_report->statuss->ViewAttributes() ?>>
<?php echo $inventory_staysafe_report->statuss->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($inventory_staysafe_report->approved_by->Visible) { // approved_by ?>
		<td data-name="approved_by"<?php echo $inventory_staysafe_report->approved_by->CellAttributes() ?>>
<span id="el<?php echo $inventory_staysafe_report_list->RowCnt ?>_inventory_staysafe_report_approved_by" class="inventory_staysafe_report_approved_by">
<span<?php echo $inventory_staysafe_report->approved_by->ViewAttributes() ?>>
<?php echo $inventory_staysafe_report->approved_by->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($inventory_staysafe_report->verified_by->Visible) { // verified_by ?>
		<td data-name="verified_by"<?php echo $inventory_staysafe_report->verified_by->CellAttributes() ?>>
<span id="el<?php echo $inventory_staysafe_report_list->RowCnt ?>_inventory_staysafe_report_verified_by" class="inventory_staysafe_report_verified_by">
<span<?php echo $inventory_staysafe_report->verified_by->ViewAttributes() ?>>
<?php echo $inventory_staysafe_report->verified_by->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
<?php
...
$inventory_staysafe_report_list->ListOptions->Render("body", "right", $inventory_staysafe_report_list->RowCnt);
?>
	</tr>
<?php
	}
	if ($inventory_staysafe_report->CurrentAction <> "gridadd")
		$inventory_staysafe_report_list->Recordset->MoveNext();
}
?>
</tbody>
</table>
<?php } ?>
<?php if ($inventory_staysafe_report->CurrentAction == "") { ?>
...
// Close recordset
if ($inventory_staysafe_report_list->Recordset)
	$inventory_staysafe_report_list->Recordset->Close();
?>
</div>
<?php } ?>
<?php if ($inventory_staysafe_report_list->TotalRecs == 0 && $inventory_staysafe_report->CurrentAction == "") { // Show other options ?>
<div class="ewListOtherOptions">
<?php
	foreach ($inventory_staysafe_report_list->OtherOptions as &$option) {
		$option->ButtonClass = "";
...
<?php if ($inventory_staysafe_report->Export == "") { ?>
<script type="text/javascript">
finventory_staysafe_reportlistsrch.FilterList = <?php echo $inventory_staysafe_report_list->GetFilterList() ?>;
finventory_staysafe_reportlistsrch.Init();
finventory_staysafe_reportlist.Init();
</script>
<?php } ?>
<?php
$inventory_staysafe_report_list->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($inventory_staysafe_report->Export == "") { ?>
...
<?php
$inventory_staysafe_report_list->Page_Terminate();
?>
