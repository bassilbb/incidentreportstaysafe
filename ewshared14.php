...
if (IsLoggedIn()) {
		$MyReportCount = 0;
			...
			if (CurrentUserLevel() == 3) {
				$_SESSION['MyReportCount'] = ew_ExecuteScalar("SELECT COUNT(id) FROM `report_form` WHERE `status` in (3,6)");
			}
			/*if (CurrentUserLevel() == 3) {
			...
			if (CurrentUserLevel() == 4) {
				$_SESSION['MyReportCount'] = ew_ExecuteScalar("SELECT COUNT(id) FROM `report_form` WHERE (`status` in (3,6,7)AND `branch` = '".$_SESSION['Branch']."')OR (`status` in (4) AND `assign` = '".$_SESSION['Staff_ID']."') OR (`status` in (0,1) AND `staff_id` = '".$_SESSION['Staff_ID']."')");
			}
