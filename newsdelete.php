	...
	function DeleteRows() {
		...
		if ($DeleteRows) {
			...
			foreach ($rsold as $row) {
				...
				$OldFiles = ew_Empty($row['news_image']) ? array() : explode(EW_MULTIPLE_UPLOAD_SEPARATOR, $row['news_image']);
				$OldFileCount = count($OldFiles);
				for ($i = 0; $i < $OldFileCount; $i++) {
					if (file_exists($this->news_image->OldPhysicalUploadPath() . $OldFiles[$i]))
...
<?php if ($news->news->Visible) { // news ?>
		<th class="<?php echo $news->news->HeaderCellClass() ?>"><span id="elh_news_news" class="news_news"><?php echo $news->news->FldCaption() ?></span></th>
<?php } ?>
<?php if ($news->news_image->Visible) { // news_image ?>
		<th class="<?php echo $news->news_image->HeaderCellClass() ?>"><span id="elh_news_news_image" class="news_news_image"><?php echo $news->news_image->FldCaption() ?></span></th>
<?php } ?>
<?php if ($news->news_content->Visible) { // news_content ?>
		<th class="<?php echo $news->news_content->HeaderCellClass() ?>"><span id="elh_news_news_content" class="news_news_content"><?php echo $news->news_content->FldCaption() ?></span></th>
<?php } ?>
	</tr>
	</thead>
	<tbody>
...
<?php if ($news->news->Visible) { // news ?>
		<td<?php echo $news->news->CellAttributes() ?>>
<span id="el<?php echo $news_delete->RowCnt ?>_news_news" class="news_news">
...
<?php } ?>
<?php if ($news->news_image->Visible) { // news_image ?>
		<td<?php echo $news->news_image->CellAttributes() ?>>
<span id="el<?php echo $news_delete->RowCnt ?>_news_news_image" class="news_news_image">
<span>
...
<?php } ?>
<?php if ($news->news_content->Visible) { // news_content ?>
		<td<?php echo $news->news_content->CellAttributes() ?>>
<span id="el<?php echo $news_delete->RowCnt ?>_news_news_content" class="news_news_content">
