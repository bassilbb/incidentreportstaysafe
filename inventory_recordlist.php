...
<?php include_once "inventory_recordinfo.php" ?>
...
$inventory_record_list = NULL; // Initialize page object first

class cinventory_record_list extends cinventory_record {
	...
	// Table name
	var $TableName = 'inventory_record';

	// Page object name
	var $PageObjName = 'inventory_record_list';

	// Grid form hidden field names
	var $FormName = 'finventory_recordlist';
	...
	function __construct() {
		...
		// Table object (inventory_record)
		if (!isset($GLOBALS["inventory_record"]) || get_class($GLOBALS["inventory_record"]) == "cinventory_record") {
			$GLOBALS["inventory_record"] = &$this;
			$GLOBALS["Table"] = &$GLOBALS["inventory_record"];
		}
		...
		$this->AddUrl = "inventory_recordadd.php";
		$this->InlineAddUrl = $this->PageUrl() . "a=add";
		$this->GridAddUrl = $this->PageUrl() . "a=gridadd";
		$this->GridEditUrl = $this->PageUrl() . "a=gridedit";
		$this->MultiDeleteUrl = "inventory_recorddelete.php";
		$this->MultiUpdateUrl = "inventory_recordupdate.php";
		...
		if (!defined("EW_TABLE_NAME"))
			define("EW_TABLE_NAME", 'inventory_record', TRUE);
		...
		$this->FilterOptions->TagClassName = "ewFilterOption finventory_recordlistsrch";
	...
	function Page_Terminate($url = "") {
		...
		// Export
		global $EW_EXPORT, $inventory_record;
		if ($this->CustomExport <> "" && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, $EW_EXPORT)) {
				$sContent = ob_get_contents();
			if ($gsExportFile == "") $gsExportFile = $this->TableVar;
			$class = $EW_EXPORT[$this->CustomExport];
			if (class_exists($class)) {
				$doc = new $class($inventory_record);
				$doc->Text = $sContent;
	...
	function GetFilterList() {
		...
		if (EW_SEARCH_FILTER_OPTION == "Server" && isset($UserProfile))
			$sSavedFilterList = $UserProfile->GetSearchFilters(CurrentUserName(), "finventory_recordlistsrch");
	...
	function ProcessFilterList() {
		...
		if (@$_POST["ajax"] == "savefilters") { // Save filter request (Ajax)
			...
			$UserProfile->SetSearchFilters(CurrentUserName(), "finventory_recordlistsrch", $filters);
	...
	function SetupOtherOptions() {
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"finventory_recordlistsrch\" href=\"#\">" . $Language->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"finventory_recordlistsrch\" href=\"#\">" . $Language->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->ListActions->Items as $listaction) {
				if ($listaction->Select == EW_ACTION_MULTIPLE) {
					...
					$item->Body = "<a class=\"ewAction ewListAction\" title=\"" . ew_HtmlEncode($caption) . "\" data-caption=\"" . ew_HtmlEncode($caption) . "\" href=\"\" onclick=\"ew_SubmitAction(event,jQuery.extend({f:document.finventory_recordlist}," . $listaction->ToJson(TRUE) . "));return false;\">" . $icon . "</a>";
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $Language->Phrase("SearchPanel") . "\" data-caption=\"" . $Language->Phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"finventory_recordlistsrch\">" . $Language->Phrase("SearchLink") . "</button>";
		$item->Visible = TRUE;
	...
	function SetupExportOptions() {
		...
		$url = "";
		$item->Body = "<button id=\"emf_inventory_record\" class=\"ewExportLink ewEmail\" title=\"" . $Language->Phrase("ExportToEmailText") . "\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_inventory_record',hdr:ewLanguage.Phrase('ExportToEmailText'),f:document.finventory_recordlist,sel:false" . $url . "});\">" . $Language->Phrase("ExportToEmail") . "</button>";
		$item->Visible = FALSE;
...
// Create page object
if (!isset($inventory_record_list)) $inventory_record_list = new cinventory_record_list();

// Page init
$inventory_record_list->Page_Init();

// Page main
$inventory_record_list->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$inventory_record_list->Page_Render();
?>
<?php include_once "header.php" ?>
<?php if ($inventory_record->Export == "") { ?>
<script type="text/javascript">

// Form object
var CurrentPageID = EW_PAGE_ID = "list";
var CurrentForm = finventory_recordlist = new ew_Form("finventory_recordlist", "list");
finventory_recordlist.FormKeyCountName = '<?php echo $inventory_record_list->FormKeyCountName ?>';

// Form_CustomValidate event
finventory_recordlist.Form_CustomValidate = 
...
finventory_recordlist.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
finventory_recordlist.Lists["x_recieved_by"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_firstname","x_lastname","x_staffno",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"users"};
finventory_recordlist.Lists["x_recieved_by"].Data = "<?php echo $inventory_record_list->recieved_by->LookupFilterQuery(FALSE, "list") ?>";
...
<?php if ($inventory_record->Export == "") { ?>
<div class="ewToolbar">
<?php if ($inventory_record_list->TotalRecs > 0 && $inventory_record_list->ExportOptions->Visible()) { ?>
<?php $inventory_record_list->ExportOptions->Render("body") ?>
<?php } ?>
<?php if ($inventory_record_list->SearchOptions->Visible()) { ?>
<?php $inventory_record_list->SearchOptions->Render("body") ?>
<?php } ?>
<?php if ($inventory_record_list->FilterOptions->Visible()) { ?>
<?php $inventory_record_list->FilterOptions->Render("body") ?>
<?php } ?>
<div class="clearfix"></div>
</div>
<?php } ?>
<?php
	$bSelectLimit = $inventory_record_list->UseSelectLimit;
	if ($bSelectLimit) {
		if ($inventory_record_list->TotalRecs <= 0)
			$inventory_record_list->TotalRecs = $inventory_record->ListRecordCount();
	} else {
		if (!$inventory_record_list->Recordset && ($inventory_record_list->Recordset = $inventory_record_list->LoadRecordset()))
			$inventory_record_list->TotalRecs = $inventory_record_list->Recordset->RecordCount();
	}
	$inventory_record_list->StartRec = 1;
	if ($inventory_record_list->DisplayRecs <= 0 || ($inventory_record->Export <> "" && $inventory_record->ExportAll)) // Display all records
		$inventory_record_list->DisplayRecs = $inventory_record_list->TotalRecs;
	if (!($inventory_record->Export <> "" && $inventory_record->ExportAll))
		$inventory_record_list->SetupStartRec(); // Set up start record position
	if ($bSelectLimit)
		$inventory_record_list->Recordset = $inventory_record_list->LoadRecordset($inventory_record_list->StartRec-1, $inventory_record_list->DisplayRecs);

	// Set no record found message
	if ($inventory_record->CurrentAction == "" && $inventory_record_list->TotalRecs == 0) {
		if (!$Security->CanList())
			$inventory_record_list->setWarningMessage(ew_DeniedMsg());
		if ($inventory_record_list->SearchWhere == "0=101")
			$inventory_record_list->setWarningMessage($Language->Phrase("EnterSearchCriteria"));
		else
			$inventory_record_list->setWarningMessage($Language->Phrase("NoRecord"));
	}
$inventory_record_list->RenderOtherOptions();
?>
<?php if ($Security->CanSearch()) { ?>
<?php if ($inventory_record->Export == "" && $inventory_record->CurrentAction == "") { ?>
<form name="finventory_recordlistsrch" id="finventory_recordlistsrch" class="form-inline ewForm ewExtSearchForm" action="<?php echo ew_CurrentPage() ?>">
<?php $SearchPanelClass = ($inventory_record_list->SearchWhere <> "") ? " in" : " in"; ?>
<div id="finventory_recordlistsrch_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
<input type="hidden" name="cmd" value="search">
<input type="hidden" name="t" value="inventory_record">
	<div class="ewBasicSearch">
<div id="xsr_1" class="ewRow">
	<div class="ewQuickSearch input-group">
	<input type="text" name="<?php echo EW_TABLE_BASIC_SEARCH ?>" id="<?php echo EW_TABLE_BASIC_SEARCH ?>" class="form-control" value="<?php echo ew_HtmlEncode($inventory_record_list->BasicSearch->getKeyword()) ?>" placeholder="<?php echo ew_HtmlEncode($Language->Phrase("Search")) ?>">
	<input type="hidden" name="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" id="<?php echo EW_TABLE_BASIC_SEARCH_TYPE ?>" value="<?php echo ew_HtmlEncode($inventory_record_list->BasicSearch->getType()) ?>">
	<div class="input-group-btn">
		<button type="button" data-toggle="dropdown" class="btn btn-default"><span id="searchtype"><?php echo $inventory_record_list->BasicSearch->getTypeNameShort() ?></span><span class="caret"></span></button>
		<ul class="dropdown-menu pull-right" role="menu">
			<li<?php if ($inventory_record_list->BasicSearch->getType() == "") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this)"><?php echo $Language->Phrase("QuickSearchAuto") ?></a></li>
			<li<?php if ($inventory_record_list->BasicSearch->getType() == "=") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'=')"><?php echo $Language->Phrase("QuickSearchExact") ?></a></li>
			<li<?php if ($inventory_record_list->BasicSearch->getType() == "AND") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'AND')"><?php echo $Language->Phrase("QuickSearchAll") ?></a></li>
			<li<?php if ($inventory_record_list->BasicSearch->getType() == "OR") echo " class=\"active\""; ?>><a href="javascript:void(0);" onclick="ew_SetSearchType(this,'OR')"><?php echo $Language->Phrase("QuickSearchAny") ?></a></li>
		</ul>
...
<?php } ?>
<?php } ?>
<?php $inventory_record_list->ShowPageHeader(); ?>
<?php
$inventory_record_list->ShowMessage();
?>
<?php if ($inventory_record_list->TotalRecs > 0 || $inventory_record->CurrentAction <> "") { ?>
<div class="box ewBox ewGrid<?php if ($inventory_record_list->IsAddOrEdit()) { ?> ewGridAddEdit<?php } ?> inventory_record">
<?php if ($inventory_record->Export == "") { ?>
<div class="box-header ewGridUpperPanel">
<?php if ($inventory_record->CurrentAction <> "gridadd" && $inventory_record->CurrentAction <> "gridedit") { ?>
<form name="ewPagerForm" class="form-inline ewForm ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($inventory_record_list->Pager)) $inventory_record_list->Pager = new cPrevNextPager($inventory_record_list->StartRec, $inventory_record_list->DisplayRecs, $inventory_record_list->TotalRecs, $inventory_record_list->AutoHidePager) ?>
<?php if ($inventory_record_list->Pager->RecordCount > 0 && $inventory_record_list->Pager->Visible) { ?>
<div class="ewPager">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($inventory_record_list->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $inventory_record_list->PageUrl() ?>start=<?php echo $inventory_record_list->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($inventory_record_list->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $inventory_record_list->PageUrl() ?>start=<?php echo $inventory_record_list->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $inventory_record_list->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($inventory_record_list->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $inventory_record_list->PageUrl() ?>start=<?php echo $inventory_record_list->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($inventory_record_list->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $inventory_record_list->PageUrl() ?>start=<?php echo $inventory_record_list->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $inventory_record_list->Pager->PageCount ?></span>
</div>
<?php } ?>
<?php if ($inventory_record_list->Pager->RecordCount > 0) { ?>
<div class="ewPager ewRec">
	<span><?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $inventory_record_list->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $inventory_record_list->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $inventory_record_list->Pager->RecordCount ?></span>
</div>
<?php } ?>
<?php if ($inventory_record_list->TotalRecs > 0 && (!$inventory_record_list->AutoHidePageSizeSelector || $inventory_record_list->Pager->Visible)) { ?>
<div class="ewPager">
<input type="hidden" name="t" value="inventory_record">
<select name="<?php echo EW_TABLE_REC_PER_PAGE ?>" class="form-control input-sm ewTooltip" title="<?php echo $Language->Phrase("RecordsPerPage") ?>" onchange="this.form.submit();">
<option value="5"<?php if ($inventory_record_list->DisplayRecs == 5) { ?> selected<?php } ?>>5</option>
<option value="10"<?php if ($inventory_record_list->DisplayRecs == 10) { ?> selected<?php } ?>>10</option>
<option value="15"<?php if ($inventory_record_list->DisplayRecs == 15) { ?> selected<?php } ?>>15</option>
<option value="20"<?php if ($inventory_record_list->DisplayRecs == 20) { ?> selected<?php } ?>>20</option>
<option value="50"<?php if ($inventory_record_list->DisplayRecs == 50) { ?> selected<?php } ?>>50</option>
<option value="ALL"<?php if ($inventory_record->getRecordsPerPage() == -1) { ?> selected<?php } ?>><?php echo $Language->Phrase("AllRecords") ?></option>
</select>
</div>
<?php } ?>
</form>
...
<?php
	foreach ($inventory_record_list->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<div class="clearfix"></div>
...
<?php } ?>
<form name="finventory_recordlist" id="finventory_recordlist" class="form-inline ewForm ewListForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($inventory_record_list->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $inventory_record_list->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="inventory_record">
<div id="gmp_inventory_record" class="<?php if (ew_IsResponsiveLayout()) { ?>table-responsive <?php } ?>ewGridMiddlePanel">
<?php if ($inventory_record_list->TotalRecs > 0 || $inventory_record->CurrentAction == "gridedit") { ?>
<table id="tbl_inventory_recordlist" class="table ewTable">
<thead>
	<tr class="ewTableHeader">
<?php

// Header row
$inventory_record_list->RowType = EW_ROWTYPE_HEADER;

// Render list options
$inventory_record_list->RenderListOptions();

// Render list options (header, left)
$inventory_record_list->ListOptions->Render("header", "left");
?>
<?php if ($inventory_record->date_recieved->Visible) { // date_recieved ?>
	<?php if ($inventory_record->SortUrl($inventory_record->date_recieved) == "") { ?>
		<th data-name="date_recieved" class="<?php echo $inventory_record->date_recieved->HeaderCellClass() ?>"><div id="elh_inventory_record_date_recieved" class="inventory_record_date_recieved"><div class="ewTableHeaderCaption"><?php echo $inventory_record->date_recieved->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="date_recieved" class="<?php echo $inventory_record->date_recieved->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $inventory_record->SortUrl($inventory_record->date_recieved) ?>',1);"><div id="elh_inventory_record_date_recieved" class="inventory_record_date_recieved">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $inventory_record->date_recieved->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($inventory_record->date_recieved->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($inventory_record->date_recieved->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($inventory_record->reference_id->Visible) { // reference_id ?>
	<?php if ($inventory_record->SortUrl($inventory_record->reference_id) == "") { ?>
		<th data-name="reference_id" class="<?php echo $inventory_record->reference_id->HeaderCellClass() ?>"><div id="elh_inventory_record_reference_id" class="inventory_record_reference_id"><div class="ewTableHeaderCaption"><?php echo $inventory_record->reference_id->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="reference_id" class="<?php echo $inventory_record->reference_id->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $inventory_record->SortUrl($inventory_record->reference_id) ?>',1);"><div id="elh_inventory_record_reference_id" class="inventory_record_reference_id">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $inventory_record->reference_id->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($inventory_record->reference_id->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($inventory_record->reference_id->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($inventory_record->material_name->Visible) { // material_name ?>
	<?php if ($inventory_record->SortUrl($inventory_record->material_name) == "") { ?>
		<th data-name="material_name" class="<?php echo $inventory_record->material_name->HeaderCellClass() ?>"><div id="elh_inventory_record_material_name" class="inventory_record_material_name"><div class="ewTableHeaderCaption"><?php echo $inventory_record->material_name->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="material_name" class="<?php echo $inventory_record->material_name->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $inventory_record->SortUrl($inventory_record->material_name) ?>',1);"><div id="elh_inventory_record_material_name" class="inventory_record_material_name">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $inventory_record->material_name->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($inventory_record->material_name->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($inventory_record->material_name->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($inventory_record->quantity->Visible) { // quantity ?>
	<?php if ($inventory_record->SortUrl($inventory_record->quantity) == "") { ?>
		<th data-name="quantity" class="<?php echo $inventory_record->quantity->HeaderCellClass() ?>"><div id="elh_inventory_record_quantity" class="inventory_record_quantity"><div class="ewTableHeaderCaption"><?php echo $inventory_record->quantity->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="quantity" class="<?php echo $inventory_record->quantity->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $inventory_record->SortUrl($inventory_record->quantity) ?>',1);"><div id="elh_inventory_record_quantity" class="inventory_record_quantity">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $inventory_record->quantity->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($inventory_record->quantity->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($inventory_record->quantity->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($inventory_record->type->Visible) { // type ?>
	<?php if ($inventory_record->SortUrl($inventory_record->type) == "") { ?>
		<th data-name="type" class="<?php echo $inventory_record->type->HeaderCellClass() ?>"><div id="elh_inventory_record_type" class="inventory_record_type"><div class="ewTableHeaderCaption"><?php echo $inventory_record->type->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="type" class="<?php echo $inventory_record->type->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $inventory_record->SortUrl($inventory_record->type) ?>',1);"><div id="elh_inventory_record_type" class="inventory_record_type">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $inventory_record->type->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($inventory_record->type->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($inventory_record->type->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($inventory_record->capacity->Visible) { // capacity ?>
	<?php if ($inventory_record->SortUrl($inventory_record->capacity) == "") { ?>
		<th data-name="capacity" class="<?php echo $inventory_record->capacity->HeaderCellClass() ?>"><div id="elh_inventory_record_capacity" class="inventory_record_capacity"><div class="ewTableHeaderCaption"><?php echo $inventory_record->capacity->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="capacity" class="<?php echo $inventory_record->capacity->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $inventory_record->SortUrl($inventory_record->capacity) ?>',1);"><div id="elh_inventory_record_capacity" class="inventory_record_capacity">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $inventory_record->capacity->FldCaption() ?><?php echo $Language->Phrase("SrchLegend") ?></span><span class="ewTableHeaderSort"><?php if ($inventory_record->capacity->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($inventory_record->capacity->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($inventory_record->recieved_by->Visible) { // recieved_by ?>
	<?php if ($inventory_record->SortUrl($inventory_record->recieved_by) == "") { ?>
		<th data-name="recieved_by" class="<?php echo $inventory_record->recieved_by->HeaderCellClass() ?>"><div id="elh_inventory_record_recieved_by" class="inventory_record_recieved_by"><div class="ewTableHeaderCaption"><?php echo $inventory_record->recieved_by->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="recieved_by" class="<?php echo $inventory_record->recieved_by->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $inventory_record->SortUrl($inventory_record->recieved_by) ?>',1);"><div id="elh_inventory_record_recieved_by" class="inventory_record_recieved_by">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $inventory_record->recieved_by->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($inventory_record->recieved_by->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($inventory_record->recieved_by->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
<?php if ($inventory_record->statuss->Visible) { // statuss ?>
	<?php if ($inventory_record->SortUrl($inventory_record->statuss) == "") { ?>
		<th data-name="statuss" class="<?php echo $inventory_record->statuss->HeaderCellClass() ?>"><div id="elh_inventory_record_statuss" class="inventory_record_statuss"><div class="ewTableHeaderCaption"><?php echo $inventory_record->statuss->FldCaption() ?></div></div></th>
	<?php } else { ?>
		<th data-name="statuss" class="<?php echo $inventory_record->statuss->HeaderCellClass() ?>"><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $inventory_record->SortUrl($inventory_record->statuss) ?>',1);"><div id="elh_inventory_record_statuss" class="inventory_record_statuss">
			<div class="ewTableHeaderBtn"><span class="ewTableHeaderCaption"><?php echo $inventory_record->statuss->FldCaption() ?></span><span class="ewTableHeaderSort"><?php if ($inventory_record->statuss->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($inventory_record->statuss->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span></div>
		</div></div></th>
	<?php } ?>
<?php } ?>
...
$inventory_record_list->ListOptions->Render("header", "right");
?>
	</tr>
</thead>
<tbody>
<?php
if ($inventory_record->ExportAll && $inventory_record->Export <> "") {
	$inventory_record_list->StopRec = $inventory_record_list->TotalRecs;
} else {

	// Set the last record to display
	if ($inventory_record_list->TotalRecs > $inventory_record_list->StartRec + $inventory_record_list->DisplayRecs - 1)
		$inventory_record_list->StopRec = $inventory_record_list->StartRec + $inventory_record_list->DisplayRecs - 1;
	else
		$inventory_record_list->StopRec = $inventory_record_list->TotalRecs;
}
$inventory_record_list->RecCnt = $inventory_record_list->StartRec - 1;
if ($inventory_record_list->Recordset && !$inventory_record_list->Recordset->EOF) {
	$inventory_record_list->Recordset->MoveFirst();
	$bSelectLimit = $inventory_record_list->UseSelectLimit;
	if (!$bSelectLimit && $inventory_record_list->StartRec > 1)
		$inventory_record_list->Recordset->Move($inventory_record_list->StartRec - 1);
} elseif (!$inventory_record->AllowAddDeleteRow && $inventory_record_list->StopRec == 0) {
	$inventory_record_list->StopRec = $inventory_record->GridAddRowCount;
}

// Initialize aggregate
$inventory_record->RowType = EW_ROWTYPE_AGGREGATEINIT;
$inventory_record->ResetAttrs();
$inventory_record_list->RenderRow();
while ($inventory_record_list->RecCnt < $inventory_record_list->StopRec) {
	$inventory_record_list->RecCnt++;
	if (intval($inventory_record_list->RecCnt) >= intval($inventory_record_list->StartRec)) {
		$inventory_record_list->RowCnt++;

		// Set up key count
		$inventory_record_list->KeyCount = $inventory_record_list->RowIndex;

		// Init row class and style
		$inventory_record->ResetAttrs();
		$inventory_record->CssClass = "";
		if ($inventory_record->CurrentAction == "gridadd") {
		} else {
			$inventory_record_list->LoadRowValues($inventory_record_list->Recordset); // Load row values
		}
		$inventory_record->RowType = EW_ROWTYPE_VIEW; // Render view

		// Set up row id / data-rowindex
		$inventory_record->RowAttrs = array_merge($inventory_record->RowAttrs, array('data-rowindex'=>$inventory_record_list->RowCnt, 'id'=>'r' . $inventory_record_list->RowCnt . '_inventory_record', 'data-rowtype'=>$inventory_record->RowType));
		...
		$inventory_record_list->RenderListOptions();
?>
	<tr<?php echo $inventory_record->RowAttributes() ?>>
<?php

// Render list options (body, left)
$inventory_record_list->ListOptions->Render("body", "left", $inventory_record_list->RowCnt);
?>
	<?php if ($inventory_record->date_recieved->Visible) { // date_recieved ?>
		<td data-name="date_recieved"<?php echo $inventory_record->date_recieved->CellAttributes() ?>>
<span id="el<?php echo $inventory_record_list->RowCnt ?>_inventory_record_date_recieved" class="inventory_record_date_recieved">
<span<?php echo $inventory_record->date_recieved->ViewAttributes() ?>>
<?php echo $inventory_record->date_recieved->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($inventory_record->reference_id->Visible) { // reference_id ?>
		<td data-name="reference_id"<?php echo $inventory_record->reference_id->CellAttributes() ?>>
<span id="el<?php echo $inventory_record_list->RowCnt ?>_inventory_record_reference_id" class="inventory_record_reference_id">
<span<?php echo $inventory_record->reference_id->ViewAttributes() ?>>
<?php echo $inventory_record->reference_id->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($inventory_record->material_name->Visible) { // material_name ?>
		<td data-name="material_name"<?php echo $inventory_record->material_name->CellAttributes() ?>>
<span id="el<?php echo $inventory_record_list->RowCnt ?>_inventory_record_material_name" class="inventory_record_material_name">
<span<?php echo $inventory_record->material_name->ViewAttributes() ?>>
<?php echo $inventory_record->material_name->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($inventory_record->quantity->Visible) { // quantity ?>
		<td data-name="quantity"<?php echo $inventory_record->quantity->CellAttributes() ?>>
<span id="el<?php echo $inventory_record_list->RowCnt ?>_inventory_record_quantity" class="inventory_record_quantity">
<span<?php echo $inventory_record->quantity->ViewAttributes() ?>>
<?php echo $inventory_record->quantity->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($inventory_record->type->Visible) { // type ?>
		<td data-name="type"<?php echo $inventory_record->type->CellAttributes() ?>>
<span id="el<?php echo $inventory_record_list->RowCnt ?>_inventory_record_type" class="inventory_record_type">
<span<?php echo $inventory_record->type->ViewAttributes() ?>>
<?php echo $inventory_record->type->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($inventory_record->capacity->Visible) { // capacity ?>
		<td data-name="capacity"<?php echo $inventory_record->capacity->CellAttributes() ?>>
<span id="el<?php echo $inventory_record_list->RowCnt ?>_inventory_record_capacity" class="inventory_record_capacity">
<span<?php echo $inventory_record->capacity->ViewAttributes() ?>>
<?php echo $inventory_record->capacity->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($inventory_record->recieved_by->Visible) { // recieved_by ?>
		<td data-name="recieved_by"<?php echo $inventory_record->recieved_by->CellAttributes() ?>>
<span id="el<?php echo $inventory_record_list->RowCnt ?>_inventory_record_recieved_by" class="inventory_record_recieved_by">
<span<?php echo $inventory_record->recieved_by->ViewAttributes() ?>>
<?php echo $inventory_record->recieved_by->ListViewValue() ?></span>
</span>
</td>
	<?php } ?>
	<?php if ($inventory_record->statuss->Visible) { // statuss ?>
		<td data-name="statuss"<?php echo $inventory_record->statuss->CellAttributes() ?>>
...
$inventory_record_list->ListOptions->Render("body", "right", $inventory_record_list->RowCnt);
?>
	</tr>
<?php
	}
	if ($inventory_record->CurrentAction <> "gridadd")
		$inventory_record_list->Recordset->MoveNext();
}
?>
</tbody>
</table>
<?php } ?>
<?php if ($inventory_record->CurrentAction == "") { ?>
...
// Close recordset
if ($inventory_record_list->Recordset)
	$inventory_record_list->Recordset->Close();
?>
</div>
<?php } ?>
<?php if ($inventory_record_list->TotalRecs == 0 && $inventory_record->CurrentAction == "") { // Show other options ?>
<div class="ewListOtherOptions">
<?php
	foreach ($inventory_record_list->OtherOptions as &$option) {
		$option->ButtonClass = "";
...
<?php if ($inventory_record->Export == "") { ?>
<script type="text/javascript">
finventory_recordlistsrch.FilterList = <?php echo $inventory_record_list->GetFilterList() ?>;
finventory_recordlistsrch.Init();
finventory_recordlist.Init();
</script>
<?php } ?>
<?php
$inventory_record_list->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($inventory_record->Export == "") { ?>
...
<?php
$inventory_record_list->Page_Terminate();
?>
