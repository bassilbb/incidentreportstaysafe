...
$inventory_staysafe_report_edit = NULL; // Initialize page object first

class cinventory_staysafe_report_edit extends cinventory_staysafe_report {
	...
	// Page object name
	var $PageObjName = 'inventory_staysafe_report_edit';
	...
	function Page_Main() {
		...
		if ($this->TotalRecs <= 0) { // No record found
			...
			$this->Page_Terminate("inventory_staysafe_reportlist.php"); // Return to list page
		...
		switch ($this->CurrentAction) {
			...
			Case "U": // Update
				...
				if (ew_GetPageName($sReturnUrl) == "inventory_staysafe_reportlist.php")
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// date_recieved
			$this->date_recieved->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->date_recieved->CurrentValue, 0), NULL, $this->date_recieved->ReadOnly);
...
// Create page object
if (!isset($inventory_staysafe_report_edit)) $inventory_staysafe_report_edit = new cinventory_staysafe_report_edit();

// Page init
$inventory_staysafe_report_edit->Page_Init();

// Page main
$inventory_staysafe_report_edit->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$inventory_staysafe_report_edit->Page_Render();
?>
...
var CurrentForm = finventory_staysafe_reportedit = new ew_Form("finventory_staysafe_reportedit", "edit");

// Validate form
finventory_staysafe_reportedit.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $inventory_staysafe_report->reference_id->FldCaption(), $inventory_staysafe_report->reference_id->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_material_name");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $inventory_staysafe_report->material_name->FldCaption(), $inventory_staysafe_report->material_name->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_type");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $inventory_staysafe_report->type->FldCaption(), $inventory_staysafe_report->type->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_capacity");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $inventory_staysafe_report->capacity->FldCaption(), $inventory_staysafe_report->capacity->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_quantity");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $inventory_staysafe_report->quantity->FldCaption(), $inventory_staysafe_report->quantity->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_recieved_by");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $inventory_staysafe_report->recieved_by->FldCaption(), $inventory_staysafe_report->recieved_by->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_recieved_action");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $inventory_staysafe_report->recieved_action->FldCaption(), $inventory_staysafe_report->recieved_action->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_recieved_comment");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $inventory_staysafe_report->recieved_comment->FldCaption(), $inventory_staysafe_report->recieved_comment->ReqErrMsg)) ?>");
			...
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $inventory_staysafe_report->approver_action->FldCaption(), $inventory_staysafe_report->approver_action->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_approver_comment");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $inventory_staysafe_report->approver_comment->FldCaption(), $inventory_staysafe_report->approver_comment->ReqErrMsg)) ?>");
			...
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $inventory_staysafe_report->verified_action->FldCaption(), $inventory_staysafe_report->verified_action->ReqErrMsg)) ?>");
			elm = this.GetElements("x" + infix + "_verified_comment");
			if (elm && !ew_IsHidden(elm) && !ew_HasValue(elm))
				return this.OnError(elm, "<?php echo ew_JsEncode2(str_replace("%s", $inventory_staysafe_report->verified_comment->FldCaption(), $inventory_staysafe_report->verified_comment->ReqErrMsg)) ?>");
...
finventory_staysafe_reportedit.Form_CustomValidate = 
...
finventory_staysafe_reportedit.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;

// Dynamic selection lists
finventory_staysafe_reportedit.Lists["x_material_name"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_material_name","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"inventory_staysafe"};
finventory_staysafe_reportedit.Lists["x_material_name"].Data = "<?php echo $inventory_staysafe_report_edit->material_name->LookupFilterQuery(FALSE, "edit") ?>";
finventory_staysafe_reportedit.AutoSuggests["x_material_name"] = <?php echo json_encode(array("data" => "ajax=autosuggest&" . $inventory_staysafe_report_edit->material_name->LookupFilterQuery(TRUE, "edit"))) ?>;
finventory_staysafe_reportedit.Lists["x_recieved_by"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_firstname","x_lastname","x_staffno",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"users"};
finventory_staysafe_reportedit.Lists["x_recieved_by"].Data = "<?php echo $inventory_staysafe_report_edit->recieved_by->LookupFilterQuery(FALSE, "edit") ?>";
finventory_staysafe_reportedit.AutoSuggests["x_recieved_by"] = <?php echo json_encode(array("data" => "ajax=autosuggest&" . $inventory_staysafe_report_edit->recieved_by->LookupFilterQuery(TRUE, "edit"))) ?>;
finventory_staysafe_reportedit.Lists["x_recieved_action"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
finventory_staysafe_reportedit.Lists["x_recieved_action"].Options = <?php echo json_encode($inventory_staysafe_report_edit->recieved_action->Options()) ?>;
finventory_staysafe_reportedit.Lists["x_approver_action"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
finventory_staysafe_reportedit.Lists["x_approver_action"].Options = <?php echo json_encode($inventory_staysafe_report_edit->approver_action->Options()) ?>;
finventory_staysafe_reportedit.Lists["x_statuss"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_description","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"statuss"};
finventory_staysafe_reportedit.Lists["x_statuss"].Data = "<?php echo $inventory_staysafe_report_edit->statuss->LookupFilterQuery(FALSE, "edit") ?>";
finventory_staysafe_reportedit.Lists["x_approved_by"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_firstname","x_lastname","x_staffno",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"users"};
finventory_staysafe_reportedit.Lists["x_approved_by"].Data = "<?php echo $inventory_staysafe_report_edit->approved_by->LookupFilterQuery(FALSE, "edit") ?>";
finventory_staysafe_reportedit.AutoSuggests["x_approved_by"] = <?php echo json_encode(array("data" => "ajax=autosuggest&" . $inventory_staysafe_report_edit->approved_by->LookupFilterQuery(TRUE, "edit"))) ?>;
finventory_staysafe_reportedit.Lists["x_verified_action"] = {"LinkField":"","Ajax":null,"AutoFill":false,"DisplayFields":["","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":""};
finventory_staysafe_reportedit.Lists["x_verified_action"].Options = <?php echo json_encode($inventory_staysafe_report_edit->verified_action->Options()) ?>;
finventory_staysafe_reportedit.Lists["x_verified_by"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_firstname","x_lastname","x_staffno",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"users"};
finventory_staysafe_reportedit.Lists["x_verified_by"].Data = "<?php echo $inventory_staysafe_report_edit->verified_by->LookupFilterQuery(FALSE, "edit") ?>";
finventory_staysafe_reportedit.AutoSuggests["x_verified_by"] = <?php echo json_encode(array("data" => "ajax=autosuggest&" . $inventory_staysafe_report_edit->verified_by->LookupFilterQuery(TRUE, "edit"))) ?>;
...
<?php $inventory_staysafe_report_edit->ShowPageHeader(); ?>
<?php
$inventory_staysafe_report_edit->ShowMessage();
?>
<?php if (!$inventory_staysafe_report_edit->IsModal) { ?>
<form name="ewPagerForm" class="form-horizontal ewForm ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($inventory_staysafe_report_edit->Pager)) $inventory_staysafe_report_edit->Pager = new cPrevNextPager($inventory_staysafe_report_edit->StartRec, $inventory_staysafe_report_edit->DisplayRecs, $inventory_staysafe_report_edit->TotalRecs, $inventory_staysafe_report_edit->AutoHidePager) ?>
<?php if ($inventory_staysafe_report_edit->Pager->RecordCount > 0 && $inventory_staysafe_report_edit->Pager->Visible) { ?>
<div class="ewPager">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($inventory_staysafe_report_edit->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $inventory_staysafe_report_edit->PageUrl() ?>start=<?php echo $inventory_staysafe_report_edit->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($inventory_staysafe_report_edit->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $inventory_staysafe_report_edit->PageUrl() ?>start=<?php echo $inventory_staysafe_report_edit->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $inventory_staysafe_report_edit->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($inventory_staysafe_report_edit->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $inventory_staysafe_report_edit->PageUrl() ?>start=<?php echo $inventory_staysafe_report_edit->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($inventory_staysafe_report_edit->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $inventory_staysafe_report_edit->PageUrl() ?>start=<?php echo $inventory_staysafe_report_edit->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $inventory_staysafe_report_edit->Pager->PageCount ?></span>
</div>
<?php } ?>
<div class="clearfix"></div>
</form>
<?php } ?>
<form name="finventory_staysafe_reportedit" id="finventory_staysafe_reportedit" class="<?php echo $inventory_staysafe_report_edit->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($inventory_staysafe_report_edit->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $inventory_staysafe_report_edit->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="inventory_staysafe_report">
<input type="hidden" name="a_edit" id="a_edit" value="U">
<input type="hidden" name="modal" value="<?php echo intval($inventory_staysafe_report_edit->IsModal) ?>">
<div class="ewEditDiv"><!-- page* -->
<?php if ($inventory_staysafe_report->id->Visible) { // id ?>
	<div id="r_id" class="form-group">
		<label id="elh_inventory_staysafe_report_id" class="<?php echo $inventory_staysafe_report_edit->LeftColumnClass ?>"><?php echo $inventory_staysafe_report->id->FldCaption() ?></label>
		<div class="<?php echo $inventory_staysafe_report_edit->RightColumnClass ?>"><div<?php echo $inventory_staysafe_report->id->CellAttributes() ?>>
<span id="el_inventory_staysafe_report_id">
<span<?php echo $inventory_staysafe_report->id->ViewAttributes() ?>>
<p class="form-control-static"><?php echo $inventory_staysafe_report->id->EditValue ?></p></span>
</span>
<input type="hidden" data-table="inventory_staysafe_report" data-field="x_id" data-page="1" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($inventory_staysafe_report->id->CurrentValue) ?>">
<?php echo $inventory_staysafe_report->id->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($inventory_staysafe_report->date_recieved->Visible) { // date_recieved ?>
	<div id="r_date_recieved" class="form-group">
		<label id="elh_inventory_staysafe_report_date_recieved" for="x_date_recieved" class="<?php echo $inventory_staysafe_report_edit->LeftColumnClass ?>"><?php echo $inventory_staysafe_report->date_recieved->FldCaption() ?></label>
		<div class="<?php echo $inventory_staysafe_report_edit->RightColumnClass ?>"><div<?php echo $inventory_staysafe_report->date_recieved->CellAttributes() ?>>
<span id="el_inventory_staysafe_report_date_recieved">
<input type="text" data-table="inventory_staysafe_report" data-field="x_date_recieved" data-page="1" name="x_date_recieved" id="x_date_recieved" size="30" placeholder="<?php echo ew_HtmlEncode($inventory_staysafe_report->date_recieved->getPlaceHolder()) ?>" value="<?php echo $inventory_staysafe_report->date_recieved->EditValue ?>"<?php echo $inventory_staysafe_report->date_recieved->EditAttributes() ?>>
...
<script type="text/javascript">
ew_CreateDateTimePicker("finventory_staysafe_reportedit", "x_date_recieved", {"ignoreReadonly":true,"useCurrent":false,"format":0});
</script>
<?php } ?>
</span>
<?php echo $inventory_staysafe_report->date_recieved->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($inventory_staysafe_report->reference_id->Visible) { // reference_id ?>
	<div id="r_reference_id" class="form-group">
		<label id="elh_inventory_staysafe_report_reference_id" for="x_reference_id" class="<?php echo $inventory_staysafe_report_edit->LeftColumnClass ?>"><?php echo $inventory_staysafe_report->reference_id->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $inventory_staysafe_report_edit->RightColumnClass ?>"><div<?php echo $inventory_staysafe_report->reference_id->CellAttributes() ?>>
<span id="el_inventory_staysafe_report_reference_id">
<input type="text" data-table="inventory_staysafe_report" data-field="x_reference_id" data-page="1" name="x_reference_id" id="x_reference_id" size="30" maxlength="50" placeholder="<?php echo ew_HtmlEncode($inventory_staysafe_report->reference_id->getPlaceHolder()) ?>" value="<?php echo $inventory_staysafe_report->reference_id->EditValue ?>"<?php echo $inventory_staysafe_report->reference_id->EditAttributes() ?>>
</span>
<?php echo $inventory_staysafe_report->reference_id->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($inventory_staysafe_report->material_name->Visible) { // material_name ?>
	<div id="r_material_name" class="form-group">
		<label id="elh_inventory_staysafe_report_material_name" class="<?php echo $inventory_staysafe_report_edit->LeftColumnClass ?>"><?php echo $inventory_staysafe_report->material_name->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $inventory_staysafe_report_edit->RightColumnClass ?>"><div<?php echo $inventory_staysafe_report->material_name->CellAttributes() ?>>
...
<input type="hidden" data-table="inventory_staysafe_report" data-field="x_material_name" data-page="1" data-value-separator="<?php echo $inventory_staysafe_report->material_name->DisplayValueSeparatorAttribute() ?>" name="x_material_name" id="x_material_name" value="<?php echo ew_HtmlEncode($inventory_staysafe_report->material_name->CurrentValue) ?>"<?php echo $wrkonchange ?>>
<script type="text/javascript">
finventory_staysafe_reportedit.CreateAutoSuggest({"id":"x_material_name","forceSelect":false});
</script>
</span>
<?php echo $inventory_staysafe_report->material_name->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($inventory_staysafe_report->type->Visible) { // type ?>
	<div id="r_type" class="form-group">
		<label id="elh_inventory_staysafe_report_type" for="x_type" class="<?php echo $inventory_staysafe_report_edit->LeftColumnClass ?>"><?php echo $inventory_staysafe_report->type->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $inventory_staysafe_report_edit->RightColumnClass ?>"><div<?php echo $inventory_staysafe_report->type->CellAttributes() ?>>
<span id="el_inventory_staysafe_report_type">
<input type="text" data-table="inventory_staysafe_report" data-field="x_type" data-page="1" name="x_type" id="x_type" size="30" maxlength="50" placeholder="<?php echo ew_HtmlEncode($inventory_staysafe_report->type->getPlaceHolder()) ?>" value="<?php echo $inventory_staysafe_report->type->EditValue ?>"<?php echo $inventory_staysafe_report->type->EditAttributes() ?>>
</span>
<?php echo $inventory_staysafe_report->type->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($inventory_staysafe_report->capacity->Visible) { // capacity ?>
	<div id="r_capacity" class="form-group">
		<label id="elh_inventory_staysafe_report_capacity" for="x_capacity" class="<?php echo $inventory_staysafe_report_edit->LeftColumnClass ?>"><?php echo $inventory_staysafe_report->capacity->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $inventory_staysafe_report_edit->RightColumnClass ?>"><div<?php echo $inventory_staysafe_report->capacity->CellAttributes() ?>>
...
<?php echo $inventory_staysafe_report->capacity->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($inventory_staysafe_report->quantity->Visible) { // quantity ?>
	<div id="r_quantity" class="form-group">
		<label id="elh_inventory_staysafe_report_quantity" for="x_quantity" class="<?php echo $inventory_staysafe_report_edit->LeftColumnClass ?>"><?php echo $inventory_staysafe_report->quantity->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $inventory_staysafe_report_edit->RightColumnClass ?>"><div<?php echo $inventory_staysafe_report->quantity->CellAttributes() ?>>
<span id="el_inventory_staysafe_report_quantity">
<input type="text" data-table="inventory_staysafe_report" data-field="x_quantity" data-page="1" name="x_quantity" id="x_quantity" size="30" maxlength="50" placeholder="<?php echo ew_HtmlEncode($inventory_staysafe_report->quantity->getPlaceHolder()) ?>" value="<?php echo $inventory_staysafe_report->quantity->EditValue ?>"<?php echo $inventory_staysafe_report->quantity->EditAttributes() ?>>
</span>
<?php echo $inventory_staysafe_report->quantity->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($inventory_staysafe_report->recieved_by->Visible) { // recieved_by ?>
	<div id="r_recieved_by" class="form-group">
		<label id="elh_inventory_staysafe_report_recieved_by" class="<?php echo $inventory_staysafe_report_edit->LeftColumnClass ?>"><?php echo $inventory_staysafe_report->recieved_by->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $inventory_staysafe_report_edit->RightColumnClass ?>"><div<?php echo $inventory_staysafe_report->recieved_by->CellAttributes() ?>>
...
<input type="hidden" data-table="inventory_staysafe_report" data-field="x_recieved_by" data-page="1" data-multiple="0" data-lookup="1" data-value-separator="<?php echo $inventory_staysafe_report->recieved_by->DisplayValueSeparatorAttribute() ?>" name="x_recieved_by" id="x_recieved_by" value="<?php echo ew_HtmlEncode($inventory_staysafe_report->recieved_by->CurrentValue) ?>"<?php echo $wrkonchange ?>>
<script type="text/javascript">
finventory_staysafe_reportedit.CreateAutoSuggest({"id":"x_recieved_by","forceSelect":false});
</script>
...
<?php echo $inventory_staysafe_report->recieved_by->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($inventory_staysafe_report->recieved_action->Visible) { // recieved_action ?>
	<div id="r_recieved_action" class="form-group">
		<label id="elh_inventory_staysafe_report_recieved_action" class="<?php echo $inventory_staysafe_report_edit->LeftColumnClass ?>"><?php echo $inventory_staysafe_report->recieved_action->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $inventory_staysafe_report_edit->RightColumnClass ?>"><div<?php echo $inventory_staysafe_report->recieved_action->CellAttributes() ?>>
<span id="el_inventory_staysafe_report_recieved_action">
<div id="tp_x_recieved_action" class="ewTemplate"><input type="radio" data-table="inventory_staysafe_report" data-field="x_recieved_action" data-page="1" data-value-separator="<?php echo $inventory_staysafe_report->recieved_action->DisplayValueSeparatorAttribute() ?>" name="x_recieved_action" id="x_recieved_action" value="{value}"<?php echo $inventory_staysafe_report->recieved_action->EditAttributes() ?>></div>
<div id="dsl_x_recieved_action" data-repeatcolumn="5" class="ewItemList" style="display: none;"><div>
<?php echo $inventory_staysafe_report->recieved_action->RadioButtonListHtml(FALSE, "x_recieved_action", 1) ?>
</div></div>
</span>
<?php echo $inventory_staysafe_report->recieved_action->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($inventory_staysafe_report->recieved_comment->Visible) { // recieved_comment ?>
	<div id="r_recieved_comment" class="form-group">
		<label id="elh_inventory_staysafe_report_recieved_comment" for="x_recieved_comment" class="<?php echo $inventory_staysafe_report_edit->LeftColumnClass ?>"><?php echo $inventory_staysafe_report->recieved_comment->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $inventory_staysafe_report_edit->RightColumnClass ?>"><div<?php echo $inventory_staysafe_report->recieved_comment->CellAttributes() ?>>
<span id="el_inventory_staysafe_report_recieved_comment">
<textarea data-table="inventory_staysafe_report" data-field="x_recieved_comment" data-page="1" name="x_recieved_comment" id="x_recieved_comment" cols="30" rows="4" placeholder="<?php echo ew_HtmlEncode($inventory_staysafe_report->recieved_comment->getPlaceHolder()) ?>"<?php echo $inventory_staysafe_report->recieved_comment->EditAttributes() ?>><?php echo $inventory_staysafe_report->recieved_comment->EditValue ?></textarea>
</span>
<?php echo $inventory_staysafe_report->recieved_comment->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($inventory_staysafe_report->date_approved->Visible) { // date_approved ?>
	<div id="r_date_approved" class="form-group">
		<label id="elh_inventory_staysafe_report_date_approved" for="x_date_approved" class="<?php echo $inventory_staysafe_report_edit->LeftColumnClass ?>"><?php echo $inventory_staysafe_report->date_approved->FldCaption() ?></label>
		<div class="<?php echo $inventory_staysafe_report_edit->RightColumnClass ?>"><div<?php echo $inventory_staysafe_report->date_approved->CellAttributes() ?>>
<span id="el_inventory_staysafe_report_date_approved">
<input type="text" data-table="inventory_staysafe_report" data-field="x_date_approved" data-page="1" name="x_date_approved" id="x_date_approved" placeholder="<?php echo ew_HtmlEncode($inventory_staysafe_report->date_approved->getPlaceHolder()) ?>" value="<?php echo $inventory_staysafe_report->date_approved->EditValue ?>"<?php echo $inventory_staysafe_report->date_approved->EditAttributes() ?>>
</span>
<?php echo $inventory_staysafe_report->date_approved->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($inventory_staysafe_report->approver_action->Visible) { // approver_action ?>
	<div id="r_approver_action" class="form-group">
		<label id="elh_inventory_staysafe_report_approver_action" class="<?php echo $inventory_staysafe_report_edit->LeftColumnClass ?>"><?php echo $inventory_staysafe_report->approver_action->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $inventory_staysafe_report_edit->RightColumnClass ?>"><div<?php echo $inventory_staysafe_report->approver_action->CellAttributes() ?>>
<span id="el_inventory_staysafe_report_approver_action">
<div id="tp_x_approver_action" class="ewTemplate"><input type="radio" data-table="inventory_staysafe_report" data-field="x_approver_action" data-page="1" data-value-separator="<?php echo $inventory_staysafe_report->approver_action->DisplayValueSeparatorAttribute() ?>" name="x_approver_action" id="x_approver_action" value="{value}"<?php echo $inventory_staysafe_report->approver_action->EditAttributes() ?>></div>
<div id="dsl_x_approver_action" data-repeatcolumn="5" class="ewItemList" style="display: none;"><div>
<?php echo $inventory_staysafe_report->approver_action->RadioButtonListHtml(FALSE, "x_approver_action", 1) ?>
</div></div>
</span>
<?php echo $inventory_staysafe_report->approver_action->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($inventory_staysafe_report->approver_comment->Visible) { // approver_comment ?>
	<div id="r_approver_comment" class="form-group">
		<label id="elh_inventory_staysafe_report_approver_comment" for="x_approver_comment" class="<?php echo $inventory_staysafe_report_edit->LeftColumnClass ?>"><?php echo $inventory_staysafe_report->approver_comment->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $inventory_staysafe_report_edit->RightColumnClass ?>"><div<?php echo $inventory_staysafe_report->approver_comment->CellAttributes() ?>>
<span id="el_inventory_staysafe_report_approver_comment">
<textarea data-table="inventory_staysafe_report" data-field="x_approver_comment" data-page="1" name="x_approver_comment" id="x_approver_comment" cols="30" rows="4" placeholder="<?php echo ew_HtmlEncode($inventory_staysafe_report->approver_comment->getPlaceHolder()) ?>"<?php echo $inventory_staysafe_report->approver_comment->EditAttributes() ?>><?php echo $inventory_staysafe_report->approver_comment->EditValue ?></textarea>
</span>
<?php echo $inventory_staysafe_report->approver_comment->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($inventory_staysafe_report->statuss->Visible) { // statuss ?>
	<div id="r_statuss" class="form-group">
		<label id="elh_inventory_staysafe_report_statuss" for="x_statuss" class="<?php echo $inventory_staysafe_report_edit->LeftColumnClass ?>"><?php echo $inventory_staysafe_report->statuss->FldCaption() ?></label>
		<div class="<?php echo $inventory_staysafe_report_edit->RightColumnClass ?>"><div<?php echo $inventory_staysafe_report->statuss->CellAttributes() ?>>
<span id="el_inventory_staysafe_report_statuss">
<select data-table="inventory_staysafe_report" data-field="x_statuss" data-page="1" data-value-separator="<?php echo $inventory_staysafe_report->statuss->DisplayValueSeparatorAttribute() ?>" id="x_statuss" name="x_statuss"<?php echo $inventory_staysafe_report->statuss->EditAttributes() ?>>
<?php echo $inventory_staysafe_report->statuss->SelectOptionListHtml("x_statuss") ?>
</select>
</span>
<?php echo $inventory_staysafe_report->statuss->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($inventory_staysafe_report->approved_by->Visible) { // approved_by ?>
	<div id="r_approved_by" class="form-group">
		<label id="elh_inventory_staysafe_report_approved_by" class="<?php echo $inventory_staysafe_report_edit->LeftColumnClass ?>"><?php echo $inventory_staysafe_report->approved_by->FldCaption() ?></label>
		<div class="<?php echo $inventory_staysafe_report_edit->RightColumnClass ?>"><div<?php echo $inventory_staysafe_report->approved_by->CellAttributes() ?>>
...
<input type="hidden" data-table="inventory_staysafe_report" data-field="x_approved_by" data-page="1" data-value-separator="<?php echo $inventory_staysafe_report->approved_by->DisplayValueSeparatorAttribute() ?>" name="x_approved_by" id="x_approved_by" value="<?php echo ew_HtmlEncode($inventory_staysafe_report->approved_by->CurrentValue) ?>"<?php echo $wrkonchange ?>>
<script type="text/javascript">
finventory_staysafe_reportedit.CreateAutoSuggest({"id":"x_approved_by","forceSelect":false});
</script>
</span>
<?php echo $inventory_staysafe_report->approved_by->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($inventory_staysafe_report->verified_date->Visible) { // verified_date ?>
	<div id="r_verified_date" class="form-group">
		<label id="elh_inventory_staysafe_report_verified_date" for="x_verified_date" class="<?php echo $inventory_staysafe_report_edit->LeftColumnClass ?>"><?php echo $inventory_staysafe_report->verified_date->FldCaption() ?></label>
		<div class="<?php echo $inventory_staysafe_report_edit->RightColumnClass ?>"><div<?php echo $inventory_staysafe_report->verified_date->CellAttributes() ?>>
<span id="el_inventory_staysafe_report_verified_date">
<input type="text" data-table="inventory_staysafe_report" data-field="x_verified_date" data-page="1" name="x_verified_date" id="x_verified_date" placeholder="<?php echo ew_HtmlEncode($inventory_staysafe_report->verified_date->getPlaceHolder()) ?>" value="<?php echo $inventory_staysafe_report->verified_date->EditValue ?>"<?php echo $inventory_staysafe_report->verified_date->EditAttributes() ?>>
</span>
<?php echo $inventory_staysafe_report->verified_date->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($inventory_staysafe_report->verified_action->Visible) { // verified_action ?>
	<div id="r_verified_action" class="form-group">
		<label id="elh_inventory_staysafe_report_verified_action" class="<?php echo $inventory_staysafe_report_edit->LeftColumnClass ?>"><?php echo $inventory_staysafe_report->verified_action->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $inventory_staysafe_report_edit->RightColumnClass ?>"><div<?php echo $inventory_staysafe_report->verified_action->CellAttributes() ?>>
<span id="el_inventory_staysafe_report_verified_action">
<div id="tp_x_verified_action" class="ewTemplate"><input type="radio" data-table="inventory_staysafe_report" data-field="x_verified_action" data-page="1" data-value-separator="<?php echo $inventory_staysafe_report->verified_action->DisplayValueSeparatorAttribute() ?>" name="x_verified_action" id="x_verified_action" value="{value}"<?php echo $inventory_staysafe_report->verified_action->EditAttributes() ?>></div>
<div id="dsl_x_verified_action" data-repeatcolumn="5" class="ewItemList" style="display: none;"><div>
<?php echo $inventory_staysafe_report->verified_action->RadioButtonListHtml(FALSE, "x_verified_action", 1) ?>
</div></div>
</span>
<?php echo $inventory_staysafe_report->verified_action->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($inventory_staysafe_report->verified_comment->Visible) { // verified_comment ?>
	<div id="r_verified_comment" class="form-group">
		<label id="elh_inventory_staysafe_report_verified_comment" for="x_verified_comment" class="<?php echo $inventory_staysafe_report_edit->LeftColumnClass ?>"><?php echo $inventory_staysafe_report->verified_comment->FldCaption() ?><?php echo $Language->Phrase("FieldRequiredIndicator") ?></label>
		<div class="<?php echo $inventory_staysafe_report_edit->RightColumnClass ?>"><div<?php echo $inventory_staysafe_report->verified_comment->CellAttributes() ?>>
<span id="el_inventory_staysafe_report_verified_comment">
<textarea data-table="inventory_staysafe_report" data-field="x_verified_comment" data-page="1" name="x_verified_comment" id="x_verified_comment" cols="30" rows="4" placeholder="<?php echo ew_HtmlEncode($inventory_staysafe_report->verified_comment->getPlaceHolder()) ?>"<?php echo $inventory_staysafe_report->verified_comment->EditAttributes() ?>><?php echo $inventory_staysafe_report->verified_comment->EditValue ?></textarea>
</span>
<?php echo $inventory_staysafe_report->verified_comment->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($inventory_staysafe_report->verified_by->Visible) { // verified_by ?>
	<div id="r_verified_by" class="form-group">
		<label id="elh_inventory_staysafe_report_verified_by" class="<?php echo $inventory_staysafe_report_edit->LeftColumnClass ?>"><?php echo $inventory_staysafe_report->verified_by->FldCaption() ?></label>
		<div class="<?php echo $inventory_staysafe_report_edit->RightColumnClass ?>"><div<?php echo $inventory_staysafe_report->verified_by->CellAttributes() ?>>
...
<input type="hidden" data-table="inventory_staysafe_report" data-field="x_verified_by" data-page="1" data-value-separator="<?php echo $inventory_staysafe_report->verified_by->DisplayValueSeparatorAttribute() ?>" name="x_verified_by" id="x_verified_by" value="<?php echo ew_HtmlEncode($inventory_staysafe_report->verified_by->CurrentValue) ?>"<?php echo $wrkonchange ?>>
<script type="text/javascript">
finventory_staysafe_reportedit.CreateAutoSuggest({"id":"x_verified_by","forceSelect":false});
</script>
</span>
<?php echo $inventory_staysafe_report->verified_by->CustomMsg ?></div></div>
	</div>
<?php } ?>
</div><!-- /page* -->
<?php if (!$inventory_staysafe_report_edit->IsModal) { ?>
<div class="form-group"><!-- buttons .form-group -->
	<div class="<?php echo $inventory_staysafe_report_edit->OffsetColumnClass ?>"><!-- buttons offset -->
<button class="btn btn-primary ewButton" name="btnAction" id="btnAction" type="submit"><?php echo $Language->Phrase("SaveBtn") ?></button>
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $inventory_staysafe_report_edit->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div><!-- /buttons offset -->
</div><!-- /buttons .form-group -->
<?php } ?>
</form>
<script type="text/javascript">
finventory_staysafe_reportedit.Init();
</script>
<?php
$inventory_staysafe_report_edit->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$inventory_staysafe_report_edit->Page_Terminate();
?>
