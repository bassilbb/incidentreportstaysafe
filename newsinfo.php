...
class cnews extends cTable {
	...
	function __construct() {
		...
		// code
		$this->code = new cField('news', 'news', 'x_code', 'code', '`code`', '`code`', 3, -1, FALSE, '`code`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->code->Sortable = TRUE; // Allow sort
		$this->code->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['code'] = &$this->code;

		// news
		$this->news = new cField('news', 'news', 'x_news', 'news', '`news`', '`news`', 200, -1, FALSE, '`news`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->news->Sortable = TRUE; // Allow sort
		$this->fields['news'] = &$this->news;

		// news_image
		$this->news_image = new cField('news', 'news', 'x_news_image', 'news_image', '`news_image`', '`news_image`', 200, -1, TRUE, '`news_image`', FALSE, FALSE, FALSE, 'IMAGE', 'FILE');
		$this->news_image->Sortable = TRUE; // Allow sort
		$this->news_image->ImageResize = TRUE;
		$this->news_image->UploadMultiple = TRUE;
		$this->news_image->Upload->UploadMultiple = TRUE;
		$this->news_image->UploadMaxFileCount = 2;
		$this->fields['news_image'] = &$this->news_image;

		// news_content
		$this->news_content = new cField('news', 'news', 'x_news_content', 'news_content', '`news_content`', '`news_content`', 201, -1, FALSE, '`news_content`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXTAREA');
		$this->news_content->Sortable = TRUE; // Allow sort
	...
	function LoadListRowValues(&$rs) {
		...
		$this->news->setDbValue($rs->fields('news'));
	...
	function RenderEditRow() {
		...
		$this->news->EditValue = $this->news->CurrentValue;
		...
		$this->news_content->EditValue = $this->news_content->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->news->Exportable) $Doc->ExportCaption($this->news);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->news->Exportable) $Doc->ExportField($this->news);
